...
At the request of ICES and GFCM, the WGEEL working group will make significant changes to its data reporting process. This change is essential to harmonize the flow of information, streamline the work alongside other working groups from ICES and enable a full use the t ofools provided by ICES and GFCM for quality control, reporting and stock assessment.

The new process will consist of several stages and will require sustained effort from all those involved to ensure long-term data quality. Thus, the WGEEL will transition from a data call comprising nine annexes to a data call structured around three formats: the Diadromous database (which will comprise a survey and a stock module), the RDBES database and the RDBfis database. The steps towards building the new data system are illustrated in an infographic (Figure @fig-datacall_workflow).
...
The first phase will begin next year, for example, with the entry of data from scientific surveys into the ICES Scientific Surveys Database. All the changes planned for 2027 are summarized in the below (see @planned_change).



::: {.callout-note icon="false" #planned_change}
### For next year
...
::: {custom-style="Annex heading"}
Annex Commercial series in Annex 9
:::
...
::: {custom-style="Annex heading"}
Annex  Commercial series in annex 2 and 3
:::
