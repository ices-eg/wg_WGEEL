...
```{r }
#| label: extract_column_names
#| eval: FALSE

# this file is not accessible unless you have signed the agreement
readr::read_csv("data/)
...
## DIADROMOUS DATABASE (Maria and Cedric)
...
The Diadromous DB is currently being developped in ICES. A first version of the database and API is planned for next year (scientific survey data). Currently the submission format for Diadromous survey data can be found here: <https://datsu.ices.dk/web/selRep.aspx?Dataset=172.> This submission format will be used in the next data call for annexes 1 to 3 and 9. It was tested during the WGEEL. Feedback on issues on the format is here : ices-tools-dev/DiadromousData: Development of the eel database and format. If new issues arise and write access to the repository is needed, please contact accessions\@ices.dk.

The current data for series and metrics (annex 1 to 3 and 9) will be imported (bulk import) during the next year. For 2027, all annex 1 (recruitment) data will be submitted in the Diadromous survey format, Series identified as commercial sampling will be excluded from the submission for both historical and 2026 data. These data will be submitted in RDBES/RDBFIS starting from 2028.
...
## Recreational landings (Clarisse)

Recreational fishing data will continue to be incorporated into the WGEEL database through Annex 4 of the standard data call. Eventually, the RDBES database (for ICES countries) and the RDBFIS database (for non-ICES countries) are expected to incorporate this data, which will be integrated through the RDBES and RDBFIS data calls.

::: {.callout-note icon="false"}
## For next year

Recreational fisheries data will be incorporate via the classic WGEEL data call using the annex 4
:::


## Destination of current biometric sampling (Diadromous Survey) (Clarisse Elsa)

Depending on the protocols used to collect biometric data, the resulting data are either fishery-dependent (for example, individuals measured on a fishing vessel or at the fish auction) or fishery-independent (for example, measurements taken during electrofishing or scientific surveys at sea). Work was carried out during the WGEEL to classify existing series into these two categories (Table X).Below are explanation about what process to follow for integrating this data based on its category.

### Series (fishery independent data)

With regard to data from fishery-independent protocols, for all countries (both ICES and non-ICES countries), these data must be integrated into the scientific survey database via the DATSU process starting next year. A list has been created to identify the data series that will be integrated via the DATSU process (Annex X)



### Commercial and recreational sampling (fishery dependent data)

Regarding biometric data collected through protocols related to commercial or recreational fisheries (fishery-dependent data, Table X), the changes will take place in several stages:

 * Next year, biometric data will not be requested in the WGEEL data call.
 * In the future, this data will be integrated through the RDBES procedure. To make this possible, for each data series, the data provider must identify the hierarchy to be used for integration into the RDBES database. The hierarchy depends on the protocol used to collect the data (for example: observers at sea on a selection of vessels who are required to collect data only for eels). It is strongly recommended that data provider read the documentation available on the [RDBES Git repository](https://github.com/ices-tools-dev/RDBES/blob/master/Documents/RDBES%20Documentation%20of%20the%20Data%20Model.docx) and take one of the training courses offered by ICES (see website).


Table of series removed from our current database (ELSA)
Full table identifying where the series go
