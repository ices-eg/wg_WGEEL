...
The [EDMO](http://European%20Directory%20of%20Marine%20Organisations%20(EDMO)%20-%20SeaDataNet) will be needed to submit data through DATSU. This is the first table. WGEEL members were asked to checked in [ICES directory of EDMO codes](https://vocab.ices.dk/?ref=1398) or in the [EDMO](http://European%20Directory%20of%20Marine%20Organisations%20(EDMO)%20-%20SeaDataNet) web site to see if the national data reporter or data collectors at stations had an EDMO. When missing, WGEEL country representative were asked to fill in all details to prepare the table for submission. 
The table prepared for maris (the structure handling the EDMO database) to fill in new EDMO codes is presented in Annex XX
...
Currently commercial landings are included in Annex 4 and stored in the WGEEL database. While this system includes a quality control procedure, it does not adhere to ICES standard, nor provide a tracking system for data upload and transmission. Among diadromous working group, WGEEL will initiate the transition to a full use of the RDBES. The DIASPARA project has developed an habitat referential that follows the hierarchy of stock structure used by diadromous working groups, and extends the marine areas used by GFCM and ICES to neighboring continental waters.

The **RDBES (Regional Database and Estimation System)** is a database system (or "data model") developed and hosted by ICES. It involves a system of data submission and analysis which maintains appropriate levels of access (data levels) for users in different roles (e.g. national estimators, stock coordinators, and stock assessors). Translation of data between these levels is done using the TAF (transparent assessment framework), ensuring that methods for processing data are open and standardizable between users.

RDBES organizes fisheries sampling data into a set of 13 standardized "hierarchies" representing commercial fisheries, recreational fisheries, scientific surveys, and associated biological and laboratory sampling. The hierarchical model links observations from sampling programmes and fishing activities down to individual fish measurements and analyses, ensuring traceability of data and supporting statistically robust estimation procedures used in ICES stock assessments and fisheries advice. To accomodate sampling programmes with different structures, there is a selection of 13 different hiearchies of data tables (named H1 to H13). See the documentation at the [RDBES github repository](https://github.com/ices-tools-dev/RDBES/) for more information.

National estimators have access to the most detailed level of data from RDBES (the "Detailed data" level). For this data to be accessible to other users, the level of detail needs to be reduced. It is the task of each country's national estimator to download their respective data, perform a simple aggregation-analysis (using a TAF template), and upload the aggregated data to the less detailed "National estimates" level. This data then becomes available for the Stock coordinator (using the catch estimation format CEF).

 WGEEL tested using the [icesRDBES R package](https://github.com/ices-tools-prod/icesRDBES) and the web-based RDBES data portal ([rdbes.ices.dk](https://rdbes.ices.dk)) for downloading data from the perspective of a national estimator. After being granted access, the process of downloading data was quite simple and resulted .csv files for various data, for example commercial landings. An example script for how to do this exists in the wgeel GitHub repository under the name "script-test-rdbes-se.R". 
 
 > cedric's comment. We need to merge the branch and leave this file in the "main" branch + provide a link to github file

Furthermore, we also compared the data currently available in RDBES with the data available in WGEEL's database. This was done using an export from RDBES including all countries' commercial landings on European eel. 14 of the 28 countries having commercial landings data in the WGEEL database did not have commercial landings in RDBES. These countries are Albania, Algeria, Croatia, **Czech Republic**, Greece, **Ireland**, Italy, Libya, Montenegro, **Morocco**, **Norway**, Slovenia, Tunisia, and Türkiye. Note that Mediterranean countries submit their commercial landings to RDBFIS, not RDBES. Non-Mediterranean countries are here printed in bold. Further, for the countries with data available in RDBES, many had differing total landings reported to RDBES and WGEEL, indicating that either of the databases may be incomplete for these countries (Figure @fig-rdbes_comparison). 

![Total landed biomass (kg) of european eel (*Anguilla anguilla*), as reported to WGEEL and RDBES for all years and countries in which RDBES data is available. Empty black bars indicate RDBES numbers, and blue bars indicate WGEEL numbers. The horisontal axis is log-shifted. Note that catches of eel in the Mediterranean are not reported to RDBES but are reported to WGEEL, which explains some inconsistencies.](images/plot-rdbes-vs-wgeel-landings.png){#fig-rdbes_comparison}

For the purpose of accessing data, the RDBES system currently seems to be ready. The main body of work will likely be in ensuring that data are appropriately submitted to the database, and that pending issues concerning the inclusion of continental habitat and stages are resolved.
...
## For next year
Recreational fisheries data will be incorporate via the classic WGEEL data call using the annex 5
:::
...
![The habitat database for WGEEL showing the regional level](images/fig-hierarchical){#fig-hierarchical.png}

The hierarchical structure of the habitat ranges from stock level to river section level or lagoon level (Figure @fig-hierarchical). The hierarchy of countries and EMU is provided but does not follow the requirement of a nested stock structure allowing to aggregate from lower to upper step. In addition, EMU's limit currently do not respect the division between marine and continental habitat. 
In order to build a comprehensive and consistent stock assessment, it is required that areas code used for the continental habitat are used. In the marine area, the standard ICES and GFCM reformed divisions will be used.
...
![The habitat database for WGEEL showing the regional level](images/habitat_regional.png){#fig-regional.png}

![The habitat database for WGEEL showing the regional level](images/habitat_subcomponent.png){#fig-subcomponent.png}
...
![The habitat database for WGEEL showing the river level](images/habitat_river.png){#fig-habitat_river.png}

## Recommendations

To : WGRDBESGOV 

 * Modifications of the format to support freshwater landings submission :
+ Make area optional or use the corresponding ICES area 
 * Fields 11 to 14 to be made optional for eel and salmon and Trutta
 * WLTYP change the documentation to reflect the documentation in WLTYP 
(Add CL coastal lagoon) use (MO MC FW CL T)
* Add life stage to landings data (the biological ) (See Vocab Stage) Check if we can make it conditional mandatory Landing location (Should be conditional mandatory on Freshwater - most freshwater will not have harbour)

To : RDBES ?

WGEEL recommends that the lagoon or river (river basin) level be used as a reference using the habitat database code (area).
...
WGEEL recommends the provision of the habitat vocabulary online, with menus allowing to select the diadromous working group (WGBAST, WGNAS or WGEEL) and then to select a hierarchy to display data. This online tool would allow users to select the code of the area for the submission in RDBES or the for data submission in the diadromous database.

To : WGRDBESGOV

WGEEL recommends to integrate the continental habitats in the RDBES datacall in 2027 


![WGEEL habitat at the regional level](images/clipboard-2274661588.png){#fig-regional}
...
- Freshwater landings will be incorporate into RDBES database but also in the WGEEL database using the annex 4.
- Marine/transitional/coastal landings will be incorporate via the classic WGEEL data call using the annex 4.
- Recreational fisheries data will be incorporate via the classic WGEEL data call using the annex 5.
- The biometric data linked to a scientific survey (fishery independent data) will be incorporate during the WGEEL data call using the DATSU process.
- The biometric data linked to a commercial or recreational landings (fishery dependent data) will not be incorporate during the WGEEL data call.
:::
...
Annex 61. The table of EDMO codes
:::

::: {custom-style="Annex heading"}
Annex 62. Commercial series in Annex 9 
:::

```{r}
#| label: tbl-annex9
#| tbl-cap: "Series identified in annex 9 which correspond to commercial fisheries and should not be submitted to the Diadromous database but be submitted instead to RDBFIS in 2028."

readxl::read_excel("data/annex9.xlsx") |>
  select(sai_name, sai_cou_code, sai_emu_nameshort, sai_hty_code)  |> 
  flextable()  |>
  flextable2ICES()
...
::: {custom-style="Annex heading"}
Annex 63. Commercial series in annex 2 and 3 
:::

```{r}
#| label: tbl-annex2and3
#| tbl-cap: "Series identified in annex 2 and 3 which correspond to commercial fisheries and should not be submitted to the Diadromous database but be submitted instead to RDBFIS in 2028."
