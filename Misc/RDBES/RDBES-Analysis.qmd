...
## Commercial landings (Eirik)

This is where we talk about RDBES
...
Depending on the protocols used to collect biometric data, the resulting data are either fishery-dependent (for example, individuals measured on a fishing vessel or at the fish auction) or fishery-independent (for example, measurements taken during electrofishing or scientific surveys at sea). Below are explanation about what process to follow for integrating this data based on its category.
...
Concernant les données de biometrie collectées via des protocoles liées à des pêcheries commerciales ou récréatives (fishery dependent data), les changements vont se faire en plusieurs étapes:

- l'année prochaine, les données de biometrie ne seront pas demandées dans l'appel à données WGEEL seront saisies via le processus classique du WGEEL

Table of series removed from our current database (ELSA) Full table identifying where the series go

## Spatial structure of habitat

The regional structure of the reporting was proposed in ICES 2025 and was developped during the diaspara project. The diaspara [habitat database](!DIASPARA habitat database creation script) is available for download in [Zenodo](https://doi.org/10.5281/zenodo.18155125). The habitat is divided into marine habitat (which follows the current ICES and GFCM areas) and continental habitat

The hierarchical structure of the habitat ranges from stock level to river section level (Figure XX) . This hierarchical level allows to aggregate data from the lower level to the upper level. E.g data collected for one river or one lagoon can be agregated as a regional or sub component (Substock) level

![The hierachical levels in the WGEEL part of the Habitat database for diadromous fishes](images/clipboard-1857698266.png)

## Conclusion and future planning

### Recommendation : Additional part about which area level will be reported \[Cedric\]

Explain the different hierarchies, and explain how to report.

### Recommendation

To WGRDBESGOV

Modifications of the format to support freshwater landings submission : Make area optional or use the corresponding ICES area

Fields 11 to 14 to be made optional for eel and salmon and Trutta WLTYP change the documentation to reflect the documentation in WLTYP (Add CL coastal lagoon) use (MO MC FW CL T)

Add life stage (See Vocab Stage) Check if we can make it conditional mandatory Landing location (Should be conditional mandatory on Freshwater - most freshwater will not have harbour)

Add lifestage to RDBES

Have a repository about
