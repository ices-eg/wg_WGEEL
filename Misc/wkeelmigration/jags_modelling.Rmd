...
ggplot(get_pattern_month(myfit_silver),aes(x=month,y=proportion))+
  ...
  theme_bw()
```
We have 3 clusters: cluster 1 corresponds to a migration wave starting in late
spring and ending in late autumn. Cluster 2 corresponds to a migration wave
starting in october and which can last a little bit during winter. Cluster
3 corresponds to a migration wave really centered around october.
...
Cluster 5 corresponds to a migration 5 concentrated in June and July. Cluster 2
is a bit similar, but more widespread from May to August. Cluster 1 has a
delayed peak with a high proportion in late summer / early autumn. Clusters 3
and 4 are quite similar with widespread waves centered around summer and a
bimodality and early autumn, with cluster 3 more widepread than cluster 4.
...
```
There is no clear spatial pattern in the clustering. This is not necessarily surprising: eels display an ontongenic shift during their life stage, 
from a migratory behaviour towards sedentary behaviour (Imbert et al. 2010). 
...
and the attractivity / passability of those fishway vary among seasons.
