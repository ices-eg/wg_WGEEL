...
file_stages <- c("G","S","S","Y","Y")
file_habitats <- c("AL","C","F","C","F")
for (i in 1:length(files)){
	load(files[i])
...
big_table_closure <- resc %>%	
		...
		mutate(duplicate=cell_spec(duplicate, "html",color="white", background= ifelse(duplicate>1,"red","white")),
				fishery_closure_percent = 
						case_when(fishery_closure_type=='PS' & reason_for_closure !="EU Closure" ~ cell_spec(fishery_closure_percent, "html",
										color ="white",
										background = "red",
										link = "#",
								...
								fishery_closure_type=='T' & reason_for_closure !="EU Closure" ~ cell_spec(fishery_closure_percent, "html",
										...
										link = "#",
										tooltip =paste0(fishery_closure_type," reason:",reason_for_closure, "; Comment: ", comment )),
								reason_for_closure =="EU Closure" ~ cell_spec(fishery_closure_percent, "html",
										color ="black",
										background = "gold",
										link = "#",
										tooltip =paste0(fishery_closure_type," reason:",reason_for_closure, "; Comment: ", comment )),
	...
	if (nrow(res4)>1){
		...
		g <- ggplot(res4,aes(x = month)) +
				...
				geom_label(aes(x=month,y=66+closure/3,label=closure, fill=reason_for_closure),
						...
						alpha=1,
						data=resc3) +
				scale_fill_manual(values=c("none given"="grey",
								"Other"="thistle",
								"EMP"="deepskyblue",
								"EU Closure"="gold")							
				)+
...
for (i in 1:nrow(countries)){
	...
	pandoc.p(str_c("Table ",nrtable," : Closures in ", countries$cou_country[i] , ". If the file is html, you can point to a value to see the reason for closure and the comment,
							T total (black), PS partial in space (red), PT partial in time (blue), PTS partial in space and time (puple). 
	...
	for (j in 1:length(the_emus_sel)){
		...
		fntablelossemp(dat=lossGAL, hty = c("T","F","FTC"), emu=the_emu)
		
		fntablelosseu(dat=lossGAL, hty = c("T","F","FTC"), emu=the_emu)
		...
		fntablelossemp(dat=lossYF, hty = c("C","MO"), emu=the_emu)
		
		fntablelosseu(dat=lossYF, hty = c("C","MO"), emu=the_emu)
