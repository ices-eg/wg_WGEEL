...
con<-dbConnect(Postgres(),host="localhost",dbname="wgeel",user="hilaire")
...
```{r maprecruit}
average_pattern <- recruitment_subset |>
  group_by(ser_nameshort, das_month) |>
  summarize(das_value = sum(das_value, na.rm = TRUE), .groups = "keep") |>
  ungroup() |>
  group_by(ser_nameshort) |>
  mutate(das_value = das_value/sum(das_value, na.rm = TRUE)) |>
  ungroup() |>
  left_join(ser2, by = "ser_nameshort")
...
ggplot(cou) +
  ...
  geom_polygon(data = formatted_patter,
               aes(x = x,
                   ...
                   col = as.factor(month),
                   group = interaction(ser_nameshort, as.factor(month))),
               alpha = .3,
               inherit.aes = FALSE)   +
  scale_color_manual("Month", values = palette) +
  scale_fill_manual("Month", values = palette) +
  coord_sf(xlim = c(3000000, 4500000),
           ylim = c(2000000, 4500000)) +
  xlab("") +
  ylab("")
```
...
cl <- makeCluster(3, 'FORK')
comparisonwaic <- parSapply(cl,2:7,
       function(nbclus){
         library(R2jags)
         mydata <- build_data(nbclus)

             m1.jags <- jags(data = mydata, 
                inits = list(generate_init(nbclus, mydata)[[1]]), 
                parameters.to.save = c("alpha_group",
                                       "cluster"), 
                model.file = "jags_model.txt",
                n.chains = 1, 
                n.iter = 10000+200000, 
                n.burnin = 200000, 
                n.thin = 1, n.adapt = 4000)
...
myclassif <- get_pattern_month2(myfit_recruitment)
...
```{r mapglasseeloutput}
pat_group <- get_pattern_month(myfit_recruitment, "group")
...
pat_group <- pat_group |>
  ...
  mutate(ser_nameshort = stringr::str_extract(group_name, "^[^\\.]+"),
...
save(g, file="glasseel_monitoring_ggplot.rdata")
ggsave("glasseel_monitoring.png", g, height=12/2.54, width = 16/2.54, dpi = 300)
```
...
```{r mapsilver}
average_pattern <- silvereel_subset |>
  group_by(ser_nameshort, das_month) |>
  summarize(das_value = sum(das_value, na.rm = TRUE), .groups = "keep") |>
  ungroup() |>
  group_by(ser_nameshort) |>
  ...
  ungroup() |>
  left_join(ser2, by = "ser_nameshort")
average_pattern$ser_y[average_pattern$ser_nameshort=="DaugS"] <- 57.049729
average_pattern$ser_x[average_pattern$ser_nameshort=="DaugS"] <- 24.059393
...
comparison <- parSapply(cl, 2:7,
       function(nbclus){
         ...
         while (!adapted){
           tryCatch({
             runjags.options(adapt.incomplete="error")
             res <- run.jags("jags_model.txt", monitor= c("deviance","WAIC",
                                                          "alpha_group",
                                                          "cluster"),
                        summarise=FALSE, adapt=40000, method="parallel",
                        sample=2000,burnin=100000,n.chains=1,
...
myclassif_silver <- get_pattern_month2(myfit_silver)
...
```{r mapglasseeloutput}
pat_group <- get_pattern_month(myfit_silver, "group")
...
pat_group$ser_y[pat_group$ser_nameshort=="DaugS"] <- 57.049729
pat_group$ser_x[pat_group$ser_nameshort=="DaugS"] <- 24.059393
...
save(g, file = "silver_monitoringggplot.rdata")
ggsave("silver_monitoring.png", g, height=12/2.54, width = 16/2.54, dpi = 300)
```
...
myclassif_yellow <- get_pattern_month2(myfit_yellow)
