...
-   [Glass Eel](#glass-eel)
    ...
    -   [Results](#results)
-   [Yellow](#yellow)
    -   [Coastal/marine waters](#coastalmarine-waters)
        -   [Data selection](#data-selection-1)
        -   [Data preparation](#data-preparation-1)
        -   [Running the model](#running-the-model-1)
    -   [Results](#results-1)
    -   [transitional waters](#transitional-waters)
        -   [Data selection](#data-selection-2)
        -   [Data preparation](#data-preparation-2)
        -   [Running the model](#running-the-model-2)
    -   [Results](#results-2)
    -   [freshwater waters](#freshwater-waters)
        -   [Data selection](#data-selection-3)
        -   [Data preparation](#data-preparation-3)
        -   [Running the model](#running-the-model-3)
    -   [Results](#results-3)
    -   [All habitats](#all-habitats)
        -   [Data selection](#data-selection-4)
        -   [Data preparation](#data-preparation-4)
        -   [Running the model](#running-the-model-4)
    -   [Results](#results-4)
-   [Silver eel](#silver-eel)
    -   [marine open, coastal and transitional
        waters](#marine-open-coastal-and-transitional-waters)
        -   [Data selection](#data-selection-5)
        -   [Data preparation](#data-preparation-5)
        -   [Running the model](#running-the-model-5)
    -   [Results](#results-5)
    -   [freshwater waters](#freshwater-waters-1)
        -   [Data selection](#data-selection-6)
        -   [Data preparation](#data-preparation-6)
        -   [Running the model](#running-the-model-6)
    -   [Results](#results-6)
-   [Siver/Yellow](#siveryellow)
    -   [Data selection](#data-selection-7)
    -   [Data preparation](#data-preparation-7)
    -   [Running the model](#running-the-model-7)
    -   [Results](#results-7)

Introduction
============

We start by loading the rdata provided by Cédric who has imported and
edited all the xlsx files. He also provides a very good overview of the
content [here](landings_seasonality.md). Based on this job, we will try
to carry out a similar analysis as for [seasonality](jags_modelling.md).
More specifically, we can use the same Bayesian model to make a
clustering of time series. For each stage, we will build a data set that
gives for each season, and each EMU (and perhaps habitat), the
proportion of catches per month.

For convenience, we rename the data.frame with names consistent with the
seasonality data.set
...
Similarly to seasonality, we will build season. For glass eels, seasons
are rather consistent in whole Europe, so we use the same definition as
in seasonality: Here, we split in october (starts of catches in Spain)
and a season y will correspond to ostober - december y-1 and january to
september y.
...
Now we should carry out data selection, more specifically, we want to
eliminate rows with two many missing data, too much zero and to check
whether there are no duplicates (though Cedric already did it)
...
missing months corresponding to insignificant months / closed months,
and we compute proportions per month for each year.
...
Given that the number of used clusters do not increase much after 4 and
that the silhouette tends to decrease, we use 4 clusters.
...
![](jags_landings_files/figure-markdown_strict/unnamed-chunk-13-1.png)
...
Clusters 1 starts in autum and last still january. Cluster 2 is shifter
one month later and lasts longer. Cluster 3 corresponds to catches in
march/may. Cluster 4 is very flat and is not really attributed.
...
We can also look at the belonging of the different groups.
    ...
    ## 2  ES_Basq      1       1    NA    NA 10000    NA
    ## 3  ES_Cata      1       1    NA    NA 10000    NA
    ## 4  FR_Adou      1       1    NA    NA 10000    NA
    ## 14 ES_Astu      2       1    NA    NA 10000    NA
    ## 15 ES_Basq      2       1    NA    NA 10000    NA
    ## 16 ES_Cant      2       1    NA    NA 10000    NA
    ## 17 ES_Cata      2       1    NA    NA 10000    NA
    ## 18 ES_Mino      2       1    NA    NA 10000    NA
    ## 20 FR_Adou      2       1    NA    NA 10000    NA
    ## 1  ES_Astu      1       2  8937    NA  1063    NA
    ## 6  FR_Bret      1       2 10000    NA    NA    NA
    ## 7  FR_Garo      1       2 10000    NA    NA    NA
    ## 8  FR_Loir      1       2 10000    NA    NA    NA
    ## 19 ES_Vale      2       2 10000    NA    NA    NA
    ## 22 FR_Bret      2       2 10000    NA    NA    NA
    ## 23 FR_Garo      2       2 10000    NA    NA    NA
    ## 24 FR_Loir      2       2 10000    NA    NA    NA
    ## 5  FR_Arto      1       3    NA    NA    NA 10000
    ## 9  FR_Sein      1       3    NA    NA    NA 10000
    ## 10 GB_NorW      1       3    NA    NA    NA 10000
    ## 11 GB_Seve      1       3    NA    NA    NA 10000
    ## 12 GB_SouW      1       3    NA    NA    NA 10000
    ## 13 GB_Wale      1       3    NA    NA    NA 10000
    ## 21 FR_Arto      2       3    NA    NA    NA 10000
    ## 25 FR_Sein      2       3    NA    NA    NA 10000
    ## 26 GB_SouW      2       3    NA    NA    NA 10000

The spatial pattern is obvious in the results. Interestingly, we saw an
EMU that change cluster between period and this seem to correspond to
management measures that have effectively shorten the fishing season.
...
![](jags_landings_files/figure-markdown_strict/unnamed-chunk-16-1.png)

    ggplot(data = cou) +  geom_sf(fill= "antiquewhite") +
            geom_sf(data=emu,aes(fill=cluster2)) + scale_fill_manual(values=cols)+
      theme_bw() +xlim(-20,30) + ylim(35,65)  

![](jags_landings_files/figure-markdown_strict/unnamed-chunk-16-2.png)
...
Similarly to seasonality, we will build season. We reuse the procedure
made for silver eel and yellow eel seasonality, i.e. defining seasons
per emu, with the season starting at the month with minimum landings.
The month with lowest catch fmin define the beggining of the season
...
Now we should carry out data selection, more specifically, we want to
eliminate rows with two many missing data, too much zero and to check
whether there are no duplicates (though Cedric already did it). We mixed
coastal and marine habitats since there are only one EMU with landings
in marine habitat
...
The situation is not well balanced. Most EMU which have data in periods
1 don't have data in period 2 and conversely.
...
While 7 gives the best overall DIC, the DIC is rather flat and the
number of cluster used does not evolve much so we stop at 3.
...
![](jags_landings_files/figure-markdown_strict/unnamed-chunk-28-1.png)
Clusters 1 peaks summer. Clusters 2 peaks in winter, cluster 3 lasts
...
We can also look at the belonging of the different groups.
    ...
    ## 9    ES_Murc_C      2       1    NA 10000    NA
    ## 2    DE_Schl_C      1       2   213    NA  9787
    ## 3  DK_total_MO      1       2    NA    NA 10000
    ## 4    SE_East_C      1       2    NA    NA 10000
    ## 5    SE_West_C      1       2    NA    NA 10000
    ## 7    DE_Schl_C      2       2    NA    NA 10000
    ## 8  DK_total_MO      2       2    NA    NA 10000
    ## 10   GB_Angl_C      2       2    NA    NA 10000
    ## 11   GB_SouE_C      2       2    NA    NA 10000
    ## 12   GB_SouW_C      2       2    NA    NA 10000
    ## 13   SE_East_C      2       2    NA    NA 10000
    ## 1    DE_Eide_C      1       3  9801    NA   199
    ## 6    DE_Eide_C      2       3 10000    NA    NA

In fact, nearly all EMUs fall in cluster 3. Cluster 2 corresponds only
to ES\_Murc and cluster 1 to DE\_Eide.
...
![](jags_landings_files/figure-markdown_strict/unnamed-chunk-31-1.png)

    ggplot(data = cou) +  geom_sf(fill= "antiquewhite") +
            geom_sf(data=emu,aes(fill=cluster2)) + scale_fill_manual(values=cols)+
      theme_bw() +xlim(-20,30) + ylim(35,65)  

![](jags_landings_files/figure-markdown_strict/unnamed-chunk-31-2.png)
...
4 appears to be a good solution: good silhouette and we have only 4
groups.
...
![](jags_landings_files/figure-markdown_strict/unnamed-chunk-41-1.png)
There is much more diversity than in coastal waters. Some clusters peak
in srping (3), summer (2), autumn (1) and one has two peaks (4).
...
We can also look at the belonging of the different groups.
    ...
    ##           ser period cluster clus1 clus2 clus3 clus4
    ## 3   FR_Adou_T      1       1    NA    NA 10000    NA
    ## 7   FR_Sein_T      1       1    NA    NA 10000    NA
    ## 11  FR_Adou_T      2       1    NA    NA 10000    NA
    ## 12  FR_Bret_T      2       1    NA    NA 10000    NA
    ## 14  FR_Garo_T      2       1    NA    NA 10000    NA
    ## 4   FR_Bret_T      1       2    NA  9635   206   159
    ## 16  FR_Sein_T      2       2    NA 10000    NA    NA
    ## 1   DE_Eide_T      1       3    NA     1    NA  9999
    ## 2   DE_Elbe_T      1       3    NA    18    NA  9982
    ## 5   FR_Garo_T      1       3    78    NA    11  9911
    ## 6   FR_Loir_T      1       3    71    NA  1266  8663
    ## 8  NO_total_T      1       3    NA    18    NA  9982
    ## 9   DE_Eide_T      2       3    NA    NA    NA 10000
    ## 10  DE_Elbe_T      2       3    NA    NA    NA 10000
    ## 15  FR_Loir_T      2       3    NA    NA    NA 10000
    ## 13  FR_Cors_T      2       4 10000    NA    NA    NA

Cluster 4 stands only for Corsica. Some French EMUs have changed
clusters after 2010 towards cluster 1 which has a small duration.
...
![](jags_landings_files/figure-markdown_strict/unnamed-chunk-44-1.png)

    ggplot(data = cou) +  geom_sf(fill= "antiquewhite") +
            geom_sf(data=emu,aes(fill=cluster2)) + scale_fill_manual(values=cols)+
      theme_bw() +xlim(-20,30) + ylim(35,65)  

![](jags_landings_files/figure-markdown_strict/unnamed-chunk-44-2.png)
...
Silhouette and DIC does not move much after 4, but only 3 clusters are
used, therefore we keep 3.
...
![](jags_landings_files/figure-markdown_strict/unnamed-chunk-54-1.png)
Clusters 1 and 3 are bivariate, with 1 peaking in spring and autumn and
3 peaking in summer and autumn. Cluster 2 is widespread from may to
november.
...
We can also look at the belonging of the different groups.
    ...
    ## 1  DE_Eide_F      1       2    NA   206  9794
    ## 2  DE_Elbe_F      1       2    NA    33  9967
    ## 3  DE_Schl_F      1       2     1    14  9985
    ## 4  FR_Garo_F      1       2    NA    NA 10000
    ## 6  FR_Rhon_F      1       2    NA     3  9997
    ## 7  IE_West_F      1       2    NA   569  9431
    ## 8  DE_Eide_F      2       2    NA    68  9932
    ## 9  DE_Elbe_F      2       2    NA    NA 10000
    ## 10 DE_Schl_F      2       2    NA    NA 10000
    ## 11 DE_Warn_F      2       2    NA    NA 10000
    ## 12 GB_Angl_F      2       2    NA    NA 10000
    ## 15 GB_Tham_F      2       2    NA    NA 10000
    ## 13  GB_Dee_F      2       3    NA 10000    NA
    ## 14 GB_NorW_F      2       3    NA 10000    NA

In fact, nearly all EMUs fall in cluster 2. Cluster 1 only corresponds
to FR\_Loir and cluster 3 to two bristish EMUs. There is no obvious
spatial pattern nor period effect.
...
![](jags_landings_files/figure-markdown_strict/unnamed-chunk-57-1.png)

    ggplot(data = cou) +  geom_sf(fill= "antiquewhite") +
            geom_sf(data=emu,aes(fill=cluster2)) + scale_fill_manual(values=cols)+
      theme_bw() +xlim(-20,30) + ylim(35,65)  

![](jags_landings_files/figure-markdown_strict/unnamed-chunk-57-2.png)
...
The number of clusters used keep increasing, there is a good silhouette
and DIC at 6.
...
![](jags_landings_files/figure-markdown_strict/unnamed-chunk-67-1.png)
Cluster 1 peaks in winter, 2 in spring, 3 in spring/summer, 5 is
wisepread from april to november and 6 peaks in autumn (after a small
peak in spring).
...
We can also look at the belonging of the different groups.
    ...
    ## 2    DE_Eide_F      1       3    NA    NA     1    NA  9074   925
    ## 4    DE_Elbe_F      1       3    NA     2    NA    NA  9972    26
    ## 10   FR_Bret_T      1       3    NA    NA  4252    NA  5070   678
    ## 22   DE_Eide_F      2       3    NA     1     1    NA  7566  2432
    ## 32   FR_Bret_T      2       3    NA    NA    NA    NA 10000    NA
    ## 34   FR_Garo_T      2       3    NA    NA    NA    NA 10000    NA
    ## 42   GB_SouW_C      2       3    NA    NA    NA    NA  8157  1843
    ## 36   FR_Sein_T      2       4    NA    NA 10000    NA    NA    NA
    ## 39    GB_Dee_F      2       4    NA    NA 10000    NA    NA    NA
    ## 41   GB_SouE_C      2       4    NA    NA 10000    NA    NA    NA
    ## 1    DE_Eide_C      1       5    NA    NA    NA    NA    NA 10000
    ## 3    DE_Eide_T      1       5    NA    NA    NA    NA    27  9973
    ## 5    DE_Elbe_T      1       5    NA    NA   132    NA    NA  9868
    ## 6    DE_Schl_C      1       5    NA    NA     7    NA   197  9796
    ## 7    DE_Schl_F      1       5    NA    NA     1    NA  1984  8015
    ## 8  DK_total_MO      1       5    NA    NA    NA    NA    NA 10000
    ## 11   FR_Garo_F      1       5    NA    NA    NA    NA    NA 10000
    ## 12   FR_Garo_T      1       5    NA    NA    NA   104   106  9790
    ## 13   FR_Loir_F      1       5    NA    NA    NA     5    66  9929
    ## 14   FR_Loir_T      1       5    NA     2    NA   103   786  9109
    ## 15   FR_Rhon_F      1       5    NA    NA    NA    NA   173  9827
    ## 17   IE_West_F      1       5    NA    NA    26    NA   564  9410
    ## 18  NO_total_T      1       5    NA    NA    10    NA    NA  9990
    ## 19   SE_East_C      1       5    NA    NA    NA    NA    NA 10000
    ## 20   SE_West_C      1       5    NA    NA    NA    NA    NA 10000
    ## 21   DE_Eide_C      2       5    NA    NA    NA    NA    NA 10000
    ## 23   DE_Eide_T      2       5    NA    NA    NA    NA    NA 10000
    ## 24   DE_Elbe_F      2       5    NA    NA    NA    NA   306  9694
    ## 25   DE_Elbe_T      2       5    NA    NA    NA    NA    NA 10000
    ## 26   DE_Schl_C      2       5    NA    NA    NA    NA    NA 10000
    ## 27   DE_Schl_F      2       5    NA    NA    NA    NA    NA 10000
    ## 28   DE_Warn_F      2       5    NA    NA    NA    NA    55  9945
    ## 29 DK_total_MO      2       5    NA    NA    NA    NA    NA 10000
    ## 35   FR_Loir_T      2       5    NA    NA    NA    NA    NA 10000
    ## 37   GB_Angl_C      2       5    NA    NA    16    NA    NA  9984
    ## 38   GB_Angl_F      2       5    NA    NA    NA    NA    NA 10000
    ## 40   GB_NorW_F      2       5    NA    NA    NA    NA    NA 10000
    ## 43   GB_Tham_F      2       5    NA    NA    NA    NA    NA 10000
    ## 44   SE_East_C      2       5    NA    NA    10    NA    26  9964
    ## 33   FR_Cors_T      2       6    NA    NA    NA 10000    NA    NA

Cluster 1 corresponds only to ES\_Murc and cluster 6 to FR\_Cors.
Cluster 2 corresponds to French EMUs in transitional waters. Clusters 3
-5 are diverse. 5 accounts for French and Deutsh EMUs (T and F) and 6 to
...
Similarly to seasonality, we will build season. We reuse the procedure
made for silver eel and silver eel seasonality, i.e. defining seasons
per emu, with the season starting at the month with minimum landings.
The month with lowest catch fmin define the beggining of the season
(month\_in\_season=1) and season y stands for the 12 months from fmin y
(e.g., if lowest migration is in december, season ranges from december
...
Looking at the data, it seems that there are few silver eel fisheries in
transitional and marine open waters, therefore, we will make an analysis
for freshwater and 1 for all other environments.
...
4 seem to be a good compromise (though only 3 clusters seem to be
effectively used)
...
![](jags_landings_files/figure-markdown_strict/unnamed-chunk-83-1.png)
Clusters 3 and 4 correspond to peak in october with 3 more widespread.
Cluster 1 corresponds to a peak in autumn/winter. Cluster 2 corresponds
to catches in winter.
...
We can also look at the belonging of the different groups.
    ...
    ## 13   ES_Murc_C      2       2    NA    NA 10000    NA
    ## 1    DE_Eide_C      1       3    NA    21    NA  9979
    ## 3    DE_Elbe_T      1       3    NA    23    NA  9977
    ## 4    DE_Schl_C      1       3    NA  1180    NA  8820
    ## 5  DK_total_MO      1       3    NA    NA    NA 10000
    ## 6    SE_East_C      1       3    NA    NA    NA 10000
    ## 7    SE_West_C      1       3    NA    NA    NA 10000
    ## 8    DE_Eide_C      2       3    NA    NA    NA 10000
    ## 10   DE_Elbe_T      2       3    NA    NA    NA 10000
    ## 11   DE_Schl_C      2       3    NA    NA    NA 10000
    ## 12 DK_total_MO      2       3    NA   916    NA  9084
    ## 18   SE_East_C      2       3    NA    NA    NA 10000
    ## 2    DE_Eide_T      1       4    NA  9985    NA    15
    ## 9    DE_Eide_T      2       4    NA 10000    NA    NA
    ## 15   GB_Angl_C      2       4    NA 10000    NA    NA
    ## 16   GB_SouE_C      2       4    NA 10000    NA    NA
    ## 17   GB_SouW_C      2       4    NA 10000    NA    NA

In fact, most EMUs fall in cluster 3. Cluster 2 corresponds only to
ES\_Murc\_C (same as for yellow eel) and one for FR\_Cors. Cluster 4
(limited fishing season) regroups GB and DE EMUs.
...
5 seem to be a good compromise: slight decrease in silhouette, but all
clusters are used and DIC is good.
...
![](jags_landings_files/figure-markdown_strict/unnamed-chunk-96-1.png)
Cluster 2 peaks in summer with a second peak in december, 5 in winter, 2
in summer. Clusters 1 and 3 are bivariate (spring and autumn).
...
We can also look at the belonging of the different groups.
    ...
    ## 5  SE_Inla_F      1       1     3    89    NA  9690   218
    ## 16 SE_Inla_F      2       2 10000    NA    NA    NA    NA
    ## 1  DE_Eide_F      1       3    NA    NA    NA    NA 10000
    ## 2  DE_Elbe_F      1       3    NA    NA    NA    NA 10000
    ## 3  DE_Schl_F      1       3    NA    NA    NA     6  9994
    ## 6  DE_Eide_F      2       3    NA    NA    NA    NA 10000
    ## 7  DE_Elbe_F      2       3    NA    NA    NA    NA 10000
    ## 8  DE_Schl_F      2       3    NA    NA    NA    NA 10000
    ## 9  DE_Warn_F      2       3    NA    NA    NA    NA 10000
    ## 15 GB_Tham_F      2       3    NA    NA    NA    NA 10000
    ## 11 GB_Angl_F      2       4    NA 10000    NA    NA    NA
    ## 12 GB_Humb_F      2       4    NA 10000    NA    NA    NA
    ## 13 GB_SouE_F      2       4    NA 10000    NA    NA    NA
    ## 14 GB_SouW_F      2       4    NA 10000    NA    NA    NA
    ## 4  FR_Loir_F      1       5    NA    NA 10000    NA    NA
    ## 10 FR_Loir_F      2       5    NA    NA 10000    NA    NA

Once again the spatial pattern is obvious. SE\_Inla changed from 1 to 2
suggesting a reduction in the fishing season.
...
![](jags_landings_files/figure-markdown_strict/unnamed-chunk-99-1.png)

    ggplot(data = cou) +  geom_sf(fill= "antiquewhite") +
            geom_sf(data=emu,aes(fill=cluster2)) + scale_fill_manual(values=cols)+
      theme_bw() +xlim(-20,30) + ylim(35,65)  

![](jags_landings_files/figure-markdown_strict/unnamed-chunk-99-2.png)

Siver/Yellow
============

Many EMUs were not able to provide landings data in which yellow and
silver eel were discriminated. In such situation, it was impossible to
decide a priori if such EMU should be analysed with either silver eel or
yellow eel stage. Therefore, we analysed such EMUs indepedently.
...
Similarly to seasonality, we will build season. We reuse the procedure
made for silver eel and YS eel seasonality, i.e. defining seasons per
emu, with the season starting at the month with minimum landings. The
...
Looking at the data, it seems that there are EMUS, therefore we will
analysed all habitats simultaneously.
...
![](jags_landings_files/figure-markdown_strict/unnamed-chunk-112-1.png)
...
Cluster 5 peaks autumn and winter, 6 is similar but shifter 1 month
later. Clusters 1 and 2 are widepread with a peak in spring/early summer
and a second one un autumn. Cluster 4 is located in autumn only and
cluster 3 in summer.
...
We can also look at the belonging of the different groups.
    ...
    ##            ser period cluster clus1 clus2 clus3 clus4 clus5 clus6
    ## 5    FR_Arto_T      1       1    NA    NA 10000    NA    NA    NA
    ## 6    FR_Bret_T      1       1    NA    NA 10000    NA    NA    NA
    ## 8    FR_Garo_T      1       1    NA    NA 10000    NA    NA    NA
    ## 9    FR_Loir_F      1       1    NA    NA 10000    NA    NA    NA
    ## 10   FR_Loir_T      1       1    NA    NA 10000    NA    NA    NA
    ## 25   FR_Rhon_T      2       1    NA    NA 10000    NA    NA    NA
    ## 1    DE_Warn_F      1       2    NA    NA    NA    NA    NA 10000
    ## 4    FR_Adou_T      1       2    NA    NA    NA    NA    NA 10000
    ## 7    FR_Garo_F      1       2    NA    NA     9  4669    NA  5322
    ## 11   FR_Rhin_F      1       2    NA    NA    NA     2    NA  9998
    ## 12   FR_Sein_F      1       2    NA    NA    NA   284    NA  9716
    ## 15 NL_total_TC      1       2    NA    NA    NA   501    NA  9499
    ## 17  PL_Oder_TC      1       2    NA    NA    NA    NA    NA 10000
    ## 18  PL_Vist_TC      1       2    NA    NA    NA    NA    NA 10000
    ## 29   PL_Oder_C      2       2    NA    NA    NA    NA    NA 10000
    ## 30   PL_Oder_T      2       2    NA    NA    NA    NA    NA 10000
    ## 31  PL_Oder_TC      2       2    NA    NA    NA    85    NA  9915
    ## 32   PL_Vist_C      2       2    NA    NA    NA     1    NA  9999
    ## 33   PL_Vist_T      2       2    NA    NA    NA    NA    NA 10000
    ## 34  PL_Vist_TC      2       2    NA    NA    NA   687    NA  9313
    ## 13   FR_Sein_T      1       3    NA    NA    NA 10000    NA    NA
    ## 14  NL_total_F      1       3    NA    NA    NA 10000    NA    NA
    ## 16  NO_total_T      1       3    NA    NA    NA 10000    NA    NA
    ## 19   SE_East_C      1       3    NA    NA    NA  9977    NA    23
    ## 20   SE_West_C      1       3    NA    NA    NA 10000    NA    NA
    ## 24  FI_total_T      2       3    NA    NA    NA 10000    NA    NA
    ## 26  NL_total_F      2       3    NA    NA    NA 10000    NA    NA
    ## 27 NL_total_TC      2       3    NA    NA     1  7579    NA  2420
    ## 21   DE_Schl_C      2       4    NA 10000    NA    NA    NA    NA
    ## 28  NO_total_T      2       4    NA 10000    NA    NA    NA    NA
    ## 2    ES_Cata_T      1       5 10000    NA    NA    NA    NA    NA
    ## 22   ES_Cata_T      2       5 10000    NA    NA    NA    NA    NA
    ## 3    ES_Murc_C      1       6    NA    NA    NA    NA 10000    NA
    ## 23   ES_Murc_C      2       6    NA    NA    NA    NA 10000    NA

Cluster 6 corresponds only to ES\_Murc and cluster 5 to ES\_Cata to
FR\_Cors. Cluster 1 corresponds to many French EMUs in transitional
waters and 2 and to 3 are diverse.
