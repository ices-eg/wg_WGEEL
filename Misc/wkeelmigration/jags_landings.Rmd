...
res$emu_nameshort[res$emu_nameshort=="ES_MINH"]="ES_Minh"
...
con = dbConnect(PostgreSQL(),dbname=cred$dbname,host = cred$host,port=cred$port,user=cred$user,password=cred$password)
#emu <- st_read(con,query="select * from carto.emu")
emu=st_read("../../data/shapefiles/tr_emu_emu.shp")
names(emu)[1]="name_short"
emu$name_short[emu$name_short=="ES_Mino"]="ES_MINH"
emu$name_short[emu$name_short=="ES_Minh"]="ES_Mino"
emu = st_make_valid(emu)
emu$emu_x <- st_coordinates(st_centroid(emu$geometry))[,1]
emu$emu_y <- st_coordinates(st_centroid(emu$geometry))[,2]
cou <- rnaturalearth::ne_countries(continent = c("Europe", "Africa"), scale =10, returnclass = "sf") |>
...
get_pattern_month <- function(res,type="cluster"){
  ...
  if (type=="cluster"){
    ...
    sub_mat <- as.data.frame(res_mat[,grep("alpha_group",colnames(res_mat))])
  }
  sub_mat <- sub_mat %>% 
    pivot_longer(cols=1:ncol(sub_mat),
                 names_to="param",
                 values_to="proportion")
  tmp <- lapply(gsub("alpha_group", "toto", as.character(sub_mat$param)),function(p) strsplit(p,"[[:punct:]]"))
...
get_pattern_month2 <- function(res,mydata){
...
```{r mapglasseel}
average_pattern <- glasseel_subset |>
    group_by(emu_nameshort, das_month) |>
  summarize(das_value = sum(das_value, na.rm = TRUE), .groups = "keep") |>
  ungroup() |>
  group_by(emu_nameshort) |>
  mutate(das_value = das_value/sum(das_value, na.rm = TRUE), .groups = "keep") |>
  ungroup() |>
  left_join(emu, by = c("emu_nameshort" = "name_short"))
  
  
  
source("create_graph.R")  
formatted_patter = format_pattern(average_pattern)

ggplot(cou) +
  geom_sf() +
  theme_bw() +
  geom_polygon(data = formatted_patter,
               aes(x = x,
                   y = y,
                   fill = as.factor(month),
                   col = as.factor(month),
                   group = interaction(emu_nameshort, as.factor(month))),
               alpha = .3,
               inherit.aes = FALSE)   +
  scale_color_manual("Month", values = palette) +
  scale_fill_manual("Month", values = palette) +
  coord_sf(xlim = range(formatted_patter$x),
           ylim = range(formatted_patter$y)) +
  xlab("") +
  ylab("")
```
...
glasseel_wide <- glasseel_wide %>%
  ungroup() |>
  mutate(across(starts_with("m"), ~ .x/total_catch_year))
```
...
myclassif <- get_pattern_month2(myfit_glasseel_landings)
...
```{r mapglassoutput}
pat_group <- get_pattern_month(myfit_glasseel_landings, "group")
...
pat_group <- pat_group |>
  ...
  summarize(proportion = median(proportion), .groups = "keep") |>
  ungroup() |>
  mutate(das_month = as.numeric(gsub("m","", month))) |>
  rename(das_value = proportion) |>
  mutate(group_name= group_name[as.integer(as.character(cluster))]) |>
  mutate(emu_nameshort = stringr::str_extract(group_name, "^[^\\.]+"),
         period = stringr::str_extract(group_name, "(?<=\\.)[^\\.]+$")) |>
  mutate(period = ifelse(period == 1, "2000-2009", "2010-2020")) |>
  left_join(emu, by = c("emu_nameshort" = "name_short"))

formatted_patter = format_pattern(pat_group, keep_period = TRUE)
...
g <- ggplot(cou) +
  ...
  ylab("") +
  facet_wrap(~period)
save(g, file = "glasseel_landings.rdata")
ggsave("glasseel_landings.png", g, height=12/2.54, width = 16/2.54, dpi = 300)
```
...
myclassif <- get_pattern_month2(myfit_yelloweel_coastal_landings)
col_toreorder=grep("clus[0-9]",names(myclassif))
names(myclassif)[col_toreorder]=paste("clus",
                                      match(paste("clus",1:nbclus,sep=""),
                                      paste("clus",clus_order,sep="")),
...
myclassif <- get_pattern_month2(myfit_yelloweel_transitional_landings)
col_toreorder=grep("clus[0-9]",names(myclassif))
names(myclassif)[col_toreorder]=paste("clus",
                                      match(paste("clus",1:nbclus,sep=""),
                                      paste("clus",clus_order,sep="")),
...
myclassif <- get_pattern_month2(myfit_yelloweel_freshwater_landings)
col_toreorder=grep("clus[0-9]",names(myclassif))
names(myclassif)[col_toreorder]=paste("clus",
                                      match(paste("clus",1:nbclus,sep=""),
                                      paste("clus",clus_order,sep="")),
...
myclassif <- get_pattern_month2(myfit_yelloweel_allhab_landings)
col_toreorder=grep("clus[0-9]",names(myclassif))
names(myclassif)[col_toreorder]=paste("clus",
                                      match(paste("clus",1:nbclus,sep=""),
                                      paste("clus",clus_order,sep="")),
...
```{r mapgsilvereel_coastal_subset}
average_pattern <- silvereel_coastal_subset |>
  mutate(emu_nameshort = gsub("_.$","", emu_nameshort)) |>
  mutate(emu_nameshort = gsub("_MO$","", emu_nameshort)) |>
  group_by(emu_nameshort, das_month) |>
  summarize(das_value = sum(das_value, na.rm = TRUE), .groups = "keep") |>
  ungroup() |>
  group_by(emu_nameshort) |>
...
average_pattern$emu_x[average_pattern$emu_nameshort == "DK_total"] = emu$emu_x[emu$name_short == "DK_Inla"]
average_pattern$emu_y[average_pattern$emu_nameshort == "DK_total"] = emu$emu_y[emu$name_short == "DK_Inla"]
...
myclassif <- get_pattern_month2(myfit_silvereel_coastal_landings)
col_toreorder=grep("clus[0-9]",names(myclassif))
names(myclassif)[col_toreorder]=paste("clus",
                                      match(paste("clus",1:nbclus,sep=""),
                                      paste("clus",clus_order,sep="")),
...
```{r mapsilvermarineoutput}
pat_group <- get_pattern_month(myfit_silvereel_coastal_landings, "group")
...
pat_group <- pat_group |>
  ...
  mutate(emu_nameshort = gsub("_.$","", emu_nameshort)) |>
  mutate(emu_nameshort = gsub("_MO$","", emu_nameshort)) |>
  mutate(emu_nameshort = ifelse(emu_nameshort == "DK_total", "DK_Inla", emu_nameshort)) |>
  mutate(period = ifelse(period == 1, "2000-2009", "2010-2020")) |>
  left_join(emu, by = c("emu_nameshort" = "name_short"))

formatted_patter = format_pattern(pat_group, 
                                  multiplier = 300000, 
                                  keep_period = TRUE)
...
g <- ggplot(cou) +
  ...
  ylab("") +
  facet_wrap(~period)
save(g, file = "silver_landings_marine_coastal_transitional_ggplot.rdata")
ggsave("silver_landings_marine_coastal_transitional.png", g, height=12/2.54, width = 16/2.54, dpi = 300)
```
...
```{r mapgsilvereel_freshwater_subset}
average_pattern <- silvereel_freshwater_subset |>
  mutate(emu_nameshort = gsub("_.$","", emu_nameshort)) |>
  mutate(emu_nameshort = gsub("_MO$","", emu_nameshort)) |>
  group_by(emu_nameshort, das_month) |>
  summarize(das_value = sum(das_value, na.rm = TRUE), .groups = "keep") |>
...
myclassif <- get_pattern_month2(myfit_silvereel_freshwater_landings)
col_toreorder=grep("clus[0-9]",names(myclassif))
names(myclassif)[col_toreorder]=paste("clus",
                                      match(paste("clus",1:nbclus,sep=""),
                                      paste("clus",clus_order,sep="")),
...
```{r mapfreshwatermarineoutput}
pat_group <- get_pattern_month(myfit_silvereel_freshwater_landings, "group")
...
g <- ggplot(cou) +
  ...
  ylab("") +
  facet_wrap(~period)
save(g, file = "silver_landings_freshwater_ggplot.rdata")
ggsave("silver_landings_freshwater.png", g, height=12/2.54, width = 16/2.54, dpi = 300)
```
...
myclassif <- get_pattern_month2(myfit_YSeel_allhab_landings)
col_toreorder=grep("clus[0-9]",names(myclassif))
names(myclassif)[col_toreorder]=paste("clus",
                                      match(paste("clus",1:nbclus,sep=""),
                                      paste("clus",clus_order,sep="")),
