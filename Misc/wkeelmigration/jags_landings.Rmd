...
#indeed, cluster 2 is not used so we removed it
pat <-get_pattern_month(myfit_glasseel_landings)
pat <- subset(pat, pat$cluster!=2)
...
```{r}
t(as.data.frame(characteristics(myfit_glasseel_landings, 6)))[-2,]
```
...
Clusters 1 and 3 starts in autum and last still january (3) or march (cluster 1). Cluster 4 is similar to cluster 6 but shited one month earlier Cluster 5 corresponds to fishing in winter.
...
```{r, warning=FALSE}
yelloweel_coatal <- subset(yelloweel, yelloweel$hty_code =="C")
...
While 7 gives the best overall DIC, the silhouette coefficient decreses dramatically after 4 clusters, therefore, we stop at 3. 
...
ggplot(pat,aes(x=month,y=proportion))+
  ...
  theme_bw()
```
Clusters 3 peaks summer. Clusters 2 and 3 are vey similar and concentrated in winter months.
...
print(myclassif[order(myclassif$cluster),])
```
In fact, nearly all EMUs fall in cluster 3. Cluster 2 corresponds only to ES_Murc.
...
With 5 clusters, the DIC is stable and all clusters are used.
...
ggplot(pat,aes(x=month,y=proportion))+
  ...
  theme_bw()
```
There is much more diversity than in coastal waters. Some clusters peak in sping (1), summer (3, 4), have two peaks (2, 5). 

We compute some statistics to characterize the clusters.
```{r}
t(as.data.frame(characteristics(myfit_yelloweel_transitional_landings, 5)))
```
Duration indicates the minimum number of months that covers 80% of the wave (1st column is the median, and the 2 next one quantiles 2.5% and 97.5% of credibility intervals). Centroid is the centroid of the migration wave (e.g. 11.5 would indicate a migration centred around mid november). The first column is the median and the two next one the quantiles 2.5 and 97.5%.
...
print(myclassif[order(myclassif$cluster),])
```
Some French EMUs changed from cluster 5 to 3 indicating a reduction of the fishing season after the EMP implementation. FR_Sein also changed cluster, however, only one year of data was available before 2010.
...
While 7 gives the best overall DIC, the silhouette coefficient decreases after 3 and 3 clusters are rarely used, therefore, we keep 3. 
...
ggplot(pat,aes(x=month,y=proportion))+
  ...
  theme_bw()
```
Clusters 2 and 3 ares quite similar with question 3 lasting a bit longer. Cluster 1 is not used.
...
print(myclassif[order(myclassif$cluster),])
```
In fact, nearly all EMUs fall in cluster 3. Cluster 2 corresponds only to GB_Dee_F.
