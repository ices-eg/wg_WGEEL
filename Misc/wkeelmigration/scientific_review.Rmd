...
f <- "Literature_table_050220_all.xlsx"
path <- str_c(datawd,f)	
file <- basename(path)
mylocalfilename <- gsub(".xlsx","",file)
sea <- read_excel(path=path,"Literature review" , range="A2:AN69",col_names = TRUE)
f <- "Literature Review_table_recruitment.xlsx"
path <- str_c(datawd,f)	
file <- basename(path)
mylocalfilename <- gsub(".xlsx","",file)
seag <- read_excel(path=path,"Literature review" , range="A2:AN48",col_names = TRUE)
seag <- seag[colnames(sea)%in%colnames(seag),]
colnames(seag)
colnames(sea)
sea<-rbind(sea,seag)
...
sea_q3G <- sea_q2 %>% 
		...
		group_by(author,site,month,stage2,emu,X,Y)%>%
				summarize(N=n(),freq=round(mean(freq,na.rm=TRUE))) %>% 
				print(n=Inf)
		
		sea_q3G$site[order(sea_q3G$Y+sea_q3G$X)]
x11()
ggplot(sea_q3,aes(x = month)) +
		geom_tile(aes(y=author,fill=freq))+
		xlab("month")+
		ylab("percentage catch")+
		facet_wrap(~  stage2)

sea_q3 %>% filter(stage2=="S")%>%
		ggplot(aes(x = month)) +
		geom_col(aes(y=freq),position="dodge")+
		xlab("month")+
		ylab("percentage catch")+
