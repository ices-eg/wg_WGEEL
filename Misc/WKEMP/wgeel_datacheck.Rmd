...
```{r landingsfranceita}
landings = dbGetQuery(con,"select sum(eel_value) eel_value,eel_cou_code,eel_lfs_code,eel_year,eel_emu_nameshort from datawg.t_eelstock_eel 
                      where eel_cou_code in ('IT','FR')
...
ggplot(landings,aes(x=eel_year,y=eel_value))+geom_line(aes(col=eel_emu_nameshort))+
  scale_y_log10()+
...
```{r plotBbestB0}
ggplot(indicator,aes(x=bbest/b0))+geom_histogram()+geom_vline(xintercept=1,slope=1,lty=2)+xlab("Bbest/B0") + scale_x_log10() + ylab("frequency")
```
...
```{r plotBcurrentBbest}
ggplot(indicator,aes(x=bcurrent/bbest))+geom_histogram()+geom_vline(xintercept=1,slope=1,lty=2)+xlab("Bcurrent/Bbest") + scale_x_log10() + ylab("frequency")
```
...
```{r plotBcurrentB0}
ggplot(indicator,aes(x=bcurrent/b0))+geom_histogram()+geom_vline(xintercept=1,slope=1,lty=2)+xlab("Bcurrent/B0") + scale_x_log10() + ylab("frequency")
```
...
ggplot(indicator_sub,aes(x=b0/b0_estimated))+geom_histogram()+facet_wrap(~eel_cou_code)+scale_x_log10()+geom_vline(xintercept=1,lty=2)+xlab(expression("reported B"[0]/"estimated B"[0]))
```
