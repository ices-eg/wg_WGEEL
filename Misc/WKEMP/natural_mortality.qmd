...
# Intro
In a pristine situation, and assuming an average mortality over the lifespan, escapment can be calculated as

$$
B_0 = R_0 \cdot e^{-M_0 \cdot \Delta t}
$${eq_b0}
with $\Delta t$ the lifespan and $M_0$ the average natural mortality in pristine situation. Since recruitment is depleted, natural mortality is thought to be reduced due to compensatory density dependence mechanisms. The question here is, in the absence of anthropogenic, what would be the reduction of natural moratlity required to achieve an escapement of 40% $B_0.
...
g = ggplot(comb, aes(x=RR0, y=values)) +
  ...
  scale_linetype_manual("area", values = c("Elsewhere.Europe"=3,
                                           "North.Sea"=4))
ggsave("mortality_1.png", width=16/2.54,height=16/2.54)
