...
g2 <- ggplot(indicator_vis |> 
  filter(istable1 & !eel_emu_nameshort %in% c("DK_total","EE_total", "FI_total", "FR_total")) |>
...
g2 <- ggplot(indicator_vis |> 
  filter(!istable1 & !eel_emu_nameshort %in% c("IE_total","GB_total","LT_total", "LV_total")) |>
  select(eel_emu_nameshort, eel_year, bcurrent) |> 
  mutate(bcurrent = ifelse(is.na(bcurrent), "absent", "present")), 
  aes(eel_emu_nameshort, eel_year, fill = bcurrent))+
  geom_tile() +
...
g3 <- ggplot(indicator_vis |>
  filter(istable1 & !eel_emu_nameshort %in% c("EE_total", "FI_total", "FR_total", "DK_total")) |>
...
For some EMUs like SE_Inla, this is likely related to using a B\_{0} that is coming from the late 80s situation. For EE, it seems rather due to the inclusion of restocking in $B_{best}$ (i.e. overestimation of $B_{best}$). For others, $B_0$ is likely underestimated.
...
<br> In the absence of density dependence $B_{0_{adj}}$/$B_0$ is expected to be close to 1. In the presence of density dependent mortality, given the current recruitment decline, $B_{0_{adj}}$/$B_0$ might be higher than 1.
