...
## Logistic regression
### Do changes in SigA effect Bcurr?

```{r delta_Bcurr_vs_sigA}

require(lme4)

delta <- Kendall_mort_tab %>%
                select(eel_emu_nameshort, Period, suma_initial, suma_current, Delta_suma) %>%
                left_join(Kendall_bio_tab %>% select(eel_emu_nameshort, Tau_b0_adj, p_b0_adj, Period), by = "eel_emu_nameshort") %>%
                mutate(binary_b0_adj = ifelse(Tau_b0_adj > 0 & p_b0_adj < 0.1, 1, 0),
                       test_period = ifelse(Period.x != Period.y, "WARNING", "same period"),
                       Tau_b0_adj = NULL,
                       p_b0_adj = NULL) %>%
                rename(Period = Period.x) %>%
                mutate(Period.y = NULL) %>%
                drop_na()
logit_delta <- glm(binary_b0_adj ~ Delta_suma, family=binomial(link="logit"), data = delta)

test <- as.data.frame(summary(logit_delta)$coefficients) %>%
        mutate(Coefficient = ifelse(row_number() == 1, "Intercept", "Delta_sumA")) %>%
        relocate(Coefficient, .before = Estimate)
              
new_delta <- expand.grid(Delta_suma = seq(-2, 2, by =0.1))
                         

ggplot(delta, aes(x=Delta_suma, y=binary_b0_adj)) +
  geom_point(size=0.9)+
  theme_bw()+
  geom_line(data= new_delta, aes(x=Delta_suma, y=predict(logit_delta, type = "response", newdata = new_delta)), size = 0.3)+
  labs(x = "Delta_sumA", y = "Effect on Bcurrent (1 = present, 0 = absent)")

flextable(test) %>% autofit()
...
Logistic regression of change in Bcurren/Bo_adjusted over Delta sumA (Time period see Kendall table). Effect of Delta sumA is not significant (see table) 

### Effect of measures on Stock indicators
...
#list_measures_n <- unique(measures_n$measure_type)
...
ggplot(measures_n, aes(x=n, y=b0_adj)) +
  ...
  labs(x = "number of measures", y = "Effect on b0_adj (1 = present, 0 = absent)")+facet_wrap(~measure_type)
...
#ggplot(measures_scores, aes(x=score, y=b0_adj)) +
  #geom_point(aes(col=measure_type), size=0.9)+
  #theme_bw()+
  #geom_line(data = new_measures_scores, aes(x=score, y=predict(logit_measures_scores_b0_adj, type = "response", newdata = new_measures_scores), col = measure_type), size = 0.3)+
  #labs(x = "number of measures", y = "Effect on b0_adj (1 = present, 0 = absent)")+facet_wrap(~measure_type)

#count_measures_scores <- measures_scores %>% 
                    #group_by(measure_type) %>%
                    #summarise(no_of_EMUs = n(), .groups = "keep")
