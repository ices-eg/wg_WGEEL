...
flextable(mortality %>% 
            ...
            pivot_wider(names_from=eel_typ_id, values_from=c(range, nb_value),
                        names_glue="{eel_typ_id}_{.value}",
                        names_sort=TRUE)) %>% autofit() %>% set_table_properties(layout = "autofit")
...
g = ggplot(st_transform(country_p,crs=3035)) + geom_sf(fill=NA)+theme_bw()+
...
ggsave(g, filename="data_avail.png",width=16/2.54,height=10/2.54)
knitr::include_graphics("data_avail.png")
```
...
flextable(indicator %>%
  ...
  pivot_wider(names_from=habitat,values_from=c(freq0,freq100))) %>% autofit() %>% set_table_properties(layout = "autofit")
```
...
if (!dir.exists("trendbiomassplots"))
dir.create("trendbiomassplots")
...
ggsave(g,filename="trend_biomass.png", width=16/2.54, height=16/2.54,dpi=300)
knitr::include_graphics("trend_biomass.png")

for (emu in unique(indicator$eel_emu_nameshort)){
  g = ggplot(indicator %>% filter(!is.na(bcurrent/b0)& 
    ...
    theme_bw() + ylab(expression(B[current]/B[0]))+xlab("") +
    xlim(2007,2020) + ggtitle(emu)
  
  ggsave(g,filename=paste0("trendbiomassplots/trend_biomass_",emu,".png"), width=16/2.54, height=16/2.54,dpi=300)
...
theme_bw() + ylab(expression(B[current_without_stocking]/B[0]))+xlab("") + xlim(2007,2020)+facet_wrap(~eel_cou_code,drop=TRUE,scales="free_y")
  
ggsave(g,filename="trend_biomass_without_stocking.png", width=16/2.54, height=16/2.54,dpi=300)
...
ggsave(suma,file = "trend_suma.png", width=16/2.54, height = 16/2.54,dpi=300)
knitr::include_graphics("trend_suma.png")
...
ggsave(sumf,file = "trend_sumf.png", width=16/2.54, height = 16/2.54,dpi=300)
knitr::include_graphics("trend_sumf.png")
...
ggsave(sumh,file="trend_sumh.png", width=16/2.54, height = 16/2.54,dpi=300)
knitr::include_graphics("trend_sumh.png")
...
#  scale_y_log10()+
  ...
  guides(lty=FALSE,col=FALSE)+ #geom_hline(yintercept=1)+
#  scale_y_sqrt()+
theme_bw() + ylab(expression(B[0-adj]~"in 1000t"))+xlab("") + xlim(2007,2020)+facet_wrap(~eel_cou_code,drop=TRUE,scales="free_y")
ggsave(b0trend, file="b0_trend.png",height=16/2.54,width=16/2.54)
knitr::include_graphics("b0_trend.png")
...
ggsave(b0ratio, file="b0_ratio.png",height=16/2.54,width=16/2.54)
knitr::include_graphics("b0_ratio.png")
```
...
refpoints=data.frame(indicator=c("ratio","ratio_without_stocking",
                                  "suma"),
...
for (emu in unique(sub$EMU)){
  ...
  g = ggplot(subemu,aes(x=eel_year,y=indval))+
    geom_line()+
    geom_point()+
    facet_wrap(~indicator, labeller=vlabeller,
               scales="free")+
    xlab("")+ylab("") +
    geom_hline(data=refpoints,aes(yintercept=indval),col=2,lty=3)
  ggsave(paste0("trendsfig/trends_indicator_",
                emu,
                ".png"),
...
invisible(capture.output(Kendall_bio_b0_without_stocking <- rownames_to_column(as.data.frame(t(as.data.frame(lapply(indicator %>%
                  ...
                  arrange(eel_year) %>%
                  select(-eel_year), 
                  function(y) unlist(MannKendall(y)) %>% setNames(paste0(names(.),"_without_stocking")))))), "eel_emu_nameshort")))
...
Kendall_b0_tab <-  indicator %>%
  ...
  full_join(indicator  %>% group_by(eel_emu_nameshort) %>%
              ...
              mutate(Period_b0_without_stocking = paste(min(eel_year), max(eel_year), sep = "-"),
...
flextable(Kendall_table %>%
            rename(EMU = eel_emu_nameshort) %>%
            mutate(Tau_bcurr_without_stocking = paste(ifelse(is.na(Tau_bcurr_without_stocking),"",Tau_bcurr_without_stocking), 
  ...
  set_header_labels(Period_b0_without_stocking = "Period",
...
ADG_non_EU <- all_emus %>%
  ...
  mutate(suma_trend = ifelse(p_suma > 0.1, 0,
...
flextable(ADG_summary %>% rownames_to_column("indicator")) %>% autofit() %>% set_table_properties(layout = "autofit")
...
flextable(Fisher_sumA) %>% autofit() %>% set_table_properties(layout = "autofit") %>% set_caption(caption = paste("sumA, p =", round((fisher.test(Fisher_sumA[,c(2,3)])$p.value), 3)))
flextable(Fisher_sumF) %>% autofit() %>% set_table_properties(layout = "autofit") %>% set_caption(caption = paste("sumF, p =", round((fisher.test(Fisher_sumF[,c(2,3)])$p.value), 3)))
flextable(Fisher_sumH) %>% autofit() %>% set_table_properties(layout = "autofit") %>% set_caption(caption = paste("sumH, p =", round((fisher.test(Fisher_sumH[,c(2,3)])$p.value), 3)))
flextable(Fisher_b0_adj) %>% autofit() %>% set_table_properties(layout = "autofit") %>% set_caption(caption = paste("b0 adjusted, p =", round((fisher.test(Fisher_b0_adj[,c(2,3)])$p.value), 3)))
...
flextable(logit_delta_table) %>% autofit() %>% set_table_properties(layout = "autofit")
...
flextable(summary_n_sumA %>% relocate(coefficients, .before = Estimate)) %>% autofit() %>% set_table_properties(layout = "autofit")
```
\
\
\
...
flextable(summary_n_sumF %>% relocate(coefficients, .before = Estimate)) %>% autofit() %>% set_table_properties(layout = "autofit")
```
\
\
\
...
flextable(summary_n_sumH %>% relocate(coefficients, .before = Estimate)) %>% autofit() %>% set_table_properties(layout = "autofit")
```
\
\
\
...
flextable(summary_n_b0_adj %>% relocate(coefficients, .before = Estimate)) %>% autofit() %>% set_table_properties(layout = "autofit")
```
...
flextable(summary_scores_sumA %>% relocate(coefficients, .before = Estimate)) %>% autofit() %>% set_table_properties(layout = "autofit")
```
\
\
\
...
flextable(summary_scores_sumF %>% relocate(coefficients, .before = Estimate)) %>% autofit() %>% set_table_properties(layout = "autofit")
```
\
\
\
...
flextable(summary_scores_sumH %>% relocate(coefficients, .before = Estimate)) %>% autofit() %>% set_table_properties(layout = "autofit")
```
\
\
\
...
flextable(summary_scores_b0_adj %>% relocate(coefficients, .before = Estimate)) %>% autofit() %>% set_table_properties(layout = "autofit")
```
