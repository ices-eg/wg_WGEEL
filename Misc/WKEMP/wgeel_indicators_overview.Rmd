...
```{r "Fisher's exact test to compare measures"}
...
#measures_present <- measures[measures$binary_measure != 0,]

test <- measures %>%
  group_by(measure_type, binary_measure) %>%
  summarise(effect = n())
  
            

#spread(measure_type, binary_measure)
            



test <- Kendall_mort_tab %>% select(eel_emu_nameshort, p_suma, p_sumf, p_sumh)
```
