...
library(officer)
thick <- officer::fp_border(color="black", width = 2)

Progress_visualisation <- Kendall_table %>%
              mutate(esc_tar = ifelse(Esc_perc_b0_current < 0.4, 0, 1),
                     esc_trend = ifelse(p_b0>.1,0,
                                 ifelse(Tau_b0<0, 1, 0)),
                     mort_tar = ifelse(suma_current > 0.92, 0, 1),
                     mort_trend = ifelse(p_suma>.1,0,
                                  ifelse(Tau_suma<0, 0, 1)),
                     d_suma = ifelse(Delta_suma < 0, 1, 0),
                     d_bcurr = ifelse(Delta_b0 > 0, 1, 0),
                     esc_tar_initial = ifelse(Esc_perc_b0_current < 0.4, 0, 1),
                     mort_tar_initial = ifelse(suma_initial > 0.92, 0, 1)) %>%
  select(eel_emu_nameshort, esc_tar, d_bcurr, esc_trend, mort_tar, d_suma, mort_trend, esc_tar_initial, mort_tar_initial) %>%
  replace(is.na(.), 2)
                     
columns <- c("esc_tar", "esc_tar_initial", "mort_tar", "mort_tar_initial", "d_suma", "d_bcurr", "esc_trend" , "mort_trend")

col_format <- scales::col_bin(
    palette = c("coral1", "palegreen3", "grey76"),
    domain = c(0, 1, 2)) # can ad bins by , bins = c(1,2,3)

flextable(Progress_visualisation) %>% set_header_labels(eel_emu_nameshort = "EMU",
                    esc_tar = "Escapement target",
                    esc_tar_initial = "Escapement target (initial)",
                    mort_tar = "Mortality target",
                    mort_tar_initial = "Mortality target (initial)",
                    d_suma = "\u03A3A lower",
                    d_bcurr = "Bcurr lowered in most recent report",
                    esc_trend = "constant increase in escapement",
                    mort_trend = "constant decrease in mortality") %>%
            bg(j = columns, bg = col_format) %>%
             vline() %>%
             hline() %>%
             vline(j = 7, border = thick) %>% 
             void(j = columns) %>%
             autofit()
