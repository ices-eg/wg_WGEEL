...
Figure `r run_reference("fig-natmort")` indicates that for lifespans shorter than 15 years, the reduction of natural mortality should be larger than 0.138 year^-1^, a value often used for the eel [@dekker2000]. The situation is of course even worse if considering the North Sea index rather than the Elsewhere Europe index. Even at very long lifespan (30 years), the natural mortality should be reduced by 0.05 year^-1^ (i.e. a reduction of M of about 36% compared to the standard 0.138 year^-1^ value) when assuming that current recruitment is 8% of pristine recruitment (\~ what Elsewhere Europe index indicates). This reduction is the average value over the whole lifespan of the individual, while density-dependent natural mortality is mainly restricted to younger ages.
...
```
::: {custom-style="Image Caption"}
`r add_figure_caption("fig-trendbiomass")` Trends in reported Bcurrent/B0. Each coloured line represents a specific EMU. The horizontal red line indicates the target according to the Eel Regulation.
:::
...
```
::: {custom-style="Image Caption"}
`r add_figure_caption("fig-trendbiomasswithoutstocking")` Trends in reported $B_{current\_without\_stocking}/B_0$. Each coloured line represents a specific EMU. The horizontal red line indicates the target according to the Eel Regulation.
:::
### Mortality indicators

#### Fishing mortality

Figure `r run_reference("fig-trendmortasumf")` and Table `r run_reference("tbl-trendsumf")` show the trends in reported fishing mortality.
...
Figure `r run_reference("fig-trendmortasumh")` and Table `r run_reference("tbl-trendsumh")` show the trends in reported other anthropogenic mortalities. In many EMUs, $\sum H$ is very small or equal to zero suggesting that only a part of the mortalities is accounted for. 
...
```
::: {custom-style="Image Caption"}
`r add_figure_caption("fig-trendmortasumh")` Trends in reported ΣH. Each coloured line represents a specific EMU.
:::
...
::: {custom-style="Image Caption"}
`r add_table_caption("tbl-trendsumh")` Results of Mann-Kendall test for monotonic trends per EMU for sumH ($year^{-1}$). 'n' represents the number of years for which estimates are available, while Δ is the difference between the first and the last reported values. Tau represents the direction of the trend (positive = increase, negative = decrease) and p-value indicates the significance of the trend. Trend indicates the detection of a significant increasing (↗) or decreasing trend (↘). Some countries (e.g. NL and FR) report the same value for several yearsand this might affect the results of the trend test.
:::
...
Figure `r run_reference("fig-trendmortasuma")` and Table `r run_reference('tbl-trendsuma')` show the resulting trends in reported total anthropogenic mortalities.
...
::: {custom-style="Image Caption"}
`r add_figure_caption("fig-trendmortasuma")` Trends in reported ΣA. Each coloured line represents a specific EMU.
:::
...
::: {custom-style="Image Caption"}
`r add_table_caption("tbl-precoemu")` Position of EMUs in the precautionary diagram. The colours stand for the zones in the diagram. A "x" indicates that $B_{current\_without\_stocking}$ was used to replace missing $B_{current}$.
:::
