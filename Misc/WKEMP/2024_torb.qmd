...
biomasswstrendtab <- trendtable(ind = "bcurrent_without_stocking")
```
...
#| warning: false
#| error: false
#| fig-cap: "Reduction of average natural mortality over the lifespan that would be required to achieve the management target in an  EMU given different levels of recruitment (x-axis) and lifespan (facet). The two vertical lines indicate the recruitment estimates from the WGEEL glass eel indices lagged according to the lifespan (i.e. lines for lifespan 5 corresponds to the recruitment estimates in 2024-5=1999). The horizontal line corresponds to a natural mortality of 0.138 year^-1^"
#| fig.height: 6.3
#| fig.width: 6.3
...
#| fig-height: 6.3
#| fig-width: 6.3
#| fig-cap: "Trends in reported Bcurrent/B0. Each coloured line represents a specific EMU. The horizontal red line indicates the target according to the Eel Regulation."
#| 
...
::: {custom-style="Image Caption"}
Table `r run_bookmark("tbl-trendbiomass", runs_table)`: Results of Mann-Kendall tests for monotonic trends per EMU for $B_{current}$ (kg). 'n' represents the number of years for which estimates are available, while Δ is the difference between the first and the last reported values. Tau represents the direction of the trend (positive = increase, negative = decrease) and p-values indicate the significance of the trend. Arrows indicates the detection of a significant increasing (↗) or decreasing trend (↘). Some countries (e.g. NL and FR) have been reporting the same value for several years and this might affect the results of the trend test. For France, $B_{current\_without\_stocking}$ since $B_{current}$ was not reported.
:::
...
biomasstrendtab <- biomasstrendtab |>
  bind_rows(biomasswstrendtab |> filter(!EMU %in% biomasstrendtab$EMU) |>
              mutate("without stocking" = "x")) |>
  arrange(EMU)
...
#| fig-height: 6.3
#| fig-width: 6.3
#| fig-cap: "Trends in reported $B_{current\_without\_stocking}/B_0$. Each coloured line represents a specific EMU. The horizontal red line indicates the target according to the Eel Regulation."
indicator <- indicator %>%
  mutate(eel_cou_code = ifelse(eel_cou_code == substr(eel_emu_nameshort, 1, 2),
                               eel_cou_code,
                               "INT"
...
g <- ggplot(indicator %>% filter(!is.na(bcurrent_without_stocking / b0)), aes(x = eel_year, y = bcurrent_without_stocking / b0)) +
  geom_hline(yintercept = .4, col = "red") +
  geom_line(aes(col = eel_cou_code, group = eel_emu_nameshort)) +
  #  scale_y_log10()+
  scale_color_manual("country", values = color_countries[names(color_countries) %in% unique(indicator$eel_cou_code)], ) +
...
#| fig-height: 6.3
#| fig-width: 6.3
#| fig-cap: "Trends in reported ΣF. Each coloured line represents a specific EMU."
...
#| fig-height: 6.3
#| fig-width: 6.3
#| fig-cap: "Trends in reported ΣH. Each coloured line represents a specific EMU."
...
`r sum(sumhtab[,8]<=0.05 & sumhtab[,7]<0, na.rm = TRUE)` EMUs displaying a significant decrease of $\sum H$, another `r sum(sumhtab[,8]<0.05 & sumhtab[,7]>0, na.rm = TRUE)` display a significant increase, but the vast majority show no trend. The Workshop was asked to provide further insights on trends per mortality types (e.g. hydropower, contamination). However, given that (i) the number of significant trends for the combined "others anthropogenic mortalities" indicators is very small (Table `r run_reference("tbl-trendsumh")`) and (ii) that it is unclear which mortalities were accounted for in the different EMUs (and many of them were seemingly not accounted for at all), the experts were not in position to answer precisely the request but consider that reported trends in $sumH$ did not indicate a likely change in any other types of mortalities.
...
#| fig-height: 6.3
#| fig-width: 6.3
#| fig-cap: "Trends in reported ΣA. Each coloured line represents a specific EMU."
...
<b>
Recommendation:
-	Implementing a mandatory transnational traceability system to track eels from capture to their final destination to ensure the compliance with environmental regulations and international treaties and a proper stock assessment. In the case of restocked eels, this means that a separate monitoring of the material type (R or C) is carried out throughout the trade chain, until they are released into the water. This monitoring would avoid that glass eels are diverted to other, potentially illegal, uses.
-	The traceability system should also separate glass eels intended for direct consumption from those intended for growing in aquaculture facilities. 

```{r }
#| label: fig-check_proportion
#| fig-width: 6.3
#| fig-height: 6.3
#| fig-cap: "Proportion of glass eel marked as restocking material reported by France, `trade only` identifies the proportion of glass eel that was used in trade, `including France restocking` incudes the amount of glass eel released in France as part of the restocking program."
...
g <- ggplot(fr_reserve) +
  ...
  ylim(c(0, 1)) +
  ylab("Proportion restocked in France") +
  geom_hline(yintercept=0.6, lty=2) +
  annotate("text", x = 1, y = 0.63, label = "60%") +
  theme_minimal() +
  theme(legend.position = "bottom")
fileres <- file.path(params$image_path,"proportion_restocked.png")
ggsave(file = fileres, height = 16/2.54, width = 16/2.54)
knitr::include_graphics(fileres)
```
...
::: {custom-style="Image Caption"}
Table A1: Summary of stock indicators reported to ICES by Member States and other eel-range states (UK and Norway) in 2024, and the change of stock indicators between the first reporting after 2007 and the most recent reported year (time period may differ between countries and stock indicators). Colour indicates whether stock indicators comply with the management target for biomass (Bcurrent/B0 > 0.4) or the suggested threshold for the sum of anthropogenic mortalities (ΣA < 0.92), or whether it has improved with respect to these targets since implementation (Green = compliance/improved; red = no compliance/not improved; light grey = threshold for mortality is uncertain since escapement biomass is below 40% of pristine escapement; dark grey = data not available). For France, $B_{current\_without\_stocking}$ since $B_{current}$ was not reported.
:::
...
precodata_all2 <- precodata_all |>
  mutate(b_b0 = ifelse(!startsWith(eel_emu_nameshort, "FR"),
                       bcurrent,
                       bcurrent_without_stocking)/b0)


biomasstrendtab <- trendtable(precodata_all2, "b_b0")




tab <- biomasstrendtab |>
  select(all_of(c("EMU", "Final","Initial", "Δ"))) |>
  rename(c("Bf" = "Final","BD" ="Δ", "Bi" = "Initial")) |>
  full_join(sumatab |>
  select(all_of(c("EMU", "Final","Initial", "Δ"))) |>
  rename(c("Af" = "Final","AD" ="Δ", "Ai" = "Initial"))) |>
  filter(!endsWith(EMU, "total")) |>
  mutate(Country = substring(EMU, 1, 2)) |>
  arrange(Country, EMU)
  
flex <- flextable(tab |>
                    mutate(across(!all_of(c("EMU", "Country")), ~ round(.x, digits = 3))) |>
                    select(all_of(c("Country","EMU", "Bf","Af","BD","AD","Bi","Ai")))) |>
  flextable2ICES() 



for (col in c("Bi", "Bf", "Ai","Af")){
  thres <- ifelse (startsWith(col, "B"), 0.4, 0.92)
  sign <- ifelse(startsWith(col, "B"), ">=", "<=")
  isign <- ifelse(startsWith(col, "B"), "<", ">")
  flex <- flex |>
    bg(i=as.formula(paste0("~`",col,"`",isign,thres)), j = col, bg="#FF7256") |>
    bg(i=as.formula(paste0("~`",col,"`",sign,thres)), j = col, bg="#7CCD7C")
}
...
for (col in c("Bi", "Bf", "BD","Ai", "Af", "AD")){
  flex <- flex |>
    bg(i=as.formula(paste0("~is.na(`", col, "`)")), j = col, bg ="#808080")
}
...
flex <- flex |>
  bg(i=~`Af`<0.92 & `Bf` <0.4, j = "Af", bg ="#D9D9D9") 
...
flex |> 
  ...
  "BD" = "BD",
  "Bi" = "Bi",
  "Af" = "ΣA (last reported year)",
  "AD" = "Change in ΣA since implementation", 
  "Ai"= "ΣA (first reported year)")) |> 
  flextable:::compose(part = "header", i = 1, j = "Bf",
         value = as_paragraph("B", as_sub("current"),"/B", as_sub("0")," (last reported year)")) |>
  flextable:::compose(part = "header", i = 1, j = "BD",
         value = as_paragraph("change in B", as_sub("current"),"/B", as_sub("0")," since implementation")) |> 
  flextable:::compose(part = "header", i = 1, j = "Bi",
         value = as_paragraph("B", as_sub("current"),"/B", as_sub("0")," (first reported year)")) |>
  merge_v("Country")
