...
plot3 <- ggplot(ImpactCountTable_combined, aes(x = measure_type, y = percentage, fill = impact)) +
  ...
  geom_text(aes(x = measure_type, y = 100, label = paste0("n = ", total_count)), 
            vjust = -0.2, size = 2, inherit.aes = FALSE, angle = 45, hjust = -0.2) +
...
measures_all_cleaned_EMP$impact <- gsub("low", "Low", measures_all_cleaned_EMP$impact)
...
ggplot(ImpactByYear, aes(x = factor(year_of_full_implementation), y = count, fill = factor(impact))) +
  ...
  scale_fill_viridis_d()
```
Table with no. of measures per type for each country
...
# Print the table using flextable
...
#Bit of cleaning and troubleshooting
measures_all_cleaned_EMP_2 = measures_all_cleaned_EMP
# Replace values in the progress column
measures_all_cleaned_EMP_2$progress <- gsub("NA", "Not Available / Not Pertinent", measures_all_cleaned_EMP_2$progress)
measures_all_cleaned_EMP_2$progress <- gsub("NP", "Not Available / Not Pertinent", measures_all_cleaned_EMP_2$progress)
# Combine NA and "Not Available / Not Pertinent" into one category
measures_all_cleaned_EMP_2$progress[is.na(measures_all_cleaned_EMP_2$progress)] <- "Not Available / Not Pertinent"

#PLOT 1

# Create a contingency table that includes NA values
ProgressCountTable <- table(measures_all_cleaned_EMP_2$measure_type, measures_all_cleaned_EMP_2$progress)
...
ProgressCountTable_country <- table(measures_all_cleaned_EMP_2$country, measures_all_cleaned_EMP_2$progress)
...
ProgressCountTable_combined <- table(measures_all_cleaned_EMP_2$measure_type, measures_all_cleaned_EMP_2$country, measures_all_cleaned_EMP_2$progress)
...
plot3_progress <- ggplot(ProgressCountTable_combined, aes(x = measure_type, y = percentage, fill = progress)) +
  geom_bar(stat = "identity", position = "stack") +
  geom_text(aes(x = measure_type, y = 100, label = paste0("n = ", total_count)), 
            vjust = -0.5, size = 2, inherit.aes = FALSE, angle = 45, hjust = -0.2) +
  facet_wrap(~ country, ncol = 3, nrow = 6) +
  labs(x = "Measure Type", y = "Percentage", title = "", fill = "Progress") +
  theme_void() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, size = 7),
...
#clean one entry
measures_all_cleaned_EMP$effect_size_true <- gsub("Î”Î£F", "ΔΣF", measures_all_cleaned_EMP$effect_size_true)
measures_all_cleaned_EMP$effect_size_true <- gsub("Other", "Not monitored", measures_all_cleaned_EMP$effect_size_true)
...
measures_direct <- measures_all_cleaned_EMP %>%
...
# Print each table using flextable with autofit
for (i in seq_along(direct_tables)) {
  cat("\n### Measure Type:", measure_types[i], "\n")
  ft <- flextable(direct_tables[[i]]) %>% autofit()
  print(ft)
}
...
target_by_mt <- ggplot(percentage_long_mt, aes(x = measure_type, y = no_measures, fill = category)) +
  ...
  scale_fill_viridis_d(labels = c("a_total" = "No target", "b_target" = "Target only", "c_semi" = "Achievement semi-quantified", "d_quant" = "Achievement quantified")) +
  scale_y_continuous(limits = c(0, 120), breaks = seq(0, 100, by = 25)) +
  scale_x_discrete(labels = label_wrap(10), expand = expansion(add = c(1.00, 1.05)))  #label_wrap splits it into 2 rows
