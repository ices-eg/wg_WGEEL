...
measures_target <- measures_all_cleaned_EMP %>%
  select(id, country, emu_name_short, measure_type, submeasure_type, measure_description, target_value_numeric, target_degree_achieved, target_value_achieved_numeric, target_value, target_value_achieved, target_unit, effectiveness_monitored, estimated_effect_size, effect_size_unit) %>% 
...
target <- ggplot(percentage_long, aes(x = measure_type, y = no_measures, fill = category)) +
  ...
  geom_text(aes(x = measure_type, y = 105, label = paste0("", measures_total)), 
            vjust = 0.22, size = 1.5, inherit.aes = FALSE, angle = 0) +
  labs(x = "Measure Type", y = "Percentage", title = "", fill = "Progress") +
