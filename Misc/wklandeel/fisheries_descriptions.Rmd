...
##### BE AWARE! SPAIN USED "-" instead of ">". This is only corrected in the long format!!! ####
...
# create graphs of EMUs with uncertainties for commercial fisheries 
 com_land_comp <- function(x) {
  ggplot(com_land %>% filter(eel_lfs_code == x) %>% filter(!is.na(eel_value.db) | !is.na(min_landings_cor) |!is.na(max_landings_cor)), aes(x=eel_year, y=eel_value.db)) +
  geom_line() +
...
lapply(c("G", "Y", "S", "YS"), com_land_comp)
 ...
 rec_land_comp <- function(x) {
  ggplot(rec_land %>% filter(eel_lfs_code == x) %>% filter(!is.na(eel_value.db) | !is.na(min_landings_cor) |!is.na(max_landings_cor)), aes(x=eel_year, y=eel_value.db)) +
  geom_line() +
  geom_line(aes(x=eel_year, y=min_landings_cor), color = "red") +
  geom_line(aes(x=eel_year, y=max_landings_cor), color = "red") +
  theme_bw() +
...
lapply(c("G", "Y", "S", "YS"), rec_land_comp)
