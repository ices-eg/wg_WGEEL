...
t.dropped <- t.filterchoice %>%
  ...
  select(ser_nameshort, n, reason.dropped, notes) %>% # drop method description
...
t.kept <- t.filterchoice %>%
  ...
  select(ser_nameshort, n, effort, notes) %>% # drop method description
  mutate(notes = ifelse(is.na(notes), "", notes), #change NAs to blanks in notes
  effort = case_when( #Change to reflect whether or not the reported value was divided by effort value
    effort == "index" ~ "No",
