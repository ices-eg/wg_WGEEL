...
```{r table_summary, echo = FALSE, tab.cap = paste("Summary of available silver eel series with at least", min.length,  "years of data, and with available information on quality id (rating), habitat, sampling type, effort, gear, restocking influence, and distance to sea.", sep = " ")}
d.series = data %>% 
  select(ser_nameshort, 
         ser_qal_id,
         ser_hty_code,
...
t.summary = data.frame(category = character(),
  ...
  add_row(category = "Nb of series with quality id",
         availabledata = nrow(d.series %>% filter(!is.na(ser_qal_id))),
         missingdata = nrow(d.series %>% filter(is.na(ser_qal_id)))) %>%
  add_row(category = "Nb of series with habitat data",
         availabledata = nrow(d.series %>% filter(!is.na(ser_hty_code))),
         missingdata = nrow(d.series %>% filter(is.na(ser_hty_code)))) %>%
  add_row(category = "Nb of series with sampling type",
         availabledata = nrow(d.series %>% filter(!is.na(sam_samplingtype))),
         missingdata = nrow(d.series %>% filter(is.na(sam_samplingtype)))) %>%
  ...
  add_row(category = "Nb of series with gear",
         availabledata = nrow(d.series %>% filter(!is.na(ser_sam_gear))),
         missingdata = nrow(d.series %>% filter(is.na(ser_sam_gear)))) %>%
  add_row(category = "Nb of series with restocking data",
         availabledata = nrow(d.series %>% filter(!is.na(ser_restocking))),
         missingdata = nrow(d.series %>% filter(is.na(ser_restocking)))) %>%
  add_row(category = "Nb of series with distance to sea",
         availabledata = nrow(d.series %>% filter(!is.na(ser_distanceseakm))),
         missingdata = nrow(d.series %>% filter(is.na(ser_distanceseakm)))) %>%
...
colorpalette=cbf_1 <- c("red","black") #may not be needed
...
europe_cropped <- ggplot() +
  geom_sf(data = world) +
  coord_sf(xlim = c(-13, 29), ylim = c(35, 65)) +
  ggtitle("Europe")
...
# ggplot(silver_map)+geom_sf(data=europe_cropped)+geom_sf(data=silver_map,col="red")+
  ...
  my_map=get_stamenmap(bbox = c(left = -13, bottom = 35, right =
                                  29, top = 65), zoom = 6, maptype = c("terrain-background"))
...
ggmap(my_map) + 
	geom_point(data=silver_map,aes(x=ser_x,y=ser_y, colour=silver_map$Updated),cex=2,pch=20)+
  labs(x="", y="", color="Legend Title\n") +
  scale_color_manual(labels = c("TRUE", "FALSE"), values=c("black", "red")) +
  geom_label_repel(data=silver_map,
	aes(x=ser_x,y=ser_y,label=ser_nameshort,col=silver_map$Updated),cex=1.5,  max.overlaps = Inf, label.padding = 0.1, show.legend = FALSE) +
	scale_color_manual("Updated within the last 3 years",values=colorpalette)+
	ylab("")+xlab("") +
