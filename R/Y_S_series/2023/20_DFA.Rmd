...
The best model selected thanks to AICc has 2 trends and a $R$ diagonal and equal. The two trends were represented in the Figure \@ref(fig:graph-trend-long). The factor loadings $Z$ are displayed in the Figure \@ref(fig:serie-trend-long). When the factor loading is positive (respectively negative) the given time series is positively (resp. negatively) related to the given trend. If factor loading crosses the 0, the given time series is not significatevely related to the given trend.
...
For this analysis on the data since 2007, the best model selected thanks to AICc has 1 trends and a  R diagonal and equal. This trend was represented in the Figure \@ref(fig:graph-trend-short). The factor loadings are displayed in the Figure \@ref(fig:serie-trend-short)(importance of each trend in each time series). Nearly all factor loading encompass the zero, meaning a weak support to the common trend. This can be due to a high variability in each individual time series compare to a low number of time series.
