...
```{r MARSShypotheses, echo = FALSE, tab.cap = "Mapping between silver eel time series and different hypothesis on sub-population structure. Hypotheses being tested refer to: H1 - panmictic population, i.e. all time series share the same trend; H2 - Habitat type (C - Coastal, T - Transitional, F- Freshwater); H3 - Restocking (indicator of whether the time series is affected by restocking (TRUE) or not affected by resstocking (FALSE)); H4 - ICES ecoregions; H5 - Whether a series is sampled in the North Sea region or elsewhere in Europe; H6 - Size of the catchment area (Small, Medium, Large or Sea)", message = FALSE, warning = FALSE}
...
load("C:/Users/cboulenger/Documents/wg_WGEEL/R/Y_S_series/MARSSdata.RData")
...
```{r MARSSmodeloutput, echo = FALSE, tab.cap = "Model comparison between MARSS models. H refers to the hypothesis on sub-population structure being tested, logLik - Total Log-likelihood of the model, AIC<sub>c</sub> - Akaikes information criteria with a correction for small sample sizes, num.param - number of parameters in the model, m - number of subpopulations for the model, num.param - number of parameters in the model, converged - an indicator of whether the model converged (TRUE) or not (FALSE)", message = FALSE, warning = FALSE}
...
```{r plotsubpopulation-trends, echo=FALSE, fig.cap = "Sub-population trends for the most parsimonious MARSS model(H2: Habitat type). The figure shows estimated sub-population trends for time series sampled in coastal(C; red), freshwater(F; green) and transitional waters(T; blue). Lines show mean estimates and dash lines represent 95 % confidence intervals", fig.width=16/2.54, fig.height=16/2.54, message = FALSE, warning = FALSE}
...
```{r Plotsmoothedstate, echo=FALSE, fig.cap = "MARSS model predictions of silver eel time series. Lines represent mean estimates and dashed lines represent 95 % confidence intervals. Colors display time series from coastal waters (red), freshwaters (green) and transitional waters(blue)", fig.width=16/2.54, fig.height=24/2.54, message = FALSE, warning = FALSE}
...
ggplot(Yt,aes(y=yt, x=Year, color=`Sub-populations`)) + 
  ...
  geom_line(aes(y=`Hi 95`),linetype="dashed") +
  geom_point(aes(y=y)) +
  facet_wrap(~reorder(site,ser_rank),scales="free_y", ncol=4) +
  theme_bw() + 
...
ggplot(Xt,aes(group=`Sub-populations`, y=xt, x=Year,color=`Sub-populations`)) + 
  ...
  geom_line(aes(y=`Lo 95`),linetype="dashed") +
  geom_line(aes(y=`Hi 95`),linetype="dashed") +
  facet_wrap(vars(`Sub-populations`),ncol=length(unique(Xt$`Sub-populations`))/2)  +
  theme_bw() + 
	theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1),legend.position = "top", 
