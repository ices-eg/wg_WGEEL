...
create_series = function(series_info, meta)
{
	...
	return(sqldf("SELECT ser_id FROM datawg.t_series_ser JOIN series_info USING(ser_nameshort)") %>% pull())
}
...
check_dataseries = function(dataseries, ser_data)
{
	...
	#chek for already existing series
	existing_data = inner_join(dataseries, ser_data %>% select(das_id, das_ser_id, das_year, das_value), by = c("ser_id" = "das_ser_id", "das_year" = "das_year"))
...
insert_dataseries = function(dataseries)
{
	...
	# retrieve le ser_id for further use
	return(sqldf("SELECT das_id FROM datawg.t_dataseries_das JOIN dataseries USING(ser_nameshort)") %>% pull())
}
#'INSERT INTO datawg.t_dataseries_das (das_value, das_ser_id, das_year, das_comment, das_effort, das_last_update, das_qal_id) VALUES(0, 0, 0, '', 0, '', 0);'
