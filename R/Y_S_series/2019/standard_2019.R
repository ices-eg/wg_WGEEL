...
update_series(series_info = country_data$series_info %>% inner_join(chk_series$existing_series) %>% 
				select(- ser_tblcodeid, - nrow), show_updated_series = show_updated_series, all = TRUE) 
...
{
	chk_dataseries = check_dataseries(dataseries = series_info %>% select(ser_id, ser_nameshort) %>%
					inner_join(country_data$data) %>% select(das_value, ser_id, das_year, das_comment, das_effort), ser_data)
	
	chk_dataseries$to_be_created_series$das_id = insert_dataseries(dataseries = chk_dataseries$to_be_created_series %>%
					select(-nrow) %>% mutate_at(c("das_value", "das_effort"), convert_round))
