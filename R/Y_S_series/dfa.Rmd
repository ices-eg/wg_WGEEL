...
```{r mapyellow,echo=FALSE,message=FALSE,warning=FALSE,fig.cap="Map of available yellow time series",fig.height=14/2.54}
...
# ggplot(yellow_map)+geom_sf(data=europe_cropped)+geom_sf(data=yellow_map,col="red")+
  ...
  ggmap(my_map) + 
    ...
    geom_text(data=yellow_map,
               aes(x=ser_x,y=ser_y,label=ser_nameshort),cex=1.5)+
    ylab("")+xlab("")
...
```{r seriesloadingsYE, echo=FALSE,fig.cap="Factor loadings of the yellow eel DFA (red name stands form loadings absolute values greater than 0.1)",fig.height=16/2.54}
...
```{r mapsilver,echo=FALSE,message=FALSE,warning=FALSE,fig.cap="Map of available silver eel time series",fig.height=14/2.54}
