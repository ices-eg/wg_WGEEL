...
$$
\begin{aligned}
Y_{j,t}=\mu_j + \sum_{i=1}^{n} w_{i,j} \cdot X_{i,t} +\epsilon_{j,t}~~~~ \mbox{ with } \left \{ \epsilon_{j,t} \right \}  \sim N(0,\Sigma)
\end{aligned}
$$
...
$$
\begin{aligned}
X_{i,t}=X_{i,t-1}+f_{i,t} ~~~~ \mbox{ with } f_{i,t} \sim N(0,Q)
\end{aligned}
$$
...
model_comparisons= lapply(1:4,function(m){
...
results_yellow=do.call(rbind.data.frame,lapply(model_comparisons,function(x) {
...
```{r dfa bis,echo=FALSE}
kable(results_yellow,digits=2)
best_fit_yellow=model_comparisons[[which(results_yellow$AIC==min(results_yellow$AIC))]]$dfa
```
...
H.inv<-varimax(coef(best_fit_yellow, type="matrix")$Z)$rotmat
#rotate factor loadings
Z.rot<-coef(best_fit_yellow,type="matrix")$Z%*%H.inv
#rotate trends
trends.rot<-solve(H.inv)%*%best_fit_yellow$states
scale_val=coef(best_fit_yellow)$A
...
for(i in 1:m) {
  plot(c(1:N_ts)[abs(Z.rot[,i])>minZ], as.vector(Z.rot[abs(Z.rot[,i])>minZ,i]),
       type="h", lwd=2, xlab="", ylab="", xaxt="n", ylim=ylims, xlim=c(0,N_ts+1))
  for(j in 1:N_ts) {
    if(Z.rot[j,i] > minZ) {text(j, -0.05, namesseries[j], srt=90, adj=1, cex=0.5)}
    if(Z.rot[j,i] < -minZ) {text(j, 0.05, namesseries[j], srt=90, adj=0, cex=0.5)}
...
##kept if two trends
loadings_y = as.data.frame(Z.rot[,1:2])
...
d <- broom::tidy(best_fit_yellow,type="states")
levels(d$term)=paste("italic(alpha[list(",1:m,",t)])",sep="")
ggplot(data = d) + 
  geom_line(aes(t, estimate)) +
  geom_ribbon(aes(x=t, ymin=conf.low, ymax=conf.high), linetype=2, alpha=0.1) +
  facet_wrap(~term,nrow=3,labeller=label_parsed) +
  xlab("Time Step") + ylab("")+
  scale_x_continuous(breaks=seq(1,TT,5),labels=colnames(y)[seq(1,TT,5)])+
  ylab("")+xlab("")+theme_bw()
```
...
H.inv<-varimax(coef(best_fit_silver, type="matrix")$Z)$rotmat
#rotate factor loadings
Z.rot<-coef(best_fit_silver,type="matrix")$Z%*%H.inv
#rotate trends
trends.rot<-solve(H.inv)%*%best_fit_silver$states
scale_val=coef(best_fit_silver)$A
...
d <- broom::tidy(best_fit_silver,type="states")
levels(d$term)=paste("italic(alpha[list(",1:m,",t)])",sep="")
ggplot(data = d) + 
  geom_line(aes(t, estimate)) +
  geom_ribbon(aes(x=t, ymin=conf.low, ymax=conf.high), linetype=2, alpha=0.1) +
