...
This method allow to summarize the sets of time series as a mixture of a few set 
of common trends. Then, it is possible to analyse the importance of each common 
trend in the different time series and to carry out cluster analysis. Such a clustering
exercice is worthwhile only when the number of available time series is important, 
therefore, we restricted this analysis to the recent period (post 2000) and focus
on recent trends. To complete the overview, we carried out gam analysis on a longer period
(since 1975) to analyse the long-terms trends.
...
## Trends after 2000
There are few data series started before 2000 (Figure \@ref(fig:pointperyearYE)) 
and many data values for 2020 were still missing when carrying out the analysis.
...
2019, with dataseries that have at least 10 observations on the period.
...
NO SE, with a more complex curve for SE (Figure \@ref(fig:gamYellow)). 
...
of abundance since the 2000s (Figure \@ref(fig:vennyellow)). Some timeseries are
negatively correlated to trend 1, but amont them, many are also positively
correlated to trend2, indicating a slight increase or stability at the beginning
...
## Trends since 2000
Similarly to yellow eels, few data series started before 2000 so (Figure 
\@ref(fig:pointperyearSE)). In view of this, we restricted the analysis to the period 2000 to 2019,
with time series having at least 10 observatoins over the period. . 
...
the series, a gam smoother indicates an overall decreasing trend (Figure
\@ref(fig:plotTSSE)).
...
When plotted per country, a simple gam smoother shows descreating trend in DK,
...
### Running the DFA
We applied the same method as for yellow eel so readers can refer to the 
corresponding section for further details.

### Common trends
Before running the DFA, values were logtransformed (a 0 value was recorded for a
single observation (time series SkaY) and was replaced by 10% of its lower value)
and scales ((mean deleted and divided by the standard deviation)).
...
not correlated to any trends, suggesting a stability. As for yellow eel, 
there are no obvious spatial pattern in the trends 
...
all over Europe for many years. This analyis is a first intent to analyse the 
available data. As explained in the introduction, such as analysis is complex
du to the complexity of the life of the species and its fractal dimension during
the continental growth phase [@dekker2000a].

Two complementary analysis were used: a DFA analysis to depict common trends in the 
...
management practices) among river basins and there are no clear spatial pattern
in the trends. Interestingly, while a shift of trend was detected for yellow eel
around 2007 (trend 2), this shift occured before or just after the implementation
of the Eel Regulation so that the Regulation can not be the only direct cause. 
...
Plan, but also changes in local environmental conditions for some times. On the
whole, cince other trends are nearly monotonic, it suggests that the Eel
Regulation has not led to major changes in the abundance of yellow and 
silver eel, but this it not necessarily a surprise since the growth phase 
...
in many countries, the decrease appears to start latter in Spain. As such, it would have
...
Moreover, this increase is concommitant with some increasing oscillations, for example in Ireland,
so this may have been a temporary situation. Sweeden display atypical increasing trends compared
to other countries. It is difficult to conclude on the reasons of these trends, but we can 
suggest that fishery closure in Western Sweeden may have played a role, and restocking may also
have played a role (though it occurs mainly in Inland EMUs).

As a first analysis, the results do provide a very partial results. It would be wortwhile
carrying out further analyis to understand the reasons of this different trends. More
...
method may also be analysed. As a next step, it would be worthwile comparing trends with
recruitment trends.
