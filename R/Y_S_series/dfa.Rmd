...
nbpointsgam = yellow_data %>%
  group_by(ser_id) %>%
  summarise(nbyear=n_distinct(das_year))%>%
  filter(nbyear>=10)
yellow_data_gam <-yellow_data %>%
  filter(ser_id %in% nbpointsgam$ser_id & das_year <= 2019)
...
We also fitted gam over the whole period of available data in order to put the post-2000
trends in an historical perspective. When carrying out this, the gam smoother clearl show
a decrease in abundance in all countries since the beggining of the period (Figure \@ref(fig:gamYellowHist)).
...
g0 <- gam(das_value~s(das_year), data=subset(yellow_data_std_gam,das_value>0))
g1 <- gam(das_value~s(das_year, by=ser_cou_code), data=subset(yellow_data_std_gam,das_value>0))
...
#For GAMS, we do not restrict to the post 2000 period
#we keep only data with at least 10 years
nbpointsgam = silver_data %>%
  group_by(ser_id) %>%
  summarise(nbyear=n_distinct(das_year))%>%
  filter(nbyear>=10)
silver_data_gam <-silver_data %>%
  filter(ser_id %in% nbpointsgam$ser_id & das_year <= 2019)
...
We also fitted gam over the whole period of available data in order to put the post-2000
trends in an historical perspective. When carrying out this, the gam smoother clearl show
a decrease in abundance in all countries since the beggining of the period (Figure \@ref(fig:gamSilverHist)).

```{r gamSilverHist, echo=FALSE,warning=FALSE, fig.cap="Trends per country in yellow eel abundance estimated by a gam"}
...
g0 <- gam(das_value~s(das_year), data=subset(silver_data_std_gam,das_value>0))
g1 <- gam(das_value~s(das_year, by=ser_cou_code), data=subset(silver_data_std_gam,das_value>0))
g2 <- gam(das_value~s(das_year, by=ser_nameshort), data=silver_data_std_gam)
#AIC(g0, g1, g2)
cou_code <- levels(droplevels (silver_data_std_gam$ser_cou_code))
