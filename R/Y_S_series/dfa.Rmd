...
knitr::opts_chunk$set(echo = TRUE,warning=FALSE,message=FALSE,fig.width=16/2.54)
...
This method allows summarising the sets of time series as a mixture of a few common trends. Then, it is possible to analyse the importance of each common 
trend in the different time series and to carry out cluster analysis. Such a clustering
exercise is worthwhile only when the number of available time series is important, 
...
1 to 4 common trends are tested. The best combination of $\Sigma$ and number of trends is chosen by comparing AICc criteria. 
...
```{r dfa yellow, results='hide',eval=FALSE,echo=FALSE}
if (params$rerunDFA){
  #replace 0 value
...
(NL, NO, IE, GB) except in Sweden where an increasing trend is observed  (Figure \@ref(fig:gamYellowHist)).
...
```{r dfa silver, results='hide',eval=FALSE,echo=FALSE}
if (params$rerunDFA){
  #put data in wide format and select from 1980
...
all over Europe for many years. This analysis is a first intent to analyse the 
...
silver eels, but this is not necessarily a surprise since the growth phase 
...
As a first analysis, the results do provide a very partial overview of what have ineed happened. It would be worthwhile
carrying out further analyis to understand the reasons of these different trends. More
...
method may also be analysed. As a next step, it would be worthwhile comparing trends with
recruitment trends.
