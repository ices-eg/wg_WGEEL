...
if (username == "hilaire.drouineau") {
  setwd("/home/hilaire.drouineau/Documents/Bordeaux/migrateurs/WGEEL/github/wg_WGEEL/R/Y_S_series/") # nolint: line_length_linter.
}
...
query <- "
	...
	where 
        ser_typ_id in (2,3) and
        das_value is not null and
...
nb_yellow <- dbGetQuery(
    con_wgeel,
    "SELECT count(*) FROM datawg.t_series_ser WHERE ser_typ_id in (2)"
    ) %>% pull()
nb_silver <- dbGetQuery(
    con_wgeel,
    "SELECT count(*) FROM datawg.t_series_ser WHERE  ser_typ_id in (3)"
) %>% pull()

sampling <- dbGetQuery(
    con_wgeel,
    "SELECT ser_id,	ser_nameshort, ser_cou_code,	ser_comment,	sam_samplingtype,	ser_uni_code,	gea_name_en AS type FROM datawg.t_series_ser LEFT OUTER JOIN ref.tr_gear_gea ON (gea_id = ser_sam_gear)  LEFT OUTER JOIN ref.tr_samplingtype_sam ON (sam_id = ser_sam_id) WHERE ser_typ_id IN (2,3)" # nolint: line_length_linter.
)

# colors for countries
country_ref <- dbGetQuery(
    con_wgeel,
    "SELECT cou_code, cou_country, cou_order FROM ref.tr_country_cou"
)
...
sampling <- sampling %>%
    mutate(color_country = color_countries[ser_cou_code]) %>%
    inner_join(
        country_ref,
        by = c("ser_cou_code" = "cou_code")) %>%
        group_by(ser_cou_code) %>%
        mutate(rank = cou_order * 1000 + dense_rank(ser_nameshort)
    )

mydata <- merge(
    mydata,
    sampling[, c("ser_id", "type")]
)
...
yellow_map$Updated <- yellow_map$ser_id %in%
    unique((mydata %>% filter(das_year >= 2018) %>% select(ser_id) %>% pull()))
...
map_series(
    ...
    mapbox = c(left = -13, bottom = 35, right = 27, top = 65)
)
...
#  mutate(das_value=log(das_value))%>%  # log scale remove because some series (10 in 2021) with 0)
  group_by(ser_id)%>%
  mutate(das_value=(das_value-mean(das_value,na.rm=TRUE))/sd(das_value,na.rm=TRUE))

#summary(gam(das_value ~ das_year, data = yellow_data_std))
...
g6 <- gam(das_value~s(das_year, by=ser_restocking), data=yellow_data_std)

# Graph of the results (to put r gamYellow)
yellow_gam_plot_recent=generate_dataset_plot_gam_res(g6)
ggplot(yellow_gam_plot_recent,aes(x=year,y=pred))+
	geom_line()+
	geom_ribbon(aes(ymin=lower,ymax=upper),alpha=.3)+
...
# GAM (to put r gamYellow)
g2 <- gam(das_value~s(das_year, by=ser_hty_code), data=yellow_data_std_hty)

# Graph (to put r gamYellow)

hty_code <- levels(droplevels (yellow_data_std_hty$ser_hty_code))

yellow_gam_plot_recent=generate_dataset_plot_gam_hty(g2)
ggplot(yellow_gam_plot_recent,aes(x=year,y=pred))+
	geom_line()+
	geom_ribbon(aes(ymin=lower,ymax=upper),alpha=.3)+
...
if (params$rerunDFA){
  ...
  model_comparisonsYE= mcmapply(function(s,m){
    ...
    kemz=MARSS(y, model=dfa.model, form="dfa",z.score=TRUE,control=list(maxit=2000),method="BFGS")
    aicc=MARSSaic(kemz, output="AICc")$AICc
    aic=MARSSaic(kemz, output="AIC")$AIC
    list(Trends=m,AICc=aicc,AIC=aic,Sigma=s,dfa=kemz)
  },as.character(expe$S),expe$nbtrend, mc.cores=1, SIMPLIFY=FALSE) # use mc.cores=4 if on linux
...
load("./dfa.rdata")
ft <- flextable(results_yellow[,c("Trends","Sigma","AIC")] %>% mutate(AIC = round(AIC))) #digits no more supported in colformat_num
ft <- colformat_num(ft, j="AIC", big.mark = "")
autofit(ft)
...
country = distinct(yellow_data[, c("ser_nameshort", "cou_order", "ser_cou_code")])
colnames(Z_tab) = c("Series", paste0("Trend ", 1:m))
Z_tab = Z_tab %>% inner_join(country, by = c("Series" = "ser_nameshort"))
Z_tab = Z_tab %>% arrange(cou_order, Series) %>% mutate(Country = as.character(ser_cou_code)) %>% select(Country, Series, 2:(1+m))
ft <- Z_tab  %>% mutate(across(3:(2+m), round, 2))
...
#ggplot(as.data.frame(Z_tab), aes(x = `Trend 1`, y = `Trend 2`, label = Series, col = Country))+
#	geom_text() + scale_color_manual(values = color_countries) +
...
yellow_z_dfa<-data.frame(namesseries=namesseries, z1=Z[,1], z2=Z[,2])
...
modelz2<-glm(z2~ res+hty+as.factor(sam_gear)+distanceseakm, family = gaussian, data = yellow_dfa)
...
if (params$rerunDFA_silver) {
    ...
    for (i in list_zeros){
        ...
        silver_data$das_value[i] <- 0.2 *
            min(
                silver_data$das_value[
                    silver_data$das_value > 0 &
                    silver_data$ser_id == ser_id
                ], na.rm = TRUE
            )
    }
    # put data in wide format
    data_dfa_silver <- silver_data %>%
        as_tibble() %>%
        select(ser_nameshort, das_value, das_year) %>%
        arrange(das_year) %>%
        pivot_wider(
...
ft <- flextable(results_silver[,-c(4,5)] %>% mutate(AIC = round(AIC)))
ft <- colformat_num(ft,j="AIC", big.mark = "")
autofit(ft)
...
graph_trends(
    DFA_results$trends,
    year = sort(unique(silver_data$das_year)),
    sign_trends = -1
)
...
```{r seriesloadingssilver, echo=FALSE,fig.cap="Factor loadings of the silver eel DFA (red names stand for loadings absolute values greater than 0.2)"}
ylims = c(-1.1*max(abs(Z)), 1.1*max(abs(Z)))
