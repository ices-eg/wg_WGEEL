...
yellow_group_Venn = Venn_yellow$venn %>% 
	...
	map_series(yellow_group_Venn, variable = "Venn's group", mapType = c("terrain-background"), transparent = TRUE, labels = FALSE, pch_cex = 3)
...
silver_data_gam <-silver_dataall %>%
  filter(ser_id %in% nbpointsgam$ser_id & das_year>=1975 & das_year <= (current_year-1))
...
silver_data <-silver_dataall %>%
  filter(ser_id %in% nbpoints$ser_id & das_year>=2000 & das_year<=(current_year-1))
...
`r point_per_year_SE %>% filter(das_year == current_year) %>% pull()` series have data for `r current_year`). 
