...
mydata$sam_samplingtype[mydata$ser_id %in% 292:295]="electrofishing"
...
# Yellow eel
## Available data
`r length(unique(yellow_dataall$ser_id))` time series (out of `r nb_yellow`) provide data through the current or past Data Call (Figure
\@ref(fig:mapyellow)), originating from `r length(unique(yellow_dataall$ser_cou_code))` countries and
`r length(unique(yellow_dataall$ser_emu_nameshort))` EMUs. Most of them are located in Great Britain or France.
`r length(unique(yellow_dataall$ser_id[yellow_dataall$ser_hty_code=="C"]))` time series are collected in coastal waters,
...
<!-- à voir pour faire les stat : yellow_dataall %>% select(ser_id, ser_cou_code) %>% distinct() %>% group_by(ser_cou_code) -->
<!-- %>% count() -->
...
# ggplot(yellow_map)+geom_sf(data=europe_cropped)+geom_sf(data=yellow_map,col="red")+
  ...
  ggmap(my_map) + 
    geom_point(data=yellow_map,
               aes(x=ser_x,y=ser_y),cex=.5,pch=".")+
    geom_text(data=yellow_map,
               aes(x=ser_x,y=ser_y,label=ser_nameshort,col=Updated),cex=1.5)+
    scale_color_manual("Upated",values=colorpalette)+
    ylab("")+xlab("")
...
an effort should be made in the future to collect historical data. Many data values for 2020 were still missing when carrying out the analysis. 
...
2019, with data series that have at least 10 observations on the period.
...
When plotted per country, a simple gam smoother shows decreasing and
stabilising curves in  FR, NL. The same with a more pronounced increase after
2012 is estimated for ES (DK seems to display a similar trend but the time series is shorter).
On the other hand, a linear decrease is estimated in IE, GB and DE, and an increase for
SE (Figure \@ref(fig:gamYellow)). 

```{r gamYellow, echo=FALSE,warning=FALSE, fig.cap="Trends per country in yellow eel abundance estimated by a gam (log scale)"}
...
save(results_yellow, model_comparisonsYE, yellow_wide, file="./dfa.rdata")
}
```

Two common trends were estimated after selection by AIC criteria (Table \@ref(tab:comparisonYE) and Figure \@ref(fig:trendsyellow)).

```{r comparisonYE,echo=FALSE,tab.cap='Model comparisons for yellow eel DFA'}
load("./dfa.rdata")
ft <- flextable(results_yellow[,c("Trends","Sigma","AIC")])
ft <- colformat_num(ft,j="AIC",digits=2)
autofit(ft)
...
The factor loadings $w$ are displayed in the following plots (Figure \@ref(fig:seriesloadingsYE)). Following @zuur2003, we only focused on loading with absolute values greater than 0.1 to focus on the most important trends and presented on a Venn diagram (Figure \@ref(fig:vennyellow))


```{r seriesloadingsYE, echo=FALSE,fig.cap="Factor loadings of the yellow eel DFA (red names stand for loadings absolute values greater than 0.1)",fig.height=16/2.54}
...
```{r fitsyellow, echo=FALSE,warning=FALSE, fig.cap="Yellow DFA fits to time series", fig.width=16/2.54, fig.height=16/2.54}
d <- residuals(best_fit_yellow,interval="confidence") #augment is replace since MARSS 3.11, see https://github.com/nwfsc-timeseries/MARSS/releases
...
`r length(unique(silver_dataall$ser_emu_nameshort))` EMUs. Most of them are located in Northern Europe.
`r length(unique(silver_dataall$ser_id[silver_dataall$ser_hty_code=="C"]))` time series are collected in coastal waters
and `r length(unique(silver_dataall$ser_id[silver_dataall$ser_hty_code=="F"]))` in freshwater, while the habitat type
was not reported for `r length(unique(silver_dataall$ser_id[is.na(silver_dataall$ser_hty_code)]))`.
...
```{r mapsilver,echo=FALSE,message=FALSE,warning=FALSE,fig.cap="Map of available silver time series. Updated time series correspond to time series for which at least one value was provided for years the three last years",fig.height=14/2.54}
 ...
 europe_cropped <- st_crop(worldmap, xmin = -13, xmax = 27,
                                     ymin = 35, ymax = 65)
# ggplot(silver_map)+geom_sf(data=europe_cropped)+geom_sf(data=silver_map,col="red")+
   #theme_bw()
  my_map=get_stamenmap(bbox = c(left = -13, bottom = 35, right =
                                  27, top = 65), zoom = 6, maptype = c("watercolor"))
  ggmap(my_map) + 
...
## Trends since 2000
Similarly to yellow eels, few pre-2000 data series are available in the database (Figure 
\@ref(fig:pointperyearSE)). In view of this, we restricted the analysis to the period 2000 to 2019,
with time series having at least 10 observations over the period. 
...
#  save.image(file="./silver_dfa.rdata")
  save(results_silver, model_comparisonsSE, silver_wide, file="./silver_dfa.rdata")
}
```
...
load("./silver_dfa.rdata")
results_silver$convergence=as.character(results_silver$convergence)
ft <- flextable(results_silver[,-c(4,5)])
ft <- colformat_num(ft,j="AIC",digits=2)
autofit(ft)
...
```{r seriesloadingssilver, echo=FALSE,fig.cap="Factor loadings of the silver eel DFA (red names stand for loadings absolute values greater than 0.1)"}
ylims = c(-1.1*max(abs(Z)), 1.1*max(abs(Z)))
...
d <- residuals(best_fit_silver,interval="confidence") #augment is replace since MARSS 3.11, see https://github.com/nwfsc-timeseries/MARSS/releases
d$.conf.low <- d$.fitted+qnorm(0.05/2)*d$.sigma
d$.conf.up <- d$.fitted-qnorm(0.05/2)*d$.sigma
ggplot(data = d) +
  geom_line(aes(t, .fitted)) +
