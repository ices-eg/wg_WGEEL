...
![Yellow eel series in Europe.](images/yellow.png)

There are few data series started before 2000 so we sill restrict the analysis to the dataseries that have at least 10 observations after 2000 and stops in 2019 to avoid incomplete data in 2020. 
...
This leaves `r length(unique(yellow_data$ser_id))` time series. If we plot all
the series, a gam smoother indicates an overall decreasing trend. 

```{r plotTS_YE,echo=FALSE,warning=FALSE}
...
# mutate(das_value=scale(das_value)
...
NO SE, with a more complex curve for SE. The trend correspdond to log
transformed and scaled values.

```{r gam_yellow, echo=FALSE,warning=FALSE}


g0 <- gam(das_value~s(yellow_data$das_year), data=yellow_data_std)
g1 <- gam(das_value~s(yellow_data$das_year, by=ser_cou_code), data=yellow_data_std)
g2 <- gam(das_value~s(yellow_data$das_year, by=ser_nameshort), data=yellow_data_std)
AIC(g0, g1, g2)
...
for (i in 1:length(cou_code)){
	plot(g1,residuals=TRUE,select=i, shade=TRUE,shade.col=cols[i], ylab=cou_code[i])
}
...
```{r dfa_yellow, results='hide',eval=FALSE}
#replace 0 value
...
The factor loadings are displayed in the following plots (importance of each trend in each time series). Following @zuur2003, we only focused on loading with absolute values greater than 0.1 to focus on the most important trends. 


```{r series_loadings_yellow, echo=FALSE}
...
Many series are positively correlated to Trend 1 which that indicates a constant decline of the abundance since the 2000s. Some timeseries are negatively correlated to trend 2, but amont them, many are also positively correlated to trend2, indicating a slight increase or stability at the beginning of the period and then a decrease till 2007. Regarding time series dominated mostly by a negative correlation with trend 1 (i.e. a monotonic increase over the period), they all came from Great Britain, and half of them are located far from the coast. On the whole, it is difficult to distinguish any clear spatial pattern in the trends.



```{r fits_yellow, echo=FALSE,warning=FALSE}
...
This leaves `r length(unique(silver_data$ser_id))` time series. If we plot all the series, a gam smoother indicates an overall decreasing trend.


```{r plotTS_SE,echo=FALSE,warnings=FALSE}
...
```{r gam_silver, echo=FALSE,warning=FALSE}
...
```{r dfa_silver, results='hide',eval=FALSE}
#put data in wide format and select from 1980
...
```{r series_loadings_silver, echo=FALSE}
ylims = c(-1.1*max(abs(Z)), 1.1*max(abs(Z)))
par(mfrow=c(ceiling(m/2),2), mar=c(0.5,2.5,1.5,0.5), oma=c(0.4,1,1,1))
for(i in 1:m) {
	#plot(c(1:N_ts)[abs(Z[,i])>minZ], as.vector(Z[abs(Z[,i])>minZ,i]),
...
About `r round(100*sum(silver_belonging$Trend=="Trend1+")/nrow(silver_belonging))`%  of the series are positively correlated to tend 1 indicating a decline in the abundance. However, about `r round(100*sum(silver_belonging$Trend=="Trend1-")/nrow(silver_belonging))`% are negatively correlated suggesting an increase, and about `r round(100*sum(silver_belonging$Trend=="Any")/nrow(silver_belonging))`% are not correlated to any trends, suggesting a stability. As for yellow eel, there are no obvious spatial pattern in the trends.


```{r fits_silver, echo=FALSE,warning=FALSE}
