...
biometry_individual_series <- dbGetQuery(
    con_wgeel,
...
biometry_individual_sampling <- dbGetQuery(
    con_wgeel,
    "SELECT
t_samplinginfo_sai.*,
cou_code, cou_order,
...
biometry_group_data_series_wide <- biometry_group_data_series %>%
    select(-geom, -mty_id, -mty_min, -mty_max) %>%
    pivot_wider(names_from = mty_name, values_from = meg_value)

biometry_group_data_sampling_wide <- biometry_group_data_sampling %>%
    dplyr::select(-mty_id, -mty_min, -mty_max) %>%
    pivot_wider(names_from = mty_name, values_from = meg_value)


biometry_individual_series_wide <- biometry_individual_series %>%
    pivot_wider(names_from = mty_name, values_from = mei_value)


biometry_individual_sampling_wide <- biometry_individual_sampling %>%
    pivot_wider(names_from = mty_name, values_from = mei_value)
```
