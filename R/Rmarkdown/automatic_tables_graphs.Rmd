...
CY <- 2026  # this should be set to the current year, yellow eel data will be analysed up to CY-1
library(stringr)
...
`r max(yearcom_land_YS)` (part 2), reported by countries: `r cou_text3ComLandYS`. The total 
...
column is the yearly sum for all countries included in tables 3a and 2b and 
not just the countries listed in this particular table. Note that values shown as 0.0
are due to rounding and do not correspond to an actual 0.
:::
...
::: {custom-style="Table Caption"}
Table 9: Other_landings of glass eels in millions from `r min(tableotherLandingsG$Year)` to `r max(tableotherLandingsG$Year)` for `r cou_textotherLandingG`. Note that values shown as 0.0
are due to rounding and do not correspond to an actual 0.
:::
...
::: {custom-style="Table Caption"}
Table 10: Other_landings of glass eels (Y) in millions from `r min(tableotherLandingY$Year)` to `r max(tableotherLandingY$Year)` for `r cou_textotherlandingsY`. Note that values shown as 0.0
are due to rounding and do not correspond to an actual 0.
:::
...
::: {custom-style="Table Caption"}
Table 11: Other_landings of silver eels (S) in millions from `r min(tableotherLandingsS$Year)` to `r max(tableotherLandingsS$Year)` for `r cou_textotherlandingsS`. Note that values shown as 0.0
are due to rounding and do not correspond to an actual 0.
:::
