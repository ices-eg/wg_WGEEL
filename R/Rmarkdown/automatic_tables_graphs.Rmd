...
if (getUsername() == "rbva0001") setwd("C:/Users/rbva0001/OneDrive/Rob/SLU/ICES/Working groups/WGEEL/GIT/wg_WGEEL/R/Rmarkdown")
if (getUsername() == "marieke_desender") setwd("G:/Mijn Drive/Paling/ICES/WGEEL/ICES 2026/wg_WGEEL/R/Rmarkdown")
...
CY <- lubridate::year(Sys.Date())  # this should be set to the current year, yellow eel data will be analysed up to CY-1
library(stringr)
...
knitr::opts_chunk$set(echo = TRUE, warnings = FALSE, error = TRUE, dpi = 300)
...
# You need to run the shiny from localhost or get the file from Cedric's server.
# \wg_WGEEL\R\shiny_data_visualisation\shiny_dv\data
# Otherwise you need to run source("C:/workspace/wg_WGEEL/R/shiny_data_visualisation/shiny_dv/load_data_from_database.R")
```
...
export_svg<- function(fig, figname, width = 7, height = 5, units = "in"){
  ggsave(paste0(imgwd,figname, ".svg"),
         fig,
         width = width,
         height = height, units = "in")
  rsvg::rsvg_svg(paste0(imgwd,figname, ".svg"), paste0(imgwd,figname, ".svg"))

}
```

# Key numbers (TO BE REMOVED)

### Glass eel landings
```{r keyNumbersLandingsG, results="asis", eval=TRUE, echo=FALSE}

landings_all <- filter_data("landings", typ = 4) %>% dplyr::filter(is.na(eel_missvaluequal)) |> filter(eel_value > 0)
...
save(g_CY, g_CY1, g_mean_CY2_CY6, cou_G_CY, cou_G_CY1, file = paste0(
  datawd,
...
### Yellow and Silver commercial eel landings
```{r keyNumbersLandingsYS, results="asis", eval=TRUE, echo=FALSE}
landings_YS <- filter_data("landings",
  typ = 4,
  life_stage = c("Y", "S", "YS")
) %>% dplyr::filter(is.na(eel_missvaluequal))  |> filter(eel_value > 0)
...
grouped_data <- group_data(landings_YS, 
                           geo = "country",
                           habitat = FALSE, 
                           lfs = FALSE, 
                           na.rm = TRUE)
...
landings_rec <- filter_data("landings", typ = 6) %>% dplyr::filter(is.na(eel_missvaluequal))  |> filter(eel_value > 0)
...
landings_YS_rec <- filter_data("landings",
  typ = 6,
  life_stage = c("Y", "S", "YS")
) %>% dplyr::filter(is.na(eel_missvaluequal))  |> filter(eel_value > 0)
...
- Number of yellow eels (Y) released in `r CY-1`: `r round(rel_CY1_Y*1000)` thousands, number of countries reporting: `r paste(cou_CY1_relY,sep="", collapse=", ")`
- Number of yellow eels (Y) released in `r CY-2`: `r round(rel_CY1_Y*1000)` thousands, number of countries reporting: `r paste(cou_CY2_relY,sep="", collapse=", ")`
- Number of silver eels (S) released in `r CY-1`: `r round(rel_CY1_S*1000)` thousands, number of countries reporting: `r paste(cou_CY1_relS,sep="", collapse=", ")`
- Number of silver eels (S) released in `r CY-2`: `r round(rel_CY2_S*1000)` thousands, number of countries reporting: `r  paste(cou_CY2_relS,sep="", collapse=", ")`
...
as stock levels. In addition, landings data presented here might be
...
to `r round(YS_CY1 + g_CY1, -1) ` t in `r CY-1` (glass
...
caption <- paste("Table 1:  Glass eel commercial fisheries landings (in tonnes) from 1984 to ", max(year), ", reported by countries: ",
  get_country_table(tableComLandG), ".",
  sep = ""
)
...
including reconstructed data to fill data gaps (following Dekker 2003 and ICES 2013,
a GLM was used for this reconstruction). In Spain, part of the former Basque recreational
...
```{r comlandGfig, echo=FALSE, warning=FALSE, fig.cap=titleG, fig.height=5, fig.width=10, eval=TRUE}
...
g_reconstructed_landings <- addSmallLegend(ggplot(landings3) +
  ...
  theme_bw() +
  geom_magnify(
    from = c(xmin = 2010, xmax = CY, ymin = 0, ymax = 75),
...
export_svg(g_combined_landings_G, 
           "g_combined_landings_G",
           width = 8,
           height = 8)
knitr::include_graphics(paste0(imgwd,"g_combined_landings_G", ".svg"))
```
...
2009 (Figure \@ref(fig:fishingmortality)) (data for 2009 were removed as France, which accounts for a
significant part of the landings, did not report data for that year) and
remained relatively stable since then. Compared to the lowest exploitation
rate in 2025 (resulting from a higher recruitment compared to recent years), 
the currently estimated lower recruitment for 2026 is associated to a high
exploitation rate, similar to that in 2024 but still at a lower level in
compared to pre-eel Regulation. The exploitation rate can only be estimated for glass eel since for
...
knitr::include_graphics(paste0("../recruitment/", CY, "/image/exploitation_rates_GE.svg"))
...
and silver eels and Figure \@ref(fig:corrComYS)
presents the time-series including reconstructed data to fill data gaps (following
Dekker 2003 and ICES 2013,
a GLM was used for this reconstruction). The
...
`r CY-2` (number of countries reporting `r nb_cou_Y_CY1` and `r nb_cou_Y_CY2` respectively).
...
countries <- setdiff(names(table), c("eel_year", "total"))
...
table1 <- table[, c("Year", set1)]
cou_text1ComLandYS <- get_country_table(table1)
table2 <- table[, c("Year", set2)]
cou_text2ComLandYS <- get_country_table(table2)
table3 <- table[, c("Year", set3, "total")]
cou_text3ComLandYS <- get_country_table(table3[, -ncol(table3)])
names(table1) <- gsub("NL", "NL*", gsub("FR", "FR*", names(table1)))
names(table2) <- gsub("NL", "NL*", gsub("FR", "FR*", names(table2)))
names(table3) <- gsub("NL", "NL*", gsub("FR", "FR*", names(table3)))

yearcom_land_YS <- range(table1$Year)
...
## Graph
titleY <- paste0(
  "Time-series of reported commercial yellow (Y), silver (S) and yellow-silver (YS) eel fishery landings (tonnes) 1908-", CY - 1,
...
g_raw_Clandings_YS <- ggplot(landings3) +
  ...
  theme_bw() +
  geom_magnify(
    from = c(xmin = 2010, xmax = CY - 1, ymin = 0, ymax = 3500),
    to = c(xmin = 1996.5, xmax = CY + 3, ymin = 9000, ymax = 13500), axes = "xy"
  )


export_svg(g_raw_Clandings_YS, 
           "g_raw_Clandings_YS",
           width = 8,
           height = 5)
knitr::include_graphics(paste0(imgwd,"g_raw_Clandings_YS", ".svg"))
...
export_svg(g_combined_landings_YS, 
           "g_combined_landings_YS",
           width = 7,
           height = 5)
knitr::include_graphics(paste0(imgwd,"g_combined_landings_YS", ".svg"))
...
In addition to the Regulations described in Chapter 1, section 6,
recreational fisheries in some
...
Recreational glass eel fishery landings decreased from over 1,000 t in the early
...
Recreational landings of yellow and silver eel combined summed up to
...
reported - Figure \@ref(fig:recLandYSfig)). France has provided estimates for all freshwater recreational
...
underestimated. The mean annual landings from yellow and silver eel recreational
...
g_raw_Rlandings_G <- ggplot(raw_Rlandings_G) +
  ...
  theme_bw() +
  geom_magnify(
    from = c(xmin = 2010, xmax = CY, ymin = 0, ymax = 3),
...
export_svg(g_raw_Rlandings_G, 
           "g_raw_Rlandings_G",
           width = 10,
           height = 5)
knitr::include_graphics(paste0(imgwd,"g_raw_Rlandings_G", ".svg"))
...
cou_text1rec_land_Y1 <- get_country_table(table1)
table2 <- table[, c(1, 13:ncol(table))]
cou_text2rec_land_Y <- get_country_table(table2[, -ncol(table2)])
...
ka_rec_land_Y1 <- kable(format(table1, digt = 1, drop0trailing = TRUE), format = "pandoc", caption = paste("part a,  Recreational fisheries landings (in tonnes) for yellow eel and silver eel from ", min(year), " to ", max(year), " (part 1), reported by countries: ", cou_text1rec_land_Y1, " (to be continued for other countries in next table).", sep = ""))
ka_rec_land_Y2 <- kable(format(table2, digt = 1, drop0trailing = TRUE), format = "pandoc", caption = paste("part b,  Recreational fisheries landings (in tonnes) for yellow eel and silver eel from ", min(year), " to ", max(year), " (part 2), reported by countries: ", cou_text2rec_land_Y, " , total.", sep = ""))
...
yearrec_land_Y <- range(table1$Year)
...
color_countries4plot <- color_countries4[names(color_countries4) %in% levels(landings3$eel_cou_code)]
color_countries2 <- color_countries[names(color_countries) %in% levels(landings3$eel_cou_code)]
cou_string <- get_country(data = filter(landings3, eel_value > 0))
## Graph
...
export_svg(g_raw_Rlandings_YS, 
           "g_raw_Rlandings_YS",
           width = 10,
           height = 5)
knitr::include_graphics(paste0(imgwd,"g_raw_Rlandings_YS", ".svg"))
...
all fisheries on the stock. More details on recent events are provided in Section 3.

## Other landings

All other landings, coming neither from professional nor recreational fisheries
...
lesser extent in the Netherlands and Spain (Annex XX).
...
cou_textotherlandingsY <- get_country_table(table) 
...
export_svg(g_other_landings_Y, 
           "g_other_landings_Y",
           width = 10,
           height = 5)
knitr::include_graphics(paste0(imgwd,"g_other_landings_Y", ".svg"))
...
Given that eels can be displaced at different life stages and transported from a few meters within the same waterbody (assisted migration) to being moved between waterbodies and/or EMUs (restocking), WGEEL broadly categories all of them as “releases”, though the term “restocking” is still used in some circumstances. As all data are grouped together, it is hard to distinguish the source of released eels, but other landings data should provide some information on the quantity of eels caught and subsequently released for assisted migration (or other purposes not including commercial or recreational fisheries). For more information, please see Stock Annex, Chapter 1.6.9.

Release data include eels released at the glass eel phase, either directly (G), or after a quarantine (QG), after a period of some months of growth in aquaculture (OG), at the yellow eel (Y) or silver eel (S) stage. There are also some releases of mixed life stages: Glass + Yellow eel (G+Y) and Yellow + Silver eel (Y+S) for Ireland and Spain, but they will not be presented in this report. Dekker and Beaulaton (2016) provide a review of available data that are not included in this report (based on reported data from countries).
  


## Glass eel releases (G + QG)

Restocking of glass eel (G + QG) peaked during the 1980s, followed by a decline to a
...
`r round(rel_CY2_G_t)` and `r round(rel_CY1_G_t)` t in
...
cou_text1release_G <- get_country_table(table1)
table2 <- table[, c(1, 9:ncol(table))]
cou_text2release_G <- get_country_table(table2[, -ncol(table2)])
...
ka_release_G1 <- kable(format(table1, digt = 1, drop0trailing = TRUE), format = "pandoc", caption = paste("part a:  Release of glass eel (G) and quarantined glass eel (QG) in millions from ", min(year), " to ", max(year), ")", ",  reported by countries ", cou_text1release_G, " (to be continued for other countries in next table).", sep = ""))
ka_release_G2 <- kable(format(table2, digt = 1, drop0trailing = TRUE), format = "pandoc", caption = paste("partb:  Release of glass eel in millions from ", min(year), " to ", max(year), ")", ",  reported by countries: ", cou_text2release_G, "", sep = ""))
...
export_svg(g_release_G, 
           "g_release_G",
           width = 10,
           height = 5)
knitr::include_graphics(paste0(imgwd,"g_release_G", ".svg"))
...
cou_string <- get_country(data = release3)
## Graph
titleGrelkg <- paste0("Reported releases of glass eel (G + QG, in tonnes) per country ", cou_string, ". Inset shows years since 2010 in greater resolution. ", CY-3, "-", CY, " are provisional data (not all countries are reported for the last year and Germany has a 3 years reporting cycle).")

g_releasekg_G <- ggplot(release3) +
  geom_col(aes(x = eel_year, y = eel_value, fill = eel_cou_code), position = "stack") +
  # ggtitle(title) +
  xlab("year") +
  ylab("Releases in tonnes") +
  scale_fill_manual("Country", values = color_countries4plot) +
  theme_bw() +
  geom_magnify(
    from = c(xmin = 2010, xmax = CY, ymin = 0, ymax = 20),
    to = c(xmin = 1992, xmax = CY + 1, ymin = 25, ymax = 45), axes = "xy"
  )

#print(g_releasekg_G)

export_svg(g_releasekg_G, 
           "g_releasekg_G",
           width = 10,
           height = 5)
knitr::include_graphics(paste0(imgwd,"g_releasekg_G", ".svg"))
...
# table[is.na(table)]<-''
# table$total<-as.character(table$total)
tableotherLandingsG <- table


ka_other_landings_G <- kable(format(table, digt = 1, drop0trailing = TRUE),
  format = "pandoc", caption =
...
cou_textotherLandingG <- get_country_table(table)
...
export_svg(g_other_landings_G, 
           "g_other_landings_G",
           width = 10,
           height = 5)
knitr::include_graphics(paste0(imgwd,"g_other_landings_G", ".svg"))
...
g_other_landingskg_G <- ggplot(other_landings3) +
  geom_col(aes(x = eel_year, y = eel_value, fill = eel_cou_code), position = "stack") +
  xlab("year") +
  ylab("other_landings in T") +
  scale_fill_manual("Country", values = color_countries2) +
  theme_bw()

#print(g_other_landingskg_G)
export_svg(g_other_landingskg_G, 
           "g_other_landingskg_G",
           width = 10,
           height = 5)
knitr::include_graphics(paste0(imgwd,"g_other_landingskg_G", ".svg"))
...
#print(g_release_QG)
export_svg(g_release_QG, 
           "g_release_QG",
           width = 10,
           height = 5)
knitr::include_graphics(paste0(imgwd,"g_release_QG", ".svg"))
...
filtered_data<-filter_data("release",
    typ = 8,
    life_stage = "QG")%>% dplyr::filter(is.na(eel_missvaluequal))|> filter(eel_value >0)
year<-unique(filtered_data$eel_year)
country<-unique(filtered_data$eel_cou_code)
hty<-unique(filtered_data$eel_hty_code)
grouped_data <-group_data(filtered_data,geo="country",habitat=FALSE,lfs=FALSE,na.rm=TRUE)
...
export_svg(g_releasekg_QG, 
           "g_releasekg_QG",
           width = 10,
           height = 5)
knitr::include_graphics(paste0(imgwd,"g_releasekg_QG", ".svg"))
...
Releases of ongrown eels are presented in Figure \@ref(fig:releaseOGf). They
show an increasing trend since 1990, and reached a maximum in 2022.

Releases of yellow eel are presented in Figure \@ref(fig:releasekgYf). The
quantity of yellow eels (Y) released in `r CY-2` and `r CY-1` was
`r round(rel_CY2_Y_t * 1000)` and `r round(rel_CY1_Y_t * 1000)` kg (number of countries
...
cou_textreOG <- get_country_table(table)
...
export_svg(g_release_OG, 
           "g_release_OG",
           width = 10,
           height = 5)
knitr::include_graphics(paste0(imgwd,"g_release_OG", ".svg"))
...
filtered_data<-filter_data("release",
    typ = 9,
    life_stage = c("Y"))%>% dplyr::filter(is.na(eel_missvaluequal))|> filter(eel_value >0)
...
cou_textrelY <- get_country_table(table)
...
export_svg(subplot(g_release_Y,
        coeffy=1.4, # no need to increase the scale
        minyear=2008, 
        xmin=-Inf, 
        xmax= 1975, 
        ...
        ymaxbox=2.3), 
           ...
           width = 10,
           height = 5)
knitr::include_graphics(paste0(imgwd,"g_release_Y", ".svg"))
```
...
#print(g_releasekg_Y)
export_svg(g_releasekg_Y, 
           "g_releasekg_Y",
           width = 10,
           height = 5)
knitr::include_graphics(paste0(imgwd,"g_releasekg_Y", ".svg"))
...
export_svg(g_other_landingskg_Y, 
           "g_other_landingskg_Y",
           width = 10,
           height = 5)
knitr::include_graphics(paste0(imgwd,"g_other_landingskg_Y", ".svg"))
...
export_svg(g_releasekg_OG, 
           "g_releasekg_OG",
           width = 10,
           height = 5)
knitr::include_graphics(paste0(imgwd,"g_releasekg_OG", ".svg"))
...
These are reported as released silver eel (Figure \@ref(fig:releasekgSf)).
The quantity
of silver eels (S) released in `r CY-2` and
`r CY-1` is `r round(rel_CY2_S_t)` and
`r round(rel_CY1_S_t)` t respectively (number of countries reporting:
...
cou_textrelS <- get_country_table(table)
...
export_svg(g_release_S, 
           "g_release_S",
           width = 10,
           height = 5)
knitr::include_graphics(paste0(imgwd,"g_release_S", ".svg"))
```
...
#print(g_releasekg_S)
export_svg(g_releasekg_S, 
           "g_releasekg_S",
           width = 10,
           height = 5)
knitr::include_graphics(paste0(imgwd,"g_releasekg_S", ".svg"))
```
...
cou_textotherlandingsS <- get_country_table(table)
...
export_svg(g_other_landings_S, 
           "g_other_landings_S",
           width = 10,
           height = 5)
knitr::include_graphics(paste0(imgwd,"g_other_landings_S", ".svg"))
```
...
export_svg(g_other_landingskg_S, 
           "g_other_landingskg_S",
           width = 10,
           height = 5)
knitr::include_graphics(paste0(imgwd,"g_other_landingskg_S", ".svg"))
```
...
#print(g_release_all)
export_svg(g_release_all, 
           "g_release_all",
           width = 10,
           height = 5)
knitr::include_graphics(paste0(imgwd,"g_release_all", ".svg"))
```
...
export_svg(g_releasekg_all, 
           "g_release_all",
           width = 10,
           height = 5)
knitr::include_graphics(paste0(imgwd,"g_releasekg_all", ".svg"))
```
...
export_svg(g_other_landings_all, 
           "g_other_landings_all",
           width = 10,
           height = 5)
knitr::include_graphics(paste0(imgwd,"g_other_landings_all", ".svg"))
```
...
export_svg(g_other_landingskg_all, 
           "g_other_landingskg_all",
           width = 10,
           height = 5)
knitr::include_graphics(paste0(imgwd,"g_other_landingskg_all", ".svg"))
```
...
WGEEL considered that those data are not reliable (see Chapter 3 for further details).  
...
g_aquaculture <-  ggplot(aquaculture3) + 
    ...
    theme_bw()  
#print(g_aquaculture)
export_svg(g_aquaculture, 
           "g_aquaculture",
           width = 10,
           height = 5)
knitr::include_graphics(paste0(imgwd,"g_aquaculture", ".svg"))
```
...
# Annex 9: Trends in landings and releases


::: {custom-style="Table Caption"}
Table 1: Glass eel commercial fisheries landings (in tonnes) from 1984 to `r max(tableComLandG$Year)` reported by countries:
`r get_country_table(tableComLandG)`. Note that values shown as 0.0
are due to rounding and do not correspond to an actual 0.
:::                 
...
::: {custom-style="Table Caption"}
Table 2a:  Commercial fisheries landings (in tonnes) for yellow eel and silver eel from `r min(yearcom_land_YS)` to
`r max(yearcom_land_YS)` (part 1), reported by countries: `r cou_text1ComLandYS` (to be continued for other countries in next table). Note that values shown as 0.0
are due to rounding and do not correspond to an actual 0.
:::
...
\* Landings from France are incomplete between 2002 and 2010.
...
::: {custom-style="Table Caption"}
Table 2b:  Commercial fisheries landings (in tonnes) for yellow eel and silver eel from `r min(yearcom_land_YS)` to
`r max(yearcom_land_YS)` (part 2), reported by countries: `r cou_text2ComLandYS` (to be continued for other countries in next table). Note that values shown as 0.0
are due to rounding and do not correspond to an actual 0.
:::
...
::: {custom-style="Table Caption"}
Table 2c:  Commercial fisheries landings (in tonnes) for yellow eel and silver eel from `r min(yearcom_land_YS)` to
`r max(yearcom_land_YS)` (part 3), reported by countries: `r cou_text3ComLandYS`. The total 
column is the yearly sum for all countries included in tables 2a, 2b, and 2c and 
not just the countries listed in this particular table. Note that values shown as 0.0
are due to rounding and do not correspond to an actual 0.
:::
...
::: {custom-style="Table Caption"}
Table 3a: Recreational fisheries landings (in tonnes) for yellow eel and silver
eel from `r min(yearrec_land_Y)` to `r max(yearrec_land_Y)` (part 1), reported by 
countries: `r cou_text1rec_land_Y1` (to be continued for other countries in next table). Note that values shown as 0.0
are due to rounding and do not correspond to an actual 0.
:::
...
::: {custom-style="Table Caption"}
Table 3b: Recreational fisheries landings (in tonnes) for yellow eel and silver 
eel from `r min(yearrec_land_Y)` to `r max(yearrec_land_Y)` (part 2), reported 
by countries: `r cou_text2rec_land_Y`. The total 
column is the yearly sum for all countries included in tables 3a and 3b and 
not just the countries listed in this particular table. Note that values shown as 0.0
are due to rounding and do not correspond to an actual 0.
:::
...
::: {custom-style="Table Caption"}
Table 4: Raw recreational landings (tonnes) for glass eels  from `r min(tablerecLandG$Year)` 
to `r max(tablerecLandG$Year)` for `r get_country_table(tablerecLandG)`. Note that values shown as 0.0
are due to rounding and do not correspond to an actual 0.
:::
...
::: {custom-style="Table Caption"}
Table 5a:  Release of glass eel (G) and quarantined glass eel (QG) in millions 
from `r min(tableRelG$Year)` to `r max(tableRelG$Year)` reported by countries 
`r cou_text1release_G` (to be continued for other countries in next table). Note that values shown as 0.0
are due to rounding and do not correspond to an actual 0.
...
::: {custom-style="Table Caption"}
Table 5b:  Release of glass eel (G) and quarantined glass eel (QG) in millions 
from `r min(tableRelG$Year)` to `r max(tableRelG$Year)` reported by countries 
`r cou_text2release_G`. The total 
column is the yearly sum for all countries included in Tables 5a and 5b and 
not just the countries listed in this particular table. Note that values shown as 0.0
are due to rounding and do not correspond to an actual 0.
...
::: {custom-style="Table Caption"}
Table 6: Releases of yellow eel from `r min(tableRelY$Year)` to
`r max(tableRelY$Year)` in millions, reported by countries `r cou_textrelY`. Note that values shown as 0.0
are due to rounding and do not correspond to an actual 0.
...
::: {custom-style="Table Caption"}
Table 7: Releases of silver eel from `r min(tableRelS$Year)` to `r max(tableRelS$Year)`
 in millions, reported by countries `r cou_textrelS`. Note that values shown as 0.0
are due to rounding and do not correspond to an actual 0.
:::
...
::: {custom-style="Table Caption"}
Table 8: Releases of ongrown glass eel from `r min(tableRelOG$Year)` to `r max(tableRelOG$Year)`
in millions, reported by countries: `r cou_textreOG`. Note that values shown as 0.0
are due to rounding and do not correspond to an actual 0.
:::
...
::: {custom-style="Table Caption"}
Table 9: Other landings of glass eels in millions from `r min(tableotherLandingsG$Year)` to `r max(tableotherLandingsG$Year)` for `r cou_textotherLandingG`. Note that values shown as 0.0
are due to rounding and do not correspond to an actual 0.
:::
...
::: {custom-style="Table Caption"}
Table 10: Other landings of yellow eels (Y) in millions from `r min(tableotherLandingY$Year)` to `r max(tableotherLandingY$Year)` for `r cou_textotherlandingsY`. Note that values shown as 0.0
are due to rounding and do not correspond to an actual 0.
:::
...
::: {custom-style="Table Caption"}
Table 11: Other landings of silver eels (S) in millions from `r min(tableotherLandingsS$Year)` to `r max(tableotherLandingsS$Year)` for `r cou_textotherlandingsS`. Note that values shown as 0.0
are due to rounding and do not correspond to an actual 0.
:::
...
```{r save_graphs_and_tables,echo=FALSE, warning=FALSE, error=FALSE, message=FALSE, eval=FALSE}
