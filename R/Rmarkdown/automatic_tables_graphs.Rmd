...
- Eel aquaculture within the EU in `r CY-2`: `r round(aqua_CY2)`t, countries where data were reported: `r  paste(aqua_cou_CY2,sep="", collapse=", ")`
- Data is shown until 2022 due to delayed reporting. Mean aquaculture for the previous 5 years (`r CY-7`-`r CY-3`) within the EU: `r round(aqua_mean_CY3_CY7)`t
...
including reconstructed data to fill data gaps. In Spain, a new commercial fishery was 
created in the Basque Country after the closure of the recreational one.
...
g_reconstructed_landings <- ggplot(landings3) + 
    ...
    theme_bw()+
    geom_magnify(from = c(xmin = 2010, xmax = CY, ymin = 0, ymax = 100),
...
yet been incorporated in the analyses since it is currently under evaluation.
...
regulations (e.g. Ireland, UK, Portugal, Sweden, Norway, Spain) and some
countries restricted the season for recreational fishery in freshwater
(e.g. Finland, Denmark).


Recreational glass eel fishery landings decrease from over 1000 tonnes in the early
1980s, primarily from France, (Figure \@ref(fig:recLandGfig)to values below 5t 
...
reported - \@ref(fig:recLandYSfig)). France has provided estimation for all freshwater recreational
...
g_raw_Rlandings_G <- ggplot(filter(landings3,eel_value>0)) + 
    ...
    scale_fill_manual("Country", values=color_countries4plot)+
    theme_bw()+
    geom_magnify(from = c(xmin = 2010, xmax = CY-1, ymin = 0, ymax = 40),
...
the Netherlands. In addition, in Greece and France a certain percentage of
silver eels caught by the fishery, and therefore recorded as landings, are later
released in the Mediterranean outside the lagoons (30% of caught silver eels) .
These are reported as released silver eel (Figure \@ref(fig:releasekgSf)). Spain
has made some releases of silver eels from the Encanyissada lagoon. The quantity
...
WGEEL considered that those data are not relevant.  
