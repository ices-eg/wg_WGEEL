...
extract_data = function(table_dbname, quality = c(0,1,2,4), quality_check=TRUE, allow_NULL=FALSE)
{
  ...
  df_table = data.frame(
      table_dbname = c("landings", "aquaculture", "release", "other_landings", "b0", "bbest", "bcurrent", "bcurrent_without_stocking", "sigmaa", 
          "sigmaf", "sigmah", "potential_available_habitat","silver_eel_equivalents", "sigmafallcat", 
          "sigmahallcat", "precodata", "precodata_country","precodata_all","precodata_emu",
      ...
      qal_column=c(rep("eel_qal_id",19), "das_qal_id", "ser_qal_id", rep("meg_qal_id",2) , rep("mei_qal_id",2), "sai_qal_id",rep("",4))
  )
