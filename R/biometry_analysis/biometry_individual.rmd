...
```
```{r agelength, include=TRUE}

age_length<-total_individual %>% filter(!is.na(ageyear), !is.na(lengthmm), life_stage %in% c("Y","S"))
age_length<-total_individual %>% filter(!is.na(ageyear), !is.na(lengthmm), ageyear %in% c(0,1), life_stage %in% c("Y","S"))%>% group_by(country, ser_nameshort)%>% summarise(n=n())


ggplot(age_length, aes(y=lengthmm, x=ageyear, colour=country))+geom_point()+ facet_grid(vars(life_stage))
