...
```{r available_data, tab.cap = "Number of avaible data for each biometry metrics",include }
...
#TODO: explain (or better change) TRUE/FALSE/NA
## length frequency
...
```{r lengthdecriptivegraphs-month, include=TRUE, fig.cap = "Length distribution by month and by life stage. Point is the median, bar gives 5% and 95% quantile."}
## length by month
...
```
gridExtra::grid.arrange(
ggplot(total_lengthmmg, aes(x=as.factor(month),y=q50)) + geom_pointrange(aes(ymin = q5, ymax = q95)) +ggtitle("Glass eel")+ theme(axis.text.x = element_text(size = 12, angle = 45))+  xlab("Month")+ylab("Length (mm)")  + theme_classic() ,
ggplot(total_lengthmmy, aes(x=as.factor(month),y=q50)) + geom_pointrange(aes(ymin = q5, ymax = q95)) +ggtitle("Yellow eel")+ theme(axis.text.x = element_text(size = 12, angle = 90))+  xlab("Month")+ylab("Length (mm)")  + theme_classic() ,
ggplot(total_lengthmmy, aes(x=as.factor(month),y=q50)) + geom_pointrange(aes(ymin = q5, ymax = q95)) +ggtitle("Silver eel")+ theme(axis.text.x = element_text(size = 12, angle = 90))+ xlab("Month")+ylab("Length (mm)") + theme_classic() ,
nrow = 3
)
...
gridExtra::grid.arrange(
ggplot(total_weightg, aes(x=as.factor(year),y=q50)) + 
  geom_pointrange(aes(ymin = q5, ymax = q95)) +ggtitle("Glass eel")+ 
  theme(axis.text.x = element_text(size = 12, angle = 45))+ 
  xlab("year")+ylab("weight in g"),

ggplot(total_weighty, aes(x=as.factor(year),y=q50)) + 
  geom_pointrange(aes(ymin = q5, ymax = q95)) +
  ggtitle("Yellow eel") + 
  theme(axis.text.x = element_text(size = 12, angle = 90))+ xlab("year")+ylab("weight in g"),

ggplot(total_weights, aes(x=as.factor(year),y=q50)) + 
  geom_pointrange(aes(ymin = q5, ymax = q95)) +
  ggtitle("Silver eel")+ theme(axis.text.x = element_text(size = 12, angle = 90))+ xlab("year")+ylab("weight in g"),
...
ggplot(total_age) + aes(x = ageyear, y=country, fill = country, alpha=0.5)  + 
  ...
  scale_fill_manual("Country",values=color_countries[names(color_countries) %in% unique(total_age$country)], drop = TRUE)+
...
```
```{r agelengthy, fig.height=20/2.54, include=TRUE}
...
```{r agelengths, fig.height=20/2.54, include=TRUE}
...
```{r sexratio, include=TRUE}

sexratio<-total_individual %>%
  mutate(sex=ifelse(is.na(female_proportion), "unknown", ifelse(female_proportion==1, "F", "M")))%>% 
  filter(life_stage %in% c("Y","YS","S"), sex %in% c("F","M"))
...
p + geom_boxplot()+ scale_fill_manual(values=color_countries[names(color_countries) %in% unique(reg_series_data$country)], drop = TRUE)+geom_hline(yintercept =100) + facet_grid(vars(source),vars(life_stage))+
	scale_color_discrete("Life stage") + xlab("Country") + ylab("Wr")
...
```{r glassmaplength,echo=TRUE}

plot_map_bio_emu("length","G")
...
The length of monitored standing stock yeelow eel appear to increase with the distance to Gibraltar. This is confirmed by the Kendall correlation test (tau=

. There are not enougth time sex disaggregated data to detect sex-specific length-pattern.

```{r yellowmaplength,echo=TRUE}
...
Similarly to length, a pattern is visible for weight of monitored standing stock yeelow eel. This is confirmed by the Kendall correlation test (tau=
