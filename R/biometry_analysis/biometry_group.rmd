...
if(params$load_data)
	...
	biometry_group_data_series=dbGetQuery(con_wgeel,"with sel as(SELECT d.*, t_group.*, mty_name FROM datawg.t_metricgroupseries_megser d, datawg.t_groupseries_grser t_group, ref.tr_metrictype_mty WHERE mty_id=meg_mty_id AND meg_gr_id = gr_id AND (meg_qal_id IS NULL OR meg_qal_id IN (1,2,4))) select sel.*, ser_nameshort, ser_x,ser_y,ser_emu_nameshort, ser_hty_code,ser_lfs_code from sel, datawg.t_series_ser where ser_id=grser_ser_id")
	
	biometry_group_data_sampling=dbGetQuery(con_wgeel,"with sel as(SELECT d.*, t_group.*, mty_name FROM datawg.t_metricgroupsamp_megsa d, datawg.t_groupsamp_grsa t_group, ref.tr_metrictype_mty WHERE mty_id=meg_mty_id AND meg_gr_id = gr_id AND (meg_qal_id IS NULL OR meg_qal_id IN (1,2,4))) 
	select * from sel, datawg.t_samplinginfo_sai where sai_id=grsa_sai_id")
	
	
	total_group = biometry_group_data_series %>% 
		select(ser_id = grser_ser_id, year = gr_year,  gr_id, number = gr_number, comment = gr_comment, quality = meg_qal_id, mty_name, meg_value,ser_nameshort, ser_x,ser_y,ser_emu_nameshort, ser_hty_code,ser_lfs_code) %>%
		pivot_wider(names_from = mty_name, values_from = meg_value)
	
	total_group$ser_lfs_code=factor(as.character(total_group$ser_lfs_code),levels=c("G","GY","Y","S"))
	
	gibraltar = st_transform(st_sfc(st_point(c(-5.605465,36.008260)), crs=4326),3035)
	data2 = st_transform(st_as_sf(total_group,coords=c("ser_x","ser_y"),crs=4326),3035)
	total_group$distance = st_distance(data2, gibraltar, by_element=TRUE)/1000
	rm(data2, gibraltar)
	
	dbDisconnect(con_wgeel)
#save(total_group, biometry_group_data_series, biometry_group_data_series, file = "group_biom.RData")
} else {
	load("group_biom.RData")
	warnings("The data was loaded with a RData file, please turn params `load_data` to TRUE if you want uptodate data")
}

 my_map = get_background_map(transparent = TRUE)
...
ggplot(total_group,aes(x=bio_length,y=bio_weight))+geom_point()+
  facet_wrap(.~ser_lfs_code,scale="free")+
  theme_bw()+ggtitle("mixed")+xlab("Length (mm)")+ylab("Weight (g)")
ggplot(total_group,aes(x=bio_length_m,y=bio_weight_m))+geom_point()+
  facet_wrap(.~ser_lfs_code,scale="free")+
  theme_bw()+ggtitle("male")+xlab("Length (mm)")+ylab("Weight (g)")
ggplot(total_group,aes(x=bio_length_f,y=bio_weight_f))+geom_point()+
  facet_wrap(.~ser_lfs_code,scale="free")+
  theme_bw()+ggtitle("female")+xlab("Length (mm)")+ylab("Weight (g)")

ser=total_group$bis_ser_id[which(total_group$bio_length>1250)]
...
stats_data= total_group %>% group_by(ser_emu_nameshort,ser_hty_code,ser_lfs_code) %>%
...
mean_biom_hty_emuyear <- total_group %>%
...
For the spatial analysis, we only considered ser_hty_code x EMU x lfs_code where at leat 5 total_group points were available (they may come from different time series or years).
...
plot_map_bio_emu = function(var,stage,sex=NULL){
  ...
  ggmap(my_map) + 
    geom_point(total_group=total_group,
...
autofit(flextable(stats_data %>%
          ...
          filter(rowSums(select(.,starts_with("n_")))>0)))
```
Too few total_group were correlected regarding sex to draw any conclusions regarding differences in sex-ratios among locations.
...
autofit(flextable(stats_data %>%
          ...
          filter(rowSums(select(.,starts_with("n_")))>0)))
```
For silver eels, there is few available biometry total_group, therefore, so it is difficult to detect any spatial patterns.
...
ggmap(my_map)+
  geom_point(total_group=allometry,
...
for (s in c("GY","Y","S")){
  print(ggmap(my_map)+
          geom_point(total_group=subset(allometry,Stage==s),
             aes(col=slopes,x=Ser_x,y=Ser_y))+
          geom_point(total_group=subset(allometry,Stage==s),
...
# Temporal trends
In this section, we explore the existence of temporal trends in biometry. For that purpose, we computed average biometry per EMU, habitat and year. Then we carry out Mann Kendall trend tests to detect time series with significant monotonic trend. We only keep EMUxHTY that have total_group for at least 5 years.
...
table_temporal_trend = function(var,stage,sex=NULL){
  total_group=mean_biom_hty_emuyear %>%
    filter(ser_lfs_code==stage)
  ...
  tmp=NA
  if(nrow(kept)>0){
    tmp=inner_join(kept,total_group) %>%
    group_by(ser_emu_nameshort,ser_hty_code) %>%
...
plot_temporal_trend = function(var,stage,trends,sex=NULL){
  ...
  ggplot(total_group,aes_string(x="bio_year",y=var_name))+
...
plot_ge_trend = function(var,trends){
  total_group=inner_join(total_group,kept)
