...
detect_missing_biom_morta <- function(cou="FR",
    ...
    minyear=2007,
    maxyear=2023, #maxyear corresponds to the current year where we have to fill data
    con,
    datasource="dc_2024") {
  ...
  missing_comb <- base::merge(missing_comb,complete[,c("eel_year","eel_emu_nameshort","eel_value","eel_typ_id")],
      all.x=TRUE)
  eel_typ_name_table <- dbGetQuery(con,"select typ_id eel_typ_id,typ_name as eel_typ_name  from ref.tr_typeseries_typ")
  missing_comb$eel_typ_id <- as.integer(missing_comb$eel_typ_id)
  
  missing_comb <- dplyr::inner_join(missing_comb,eel_typ_name_table)
