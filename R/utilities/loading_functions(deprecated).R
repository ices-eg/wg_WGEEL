...
# check column names
	if (!all(colnames(data_xls)%in%
		...
		cat(str_c("problem in column names",            
						...
						"file =",
						file,"\n")) 
	data_xls <-set_correct_type(data_xls)
# to avoid problems with logical in empty column we have to fix the column names
	if (nrow(data_xls)>0) {
...
# the dataset ices_division should have been loaded there
		...
		###### eel_qal_id ############## 
# eel_qual_id are now set by data integrators in new and duplicated tables		
#		data_error= rbind(data_error, check_missing(dataset=data_xls,
#						column="eel_qal_id",
#						country=country))
#		
...
############################
# Set column type for missing value, otherwise might crash later when trying to copy to the database
###########################
set_correct_type <- function(data){
	if ("eel_value_number"%in%colnames(data)){
		data$eel_value_number <- as.numeric(data$eel_value_number)	
		data$eel_value_kg <- as.numeric(data$eel_value_kg)	
	} else {
		data$eel_value <- as.numeric(data$eel_value)	
	}
	data$eel_missvaluequal <- as.character(data$eel_missvaluequal)
	data$eel_emu_nameshort <- as.character(data$eel_emu_nameshort)
	data$eel_cou_code <- as.character(data$eel_cou_code)
	data$eel_lfs_code <- as.character(data$eel_lfs_code)
	data$eel_hty_code <- as.character(data$eel_hty_code)
	data$eel_area_division <- as.character(data$eel_area_division)
	data$eel_qal_id  <- as.integer(data$eel_qal_id)
	data$eel_qal_comment  <- as.character(data$eel_qal_comment)
	data$eel_comment  <- as.character(data$eel_comment)
	data$eel_datasource <-  as.character(data$eel_datasource)
	return(data)
}
