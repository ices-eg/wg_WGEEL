#' deletion of group metric (annex 1-3 and 10)
#'
...
writedeletedgroupmetricUI <- function(id,title){
  ns <- NS(id)
  tagList(useShinyjs(),
          h2(title),
          fluidRow(
...
#' @param type series or dcf
#'
#' @return loaded data and file type


writedeletedgroupmetricServer <- function(id,globaldata,loaded_data, type = "series"){
  moduleServer(id,
               function(input, output, session) {
                 ...
                 # deleted group metrics sampling  --------------------------------------------------------							
                 
                 observeEvent(input$delete_group_metrics_button, tryCatch({
                   
                   step_filepath_deleted_group_metrics <- reactive({
                     inFile <- isolate(input$xl_deleted_group_metrics)     
                     if (is.null(inFile)){        return(NULL)
                     } else {
                       data$path_step_deleted_group_metrics <- inFile$datapath #path to a temp file             
                     }
                   })
                   
                   step_load_data <- function() {
                     path <- isolate(step_filepath_deleted_group_metrics())
                     if (is.null(data$path_step_deleted_group_metrics)) 
                       return(NULL)
                     rls <- delete_group_metrics(path, type=type)
                     message <- rls$message
                   ...
                   output$textoutput_step <- renderText({
                     ...
                     if (is.null(data$path_step_deleted_group_metrics)) "please select a dataset" else {                                      
