...
importdcfstep1UI <- function(id){
  ...
  tagList(useShinyjs(),
          ...
          fluidRow(
            ...
            box(tabsetPanel(id=ns("dcfstep1panel"), selected = "SAMPLINGS",
...
importdcfstep1Server <- function(id,globaldata,loaded_data_dcf,globaldcfpanel){
  moduleServer(id,
               function(input, output, session) {
                 mydcfpanel <- reactiveValues(dcfpanel="SERIES")
                 observe({mydcfpanel <- reactiveValues(dcfpanel=globaldcfpanel$dcfpanel)
                 updateTabsetPanel(session, "dcfstep1panel",
                                   selected = globaldcfpanel$dcfpanel
                 )})
                 observeEvent(input$dcfstep1panel,{
                   if (isolate(input$dcfstep1panel) != isolate(mydcfpanel$dcfpanel))
                     mydcfpanel$dcfpanel <- input$dcfstep1panel
                 })
                 observe({
                   loaded_data_dcf$res
                   tryCatch({
