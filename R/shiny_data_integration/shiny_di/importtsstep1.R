...
importtsstep1UI <- function(id){
  ...
  tagList(useShinyjs(),
          ...
          fluidRow(
            ...
            box(tabsetPanel(id=ns("tsstep1panel"), selected = "SERIES",
                            tabPanel("SERIES",value="SERIES",fluidRow(
                              column(width=6,
                                     h3("new series"),
                            ...
                            tabPanel("DATASERIES", value="DATASERIES",fluidRow(
                              column(width=6,h3("new data"),
                              ...
                              ,column(width=6,
                                      h3("modified data : what changed for new_data and updated_data ?"),	
                                      DT::dataTableOutput(ns("dt_highlight_change_dataseries")),
                                      h3("deleted data"),
                                      DT::dataTableOutput(ns("dt_deleted_dataseries"))
                              ))),
                            tabPanel("GROUP METRICS",value="GROUP METRICS", fluidRow(
                              column(width=6,h3("new group metrics"),
                            ...
                            tabPanel("INDIVIDUAL METRICS", value="INDIVIDUAL METRICS",fluidRow(
                              column(width=6,h3("new individual metrics"),
...
importtsstep1Server <- function(id,globaldata,loaded_data_ts, globaltspanel){
  moduleServer(id,
               function(input, output, session) {
                 mytspanel <- reactiveValues(tspanel="SERIES")
                 observe({mytspanel <- reactiveValues(tspanel=globaltspanel$tspanel)
                 
                 updateTabsetPanel(session, "tsstep1panel",
                                   selected = globaltspanel$tspanel
                 )})
                 observeEvent(input$tsstep1panel,{
                   if (isolate(input$tsstep1panel) != isolate(mytspanel$tspanel))
                     mytspanel$tspanel <- input$tsstep1panel
                 ...
                 observeEvent(input$check_duplicate_button_ts, {
                   #shinyCatch({  
                     ...
                     if (nrow(updated_individual_metrics)>0){
                       ...
                       if (nrow(new_individual_metrics)>0){
                         if (nrow(list_comp_updated_individual_metrics$new)>0){
                           mxn <- max(list_comp_updated_individual_metrics$new$id, na.rm=TRUE)
                         }   else {
                           mxm <- 0
                         }
                         if (nrow(list_comp_updated_individual_metrics$modified)>0){
                           mxm <- max(list_comp_updated_individual_metrics$modified$id, na.rm=TRUE)
                         } else {
                           mxm <- 0
                         }
