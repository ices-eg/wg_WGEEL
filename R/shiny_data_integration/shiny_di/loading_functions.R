...
load_dcf<-function(path,datasource){
	...
	#---------------------- all_other_sheets ---------------------------------------------
	fn_check_gr_ind <- function(sheet, columns,col_types, nbcol){
		data_xls <- read_excel(
				path=path,
				sheet=sheet,
				col_types=col_types,
	...
	col_types=list(
			...
			c("numeric", "text", "text",	"numeric",	"text", "numeric", "text",  "date", "text",rep("numeric", 24)),
			c("text",	"text",	"date",	"text",	"numeric",	"numeric", rep("numeric", 18),"text"), # fi_comment is at the end :-(
			c("numeric","text",	"text", "date",	 "text", "numeric",	"numeric", "text",  "date",	"text", rep("numeric", 18)),
			c("numeric","text",	"text", "date",		 "text", "numeric",	"numeric", "text",  "date",	"text", rep("numeric", 18))
			# if fi_year is there :
	...
	#stopifnot(all.equal(unlist(nbcol), sapply(col_types,length)))
	res <- purrr::pmap(list(sheet,columns,col_types, nbcol), fn_check_gr_ind)
	data_error <- 	lapply(res,function(X)X$error) %>% bind_rows()
