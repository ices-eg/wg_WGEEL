    ...
    if (ncol(data_xls)!=ifelse(sheet =="new_data",13,13)) {
      ...
      if (!all(colnames(data_xls)%in%
        ...
        if (sheet != "new_data"){
          ...
          release_tot <- data_xls[, c("eel_typ_name", "eel_year","eel_value","eel_missvaluequal","eel_emu_nameshort",
                                      "eel_cou_code", "eel_lfs_code", "eel_hty_code","eel_area_division",
                                      "eel_comment","eel_datasource")]
        }
...
load_biomass<-function(path,datasource){
  ...
  output <- lapply(c("new_data","updated_data","deleted_data"),function(sheet){
        ...
        if (nrow(data_xls)>0){
          ...
          # fix table format (lfs, hty and remove biom_ from colnames) replace NP with -1
          data_xls$eel_lfs_code <- 'S' #always S
          data_xls$eel_hty_code <- 'AL' #always AL
          data_xls <- data_xls %>% 
          ...
          data_xls$eel_area_division <- as.vector(rep(NA,nrow(data_xls)),"character")      
        }
        return(list(data=data_xls,error=data_error))
      })
  shinybusy::remove_modal_spinner()
...
load_mortality_rates<-function(path,datasource){
  ...
  output <- lapply(c("new_data","updated_data","deleted_data"),function(sheet){
    ...
    if (nrow(data_xls)>0){
      ...
      if (ncol(data_xls)!=18 & sheet %in% c("deleted_data","updated_data")) cat(str_c("updated or deleted_data: number column wrong should be  18 in template for country",country,"\n"))
      ...
      data_xls $eel_hty_code <- 'AL' #always AL
      data_xls <- data_xls %>% 
        rename_with(function(x) tolower(gsub("mort_", "", x)), starts_with("mort_")) %>%
        mutate_at(vars(starts_with("perc_")), function(x) as.numeric(ifelse(x=='NP','-1',x)))
      data_xls$eel_area_division <- as.vector(rep(NA,nrow(data_xls)),"character")
      if (nrow(data_error)>0) {
        data_error$sheet <- sheet
  ...
  shinyCatch({
    ...
    columns <- list(
      ...
      setdiff(c("ser_nameshort",	 "gr_year",	"gr_number", "gr_comment",  "lengthmm",	"weightg",	"ageyear",	"female_proportion","differentiated_proportion",
        ...
        "method_anguillicola_(1=stereomicroscope,0=visual_obs)"),contaminant),		
      setdiff(c("gr_id","ser_nameshort", "gr_year",	"gr_number", "gr_comment", "gr_lastupdate", "gr_dts_datasource", "lengthmm",	"weightg",	"ageyear",	"female_proportion","differentiated_proportion",
        "m_mean_lengthmm","m_mean_weightg","m_mean_ageyear","f_mean_lengthmm","f_mean_weightg","f_mean_age",
        "anguillicola_proportion",	"anguillicola_intensity",	"muscle_lipid_fatmeter_perc", "muscle_lipid_gravimeter_perc",	"sum_6_pcb", "teq",
      ...
      setdiff(c("fi_id","ser_nameshort","fi_id_cou","fi_date", "fi_year","fi_lfs_code", "fi_comment", "fi_lastupdate",	"fi_dts_datasource",
...
load_dcf<-function(path,datasource, use_contaminant_data = FALSE, contaminant){
  ...
  fn_check_gr_ind <- function(sheet, columns){
    ...
    #fi_year is not compulsory in the template (at least in dc2022)
    if (column_year == "fi_year" & !"fi_year" %in% names(data_xls))
      column_year = NULL
    if (!is.null(column_year)){
      data_error <- bind_rows(data_error, check_missing(
        dataset = data_xls,
  ...
  columns <- list(
    ...
    setdiff(c("gr_id", "sai_name", 	"gr_year",	"grsa_lfs_code", "gr_number",  "gr_lastupdate", "gr_dts_datasource", "lengthmm",	"weightg",	"ageyear",	"female_proportion","method_sex_(1=visual,0=use_length)"
              , "differentiated_proportion",  "m_mean_lengthmm","m_mean_weightg","m_mean_ageyear","f_mean_lengthmm","f_mean_weightg","f_mean_age","g_in_gy_proportion",	"s_in_ys_proportion",	
      "anguillicola_proportion",	"anguillicola_intensity",	"method_anguillicola_(1=stereomicroscope,0=visual_obs)",	"muscle_lipid_fatmeter_perc", "muscle_lipid_gravimeter_perc",	"sum_6_pcb", "teq",	"evex_proportion",	
      "hva_proportion",	"pb",	"hg",	"cd", "gr_comment"),contaminant),
      setdiff(c("gr_id", "sai_name", 	"gr_year",	"grsa_lfs_code", "gr_number", "gr_lastupdate", "gr_dts_datasource","lengthmm",	"weightg",	"ageyear",	"female_proportion","method_sex_(1=visual,0=use_length)",
      ...
      "anguillicola_presence_(1=present,0=absent)",	"anguillicola_intensity",	"method_anguillicola_(1=stereomicroscope,0=visual_obs)",	"muscle_lipid_fatmeter_perc", "muscle_lipid_gravimeter_perc",	"sum_6_pcb", "teq",
      "evex_presence_(1=present,0=absent)","hva_presence_(1=present,0=absent)",	"pb",	"hg",	"cd"),contaminant),
      setdiff(c("fi_id","fi_id_cou","sai_name", "fi_date",	"fi_year",	 "fi_lfs_code", "fisa_x_4326",	"fisa_y_4326", "fi_comment",  "fi_lastupdate",	"fi_dts_datasource", 
      "lengthmm",	"weightg",	"ageyear",	"eye_diam_meanmm", "pectoral_lengthmm",
      "is_female_(1=female,0=male)","method_sex_(1=visual,0=use_length)","is_differentiated_(1=differentiated,0_undifferentiated)",
      "anguillicola_presence_(1=present,0=absent)",	"anguillicola_intensity",	"method_anguillicola_(1=stereomicroscope,0=visual_obs)",	"muscle_lipid_fatmeter_perc", "muscle_lipid_gravimeter_perc",	"sum_6_pcb", "teq",
      "evex_presence_(1=present,0=absent)","hva_presence_(1=present,0=absent)",	"pb",	"hg",	"cd"),contaminant),
