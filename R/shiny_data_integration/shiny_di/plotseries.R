...
plotseriesUI <- function(id){
  ...
  tagList(useShinyjs(),
          ...
          fluidRow(
            column(width=6,
                   plotOutput(ns("duplicated_ggplot_ts"),
...
plotseriesServer <- function(id,globaldata){
  moduleServer(id,
               function(input, output, session) {
                 ...
                 output$duplicated_ggplot_ts <- renderPlot({
                   validate(need(globaldata$connectOK,"No connection"))
                 ...
                 observeEvent(input$duplicated_ggplot_ts_click,  tryCatch({
                   ...
                   year_selected = round(input$duplicated_ggplot_ts_click$x)  
                   datagr <- rvs$datagr
