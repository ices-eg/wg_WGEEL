...
% DO REPLACE FOR INSTANCE F:/workspace/wgeeldata/sweave/2016/table by F:/workspace/wgeeldata/sweave/2017/table
%%%%%%%%%%%%%%%%%%%%%%%%%% END OF THE PREAMBLE  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{document}
...
# TODO list
##########################
# trigger on update for dataseries
# fill in ser_sam_id (currently void ?)
...
wger<-chnames(wger,
		...
		c("id","value","year","nam","area_division","lifestage"))
############################################################################
...
wger[wger$area%in%c("Atlantic Ocean","Baltic","British Isle","Mediterranean Sea"),"area"]<-"Elsewhere Europe"
wger$area<-as.factor(wger$area)
# We will also need this for summary tables per recruitment site, here we go straight to 
# the result
R_stations[,"area"]<-NA
R_stations$area[R_stations$f_subarea%in%'27.4']<-"North Sea"
...
wgerinit<-wger
vv$nb_series_init<-length(unique(wger$nam)) # this is the true number at the beginning
...
# length(std_nam) 
nam<-unique(wger$nam[order(wger$nam)])
# length(nam) 
unused_series_2000_2009 = nam[!nam%in%std_nam] # "Vida" "YFS1"
...
R_stations[R_stations$rec_nameshort %in% unused_series, "unused_2000_2009"] = TRUE
#ex(std_nam)
# Inag and Maig left out from the analysis 
mean_site=data.frame(mean_2000_2009=as.numeric(tapply(mdata$value,mdata$ser_id,mean)))
mean_site$nam=tapply(mdata$nam,mdata$ser_id,function(X)unique(X))
wger=merge(wger,mean_site,by="nam",all.x=TRUE,all.y=FALSE) # here we loose the two stations Inag and Maig and also Frémur
...
nam<-unique(wger$nam[order(wger$nam)])
# length(nam) #49
unused_series_1979_1994 = nam[!nam%in%std_nam] # "Bres" "Fre"  "Inag" "Klit" "Maig" "Nors" "Sle"  "Vac"
...
mean_site=data.frame(mean_1979_1994=as.numeric(tapply(mdata$value,mdata$ser_id,mean)))
mean_site$nam=tapply(mdata$nam,mdata$ser_id,function(X)unique(X))
wger=merge(wger,mean_site,by="nam",all.x=TRUE,all.y=FALSE) 
...
mean_site=data.frame(mean=as.numeric(tapply(wger$value,wger$ser_id,mean)))
mean_site$nam=tapply(wger$nam,wger$ser_id,function(X)unique(X))
wger=merge(wger,mean_site,by="nam",all.x=TRUE,all.y=FALSE) 
wger$value_std=wger$value/wger$mean

glass_eel_yoy=wger[wger$lifestage!="yellow eel" & wger$year>1959,] #glass eel and yoy
older=wger[wger$lifestage=="yellow eel" & wger$year>1959,] #> 1 year / 37 = Parteen # &wger$ser_id!=37

##########################################################################
# Some statistics for later use, nb of year per series
#########################################################################

nb_year=colSums(ftable(xtabs(formula = value_std_1979_1994~year+nam,data=wger))>0)
names(nb_year)=colnames(xtabs(formula = value_std_1979_1994~year+nam,data=wger))

################################################################
# Building dataset for first graphs standardized to 1979-1994
# these are the graphs that were fisst built historically by the wgeel
################################################################

wger_coun=wger#subset(wger,wger$name%in%names(nb_year))
wger_coun$nam=as.factor(as.character(wger_coun$nam))
#wger_coun$sampling_type=as.character(wger_coun$sampling_type)
wger_coun<-wger_coun[!is.na(wger_coun$value_std_1979_1994),]
#unique(wger_coun$name)
datageel=subset(wger_coun,wger_coun$lifestage!="yellow eel")
datayellow=subset(wger_coun,wger_coun$lifestage=="yellow eel")
###############################################################
...
save(wger_coun,file=paste(datawd,"wger_coun.Rdata",sep="\\"))
...
last_year<-tapply(wger$year,wger$nam,function(X) max(X))
#stations updated to",CY
R_stations$areashort<-"EE"
R_stations$areashort[R_stations$area=="North Sea"]<-"EE"
series_CY<-R_stations[R_stations$ser_nameshort%in%names(last_year[last_year==CY]),c("ser_nameshort","ser_namelong","cou_country","Organisation","ser_lfs_code","areashort","ser_area_division")]
...
series_CYm1<-R_stations[R_stations$rec_nameshort%in%names(last_year[last_year==CY-1]),c("ser_nameshort","ser_namelong","cou_country","Organisation","ser_lfs_code","areashort","ser_area_division")]
vv$nCYm1<-nrow(series_CYm1) # number series updated last year only (and not this year)
vv$nCYm1g<-nrow(series_CYm1[grepl("G",series_CY$ser_lfs_code),]) # same for glass eel 
...
d_lost_ones<-data.frame("nam"=names(lost_ones),"year"=lost_ones) # data frame
series_lost<-merge(
		R_stations[R_stations$ser_nameshort%in%names(lost_ones),c(c("ser_nameshort","ser_namelong","cou_country","Organisation","ser_lfs_code","areashort","ser_area_division"))],
		d_lost_ones,
		by.y="nam",by.x="ser_nameshort")
...
#------------------------------------------------------
colnames(series_CY)<-str_c("\\scshape{",c("Code","Name","Country","Organisation","Stage","Area","Division"),"}")

xseries_CY <- xtable(x = series_CY,
		label = "table_seriesCY",
		caption = str_c("Series updated to ",CY),
		align=c("p{0cm}","p{1.3cm}","p{6cm}","p{2cm}","p{2.5cm}","p{1.2cm}","p{1.2cm}","p{1.2cm}"))
...
#------------------------------------------------------
colnames(series_CYm1)<-str_c("\\scshape{",c("Code","Name","Country","Organisation","Stage","Area","Division"),"}")
...
#------------------------------------------------------
colnames(series_lost)<-str_c("\\scshape{",c("Code","Name","Country","Organisation","Stage","Area","Division"),"}")
...
query=new("RequeteODBC")
query@sql="select * from ts.t_data_dat join ts.t_location_loc on ser_id=dat_ser_id 
		join ts.t_recruitment_rec on rec_ser_id=ser_id
...
stopifnot(length(unique(dat$rec_order))== length(unique(dat$dat_ser_id)))
...
###################################################
scal=data.frame("year"=as.numeric(names(tapply(wger_coun$value_std_1979_1994,wger_coun$year,min))),
		"data_std"=NA,
		"name"=NA,
		"lifestage"=NA,
		"ymin"= tapply(wger_coun$value_std_1979_1994,wger_coun$year,min),
		"ymax"=tapply(wger_coun$value_std_1979_1994,wger_coun$year,max),
		"mean"=tapply(wger_coun$value_std_1979_1994,wger_coun$year,mean),
		"geomean"=unlist(tapply(wger_coun$value_std_1979_1994,wger_coun$year,geomean)))
li_cl_boot=tapply(wger_coun$value_std_1979_1994,wger_coun$year,smean.cl.boot)
...
scalgeel=data.frame(
		"year"=as.numeric(names(tapply(datageel$value_std_1979_1994,datageel$year,min))),
		"data_std"=NA,
		"name"=NA,
		"lifestage"=NA,
		"ymin"= tapply(datageel$value_std_1979_1994,datageel$year,min),
		"ymax"=	tapply(datageel$value_std_1979_1994,datageel$year,max),	
		"mean"=tapply(datageel$value_std_1979_1994,datageel$year,mean),
		"geomean"=unlist(tapply(datageel$value_std_1979_1994,datageel$year,geomean))
)

scalyellow=data.frame(
		"year"=as.numeric(names(tapply(datayellow$value_std_1979_1994,datayellow$year,min))),
		"data_std"=NA,
		"name"=NA,
		"lifestage"=NA,
		"ymin"= tapply(datayellow$value_std_1979_1994,datayellow$year,min),
		"ymax"=	tapply(datayellow$value_std_1979_1994,datayellow$year,max),	
		"mean"=tapply(datayellow$value_std_1979_1994,datayellow$year,mean),
		"geomean"=unlist(tapply(datayellow$value_std_1979_1994,datayellow$year,geomean)))
scaldata=wger_coun[,c("year","data_std","name","lifestage")]
################################################################
# With scaling 2000-2010
##############################################################"
scal_2000_2009=data.frame("year"=as.numeric(names(tapply(wger_coun$value_std_2000_2009,wger_coun$year,min,na.rm=TRUE))),
		"data_std"=NA,
		"name"=NA,
		"lifestage"=NA,
		"ymin"= tapply(wger_coun$value_std_2000_2009,wger_coun$year,min,na.rm=TRUE),
		"ymax"=tapply(wger_coun$value_std_2000_2009,wger_coun$year,max,na.rm=TRUE),
		"mean"=tapply(wger_coun$value_std_2000_2009,wger_coun$year,mean,na.rm=TRUE),
		"geomean"=unlist(tapply(wger_coun$value_std_2000_2009,wger_coun$year,geomean)))
li_cl_boot=tapply(wger_coun$value_std_2000_2009,wger_coun$year,smean.cl.boot)
...
scalyellow_2000_2009=data.frame(
		"year"=as.numeric(names(tapply(datayellow$value_std_2000_2009,datayellow$year,min))),
		"data_std"=NA,
		"name"=NA,
		"lifestage"=NA,
		"ymin"= tapply(datayellow$value_std_2000_2009,datayellow$year,min),
		"ymax"=	tapply(datayellow$value_std_2000_2009,datayellow$year,max),	
		"mean"=tapply(datayellow$value_std_2000_2009,datayellow$year,mean),
		"geomean"=unlist(tapply(datayellow$value_std_2000_2009,datayellow$year,geomean)))
scaldata_2000_2009=wger_coun[,c("year","data_std_2000_2009","name","lifestage")]
...
#tapply(wger_coun$value_std,wger_coun$year,mean_cl_boot)

#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
# full graph
# for memory, it is no longer used
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#g<-ggplot(wger_coun,aes(x=year, y=data_std))
...
#year_only=glm(data_std_1979_1994~year_f+as.factor(das_ser_id),data=glass_eel_yoy,family=Gamma(link=log),subset=glass_eel_yoy$value>0 & glass_eel_yoy$year>1949 ,maxit=300)
#summary(year_only)
...
model_ge_area=glm(data_std~year_f:area+nam,data=glass_eel_yoy[glass_eel_yoy$value>0 & glass_eel_yoy$year>1959,],
...
vv$modelge$value_excluded_zero<-glass_eel_yoy[glass_eel_yoy$value==0,c("data_std","nam","year","lifestage","dat_comment","geo")]

model_older=glm(data_std~year_f+as.factor(nam),data=older,family=Gamma(link=log),subset=older$value>0 & older$year>1949  ,maxit=300)
vv$modelolder<-list()
...
vv$modelolder$value_excluded_zero<-older[older$value==0,c("data_std","nam","year","lifestage","dat_comment","geo")]
...
#vil<-wger[wger$ser_id==17,c("data_std","year")]
...
#vil[16:21,"data_std"]<-c(5,4,9,12,10,8)*vil$value_std[vil$year==1971]/44
...
library(lme4)
library(MASS)
glass_eel_yoy$ldata_std=log(glass_eel_yoy$value_std)
...
model_ge_area2=glm(data_std~year_f:area+nam,data=glass_eel_yoy2[glass_eel_yoy2$value>0 ,],
		family=Gamma(link=log), maxit=300)
...
unique(older$nam)
model_older=glm(data_std~year_f+as.factor(nam),data=older,family=Gamma(link=log),subset=older$value>0 &
...
\Sexpr{nrow(vv$modelolder$value_excluded_zero)} for the yellow eel model.\\
