...
xseries_lost <- xtable(x = series_lost,
		...
		align=c("p{0cm}","p{1.3cm}","p{6cm}","p{2cm}","p{2.5cm}","p{1.2cm}","p{1.2cm}","p{1.2cm}","p{1.2cm"))
print(xseries_lost, 
...
##		caption.placement = "top",
##		NA.string = ".")
printstatseries<-statseries[,c(1,3,4,5,6,7,8,9,10)]
...
printstatseries<-merge(printstatseries,column_to_import,by.x="name",by.y="ser_nameshort")
printstatseries<-printstatseries[,c(1,10,2:9)]
...
%************************************************************************************
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
<<tableswgeel,echo=FALSE, eval=FALSE,result=hide>>=
...
scaldata=wger[,c("year","value_std_1979_1994","nam","lifestage")]
################################################################
...
scaldata_2000_2009=wger[,c("year","value_std_2000_2009","nam","lifestage")]
...
g<-ggplot(scaldata)
g1<-g+geom_point(aes(x=year, y=value_std_1979_1994,colour=nam,shape=lifestage),size=1.5)+
		geom_line(aes(x=year, y=value_std_1979_1994,colour=nam,lty=lifestage),size=0.4)+
...
g<-ggplot(scaldata)
figure3black<-g+geom_point(aes(x=year, y=value_std_1979_1994,colour=nam,shape=lifestage),size=1.5,show.legend=FALSE)+
		geom_line(aes(x=year, y=value_std_1979_1994,colour=nam,lty=lifestage),size=0.4,show.legend=FALSE)+
		#ylab("scaled 1979-1994 values")+
...
##########################################
#g<-ggplot(scaldata)
#g+geom_point(aes(x=year, y=value_std,colour=nam,shape=lifestage),size=1.5,legend=FALSE)+
#		geom_line(aes(x=year, y=value_std,colour=nam,lty=lifestage),size=0.4,legend=FALSE)+
#		opts(title="Tendance du recrutement Europeen")+
#		#ylab("scaled 1979-1994 values")+
#		geom_pointrange(data=bootscal,aes(x=year,y=mean,ymin=ymin,ymax=ymax),size=0.5,colour="beige")+
#		scale_y_log10(nam="% de 1979-1994",limits=c(0.005,10),breaks=c(0.01,0.1,1,10),labels=c("1%","10%","100%","1000%"))+
#		scale_x_continuous(nam="Annee",breaks=c(1930,1950,1970,1980,1990,2000,2010),
...
#!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
#g<-ggplot(wger,aes(x=year, y=value_std))
#g+geom_point(aes(colour=nam,shape=lifestage))+
#		geom_line(aes(colour=nam,lty=lifestage))+
#		ggtitle("Recruitment European overview")+
#		ylab("scaled 1979-1994 values log scale")+
...
########################################
#g<-ggplot(scaldata)
#(fig_katwijk<-g+geom_point(aes(x=year, y=value_std),col="grey",size=1.5)+
#		geom_line(aes(x=year, y=value_std),col="grey",size=0.4)+
#		geom_point(aes(x=year, y=value_std),colour="blue",size=1,data=scaldata[scaldata$nam=="Katwijk scientific estimate",])+
#		geom_line(aes(x=year, y=value_std),colour="blue",size=1,data=scaldata[scaldata$nam=="Katwijk scientific estimate",])+
#		ylab("scaled 1979-1994 values")+
#		geom_pointrange(data=bootscal,aes(x=year,y=mean,ymin=ymin,ymax=ymax),size=0.5,colour="black")+
#		geom_line(data=scal,aes(x=year,y=geomean),colour="red",size=1.2)+
#		theme(legend.position = "none")+
...
#year_only=glm(value_std_1979_1994~year_f+as.factor(das_ser_id),data=glass_eel_yoy,family=Gamma(link=log),subset=glass_eel_yoy$value>0 & glass_eel_yoy$year>1949 ,maxit=300)
#summary(year_only)
...
model_ge_area=glm(value_std~year_f:area+nam,data=glass_eel_yoy[glass_eel_yoy$value>0 & glass_eel_yoy$year>1959,],
...
vv$modelge$excludednam<-unique(glass_eel_yoy[glass_eel_yoy$nam%in%vv$modelge$excluded,"nam"])
vv$modelge$value_excluded_zero<-glass_eel_yoy[glass_eel_yoy$value==0,c("value_std","nam","year","lifestage","dat_comment","geo")]

model_older=glm(value_std~year_f+as.factor(nam),data=older,family=Gamma(link=log),subset=older$value>0 & older$year>1949  ,maxit=300)
vv$modelolder<-list()
...
vv$modelolder$excludednam<-unique(older[older$nam%in%vv$modelolder$excluded,"nam"])
vv$modelolder$value_excluded_zero<-older[older$value==0,c("value_std","nam","year","lifestage","dat_comment","geo")]
...
# 
require(lme4)
model_ge_area<-gls(value_std~year_f:area+nam,data=glass_eel_yoy,family=Gamma(link=log),
		subset=glass_eel_yoy$value>0 & glass_eel_yoy$year>1959 ,maxit=300)

lmer(value_std~year_f:area+nam,data=glass_eel_yoy,family=Gamma(link=log),
...
M.lm <- gls(lvalue_std~nam,data=glass_eel_yoy)

vf1Fixed <- 
		
		M.gls1 <-glmmPQL(value_std~year_f:area+nam,
...
model_ge_area2=glm(value_std~year_f:area+nam,data=glass_eel_yoy2[glass_eel_yoy2$value>0 ,],
		family=Gamma(link=log), maxit=300)
...
unique(older$nam)
model_older=glm(value_std~year_f+as.factor(nam),data=older,family=Gamma(link=log),subset=older$value>0 &
