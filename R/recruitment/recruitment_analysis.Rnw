...
\graphicspath{{C:/workspace/wgeeldata/recruitment/2017/image/}} 
...
CY<-2017 # current year ==> don't forget to update the graphics path below
# ----------------------------------
...
# here 2017 is the current year of recruitment (I have several folders one for each year)
...
if(getUsername() == 'lbeaulaton')
{
	...
	wddata <- "/home/lbeaulaton/Documents/ANGUILLE/ICES/WGEEL/WGEEL 2017 Kavala/Ranalysis"
	datawd<-str_c(wddata,"/data")
	imgwd<-str_c(wddata,"/image")
	tabwd<-str_c(wddata,"/table")
...
query ='select * from datawg.t_series_ser 
...
# 2017 removing series where ser_qal_id=0;
...
tab1<-rbind(statseries$series_kept,tab1)
tab1[1,1]<-"kept"
createxl<-function(data=tab1,sheet="recruitment_series"){
	library("XLConnect")
	xls.file<-str_c(datawd,"/","table_rec2017.xls")
...
Loire, France). Among those series \Sexpr{sn(vv[["nb_series_final"]])} have been
...
figure_check_one_series("Katw",limits=c(1960,2017))
@
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%************************************************************************************
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Both WGEEL recruitment indices for 2017 are lower than 2014, but modelling a
breakpoint arround the minima of 2011 still gives significant results when using
the lower value from 2017 (p=\Sexpr{sn(sgipee_test_for_change_ee,digits=1)}
Elsewhere Europe and p=\Sexpr{sn(sgipee_test_for_change_ns,digits=1)} North Sea ICES SGIPEE(2011)). The 2017 level with respect to 1960-1979 averages is
...
\tiny
\input{C:/workspace/wgeeldata/recruitment/2017/table/table_glm_glass_eel.tex}
\input{C:/workspace/wgeeldata/recruitment/2017/table/table_glm_yellow.tex}
\normalsize
...
some have reported preliminary data for 2017 as the season is not yet finished (Lagan (SW), Kävlingeån (SW),
 Bresle (FR)).}
Therefore, the indices given for 2017 must be considered as provisional
...
data in 1987 1988 and again from 2007 to 2017).
...
%\small
% %====================================
%\input{C:/workspace/wgeeldata/sweave/2017/table/table_n_y_area.tex}
% %====================================
%\normalsize
%\small
% %====================================
%\input{C:/workspace/wgeeldata/sweave/2017/table/table_n_y_lfs.tex}
% %====================================
%\normalsize
