...
\title{ Analysis of recruitment trend : Draft first run, with incomplete data}
...
\textcolor{Plum}{Adapt here : Among the time-series based on trap indices, some
have reported preliminary data for 2017 as the season is not yet finished (Lagan (SW), Kävlingeån (SW),
...
\end{itemize}
\item New historical data for Bann 
