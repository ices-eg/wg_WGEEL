...
priors2=na.omit(unique(series[,c("ser_nameshort","type","scale_bound_shape1","scale_bound_shape2")]))
priors2=expand_grid(priors2,x=seq(0,1,.01))
priors2 <- priors2 %>%
  mutate(prior=dbeta(x,scale_bound_shape1,scale_bound_shape2))
...
ggplot(scale_trap,aes(x=a))+
  ...
  geom_line(data=filter(priors2,type=="trap"),
            aes(x=x,y=prior),
                col=2)+
...
ggplot(scale_catch,aes(x=p))+
  geom_histogram(stat="density")+
  geom_line(data=filter(priors2,type=="catch"),
            aes(x=x,y=prior),
                col=2)+
  facet_wrap(~ser_nameshort,scales="free_y")+
  theme_bw()
...
catch_pred=do.call(rbind.data.frame,lapply(1:ncol(catch),function(i){
  ...
  tau=0#-0.5/jags_mat[,paste("tauIE[",i,"]",sep="")]
