...
format_country_names <- function(setnames, set){
  paste(paste(setnames[-length(setnames)], 
        paste0("(",set[-length(setnames)],")"), collapse = ", "),
        paste(setnames[length(setnames)], 
        paste0("(",set[length(setnames)],")")),
        sep = " and ")
}
...
if (getUsername() == "rbva0001") setwd("C:/Users/rbva0001/OneDrive/Rob/SLU/ICES/Working groups/WGEEL/GIT/wg_WGEEL/R/advice")
if (getUsername() == "marieke_desender") setwd("G:/Mijn Drive/Paling/ICES/WGEEL/ICES 2026/wg_WGEEL/R/advice")
getwd()
...
CY <- lubridate::year(Sys.Date()) # current year ==> dont forget to update the graphics path below
...
```{r keyfig, include=FALSE, warning = FALSE, message = FALSE, error=FALSE}
landingsYS <- filter_data("landings",
    typ = c(4,6),
    life_stage = c("Y","S","YS"),
    year_range = (CY-10):(CY-1)
)%>% dplyr::filter(is.na(eel_missvaluequal)) |>
  group_by(eel_year) |>
  summarize(value = sum(eel_value, na.rm = TRUE)) |>
  ungroup()
meanlandingsYS <- round(mean(landingsYS$value) / 1000)

landingsG <- filter_data("landings",
    typ = c(4,6),
    life_stage = c("G"),
    year_range = (CY-10):(CY-1)
)%>% dplyr::filter(is.na(eel_missvaluequal)) |>
  ...
  summarize(value = sum(eel_value, na.rm = TRUE)) |>
  ungroup()
meanlandingsG <- round(mean(landingsG$value) / 1000)
...
__Figure 1__ &#9; Indices of glass eel recruitment for the continental “North Sea” 
...
Convention on International Trade of Endangered Species of Wild Fauna and Flora (CITES) in 2007
that came into force in 2009
[@conventiononinternationaltradeiendangeredspeciesofwildfaunaandfloracites2022],
and Appendix II of the Convention for Conservation of Migratory Species of Wild Animals
(CMS) in 2014
...
`r round(mohn_YY/100, 2)` for the ”Yellow eel” index. While exceeding an 
...
(Mohn’s ρ = `r round(mohn_NS/100, 2)`), “Elsewhere Europe” (Mohn’s ρ = `r round(mohn_EE/100, 2)`), and “Yellow eel” (Mohn’s ρ = `r round(mohn_YY/100, 2)`. The GLM 
...
Moreover, in the absence of weighting and because of even spatial coverage,
regions with numerous time-series
...
amounted an average of `r meanlandingsYS ` t per year over the last decade 
(`r paste(CY-10, CY-1, collapse ="-")`), with several
countries reporting annual landings of over 100 tonnes per year. Commercial glass
eel total landings amounted to an average of `r meanlandingsG` t per
year over the same period. Both are in contradiction with the
advice.
...
of glass eel source basins, reliable mortality estimates at each step
...
Global climate change has and will continue to directly and indirectly affect 
both marine and freshwater habitats and species. 
It is highly likely that this will impact on migratory fish such as European eel. 
However, effects of climate change are not well understood and are yet to be
fully quantified.
...
Illegal, unreported, and unregulated (IUU) fishing is known to occur. In addition,
customs seizures indicate that the illegal export of eel could be
substantial. Some countries have collected data on
illegal fisheries and trade (i.e. the seizure of illegal nets as well as the
illegal trade of eels from countries both inside and outside the EU).
However, data insufficient to
...
compute <- stringr::str_extract_all(tab2[,2], "`r.+?`")[[1]]
for (ex in compute){
  fo <- eval(parse(text = gsub("`$","", gsub("`r ", "", ex))))
  for (i in seq_len(nrow(tab2))) tab2[i, 2] <- gsub(ex, fo, tab2[i, 2] |> pull(), fixed = TRUE)
}
flextable(tab2)  |>
  delete_part() |>
  bg(j = 1, bg = "#e8eaea", part = "all") |>
...
Landings data are not complete for the entire natural range of the European
eel.

::: {custom-style="Table Caption"}
__Table 4__&#9; European eel. Commercial landings (tonnes) of glass eels 
(1945–`r CY`), as reported to ICES by France [FR], Spain [ES],
Portugal [PT], Italy [IT] and United Kingdom (GB). Empty cell = no data, 
...
filtered_data <- filter_data("landings",
  ...
  dplyr::filter(eel_value > 0)# removing data from Ireland
...
::: {custom-style="Table Caption"}
__Table 5a__&#9; European eel. Commercial landings (tonnes) of yellow 
and silver eel (1908–`r CY-1`)  as reported to ICES by 
`r format_country_names(set1names, set1)`.
...
::: {custom-style="Table Caption"}
__Table 5b__&#9; European eel. Commercial landings (tonnes) of yellow 
and silver eel (1908–`r CY-1`)  as reported to ICES by 
`r format_country_names(set2names, set2)`.
Empty cell = no data, data not collected, or data not pertinent.
:::
...
::: {custom-style="Table Caption"}
__Table 5c__&#9; European eel. Commercial landings (tonnes) of yellow 
and silver eel (1908–`r CY-1`)  as reported to ICES by
`r format_country_names(set3names, set3)`.
...
(1978–2023)  as reported to ICES by in countries where fisheries exist,
i.e. France (FR) and Spain (ES).
Empty cell = no data, data not collected, or data not pertinent.
:::
...
silver eel (1980–`r CY-1`)  as reported to ICES by
`r format_country_names(set1names, set1)`.
Countries omitted in tables 7a and 7b include those where 
...
estimated (GLM) recruitment for glass eels (period 1950-`r CY`), with 95% prediction intervals
...
recruitment as a function of area, year, and site) was fitted to `r selection_summary$nb_series_glass_eel` time-series,
...
GLM (predicting recruitment as a function of year and site, (period 1950-`r CY-1`))
was fitted to `r selection_summary$nb_series_older`
