...
extract_data = function(data_needed, from_database=TRUE)
{
	...
	list_data_table = data.frame(data_needed = c("Landings", "Aquaculture", "Release", "B0", "Bbest", "Bcurrent", "Sigma A", "Sigma F", "Sigma H", "Potential available habitat", "Mortality in Silver Equivalents", "Sigma F all", "Sigma H all"), table_dbname = c("landings", "aquaculture", "release", "b0", "bbest", "bcurrent", "sigmaa", "sigmaf", "sigmah", "potential_available_habitat","silver_eel_equivalents", "sigmafallcat", "sigmahallcat"))
