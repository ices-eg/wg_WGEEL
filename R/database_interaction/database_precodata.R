...
extract_precodata = function(outer_join=T){
	...
	sigmaa = extract_data("Sigma A",quality=c(1,2,4),quality_check=TRUE)
	sigmaf= extract_data("Sigma F",quality=c(1,2,4),quality_check=TRUE)
	sigmah= extract_data("Sigma H",quality=c(1,2,4),quality_check=TRUE)
