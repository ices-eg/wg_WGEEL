...
total.bcurrent as t_bcurrent, total.nb as t_nb, emu.bcurrent as e_bbest, emu.nb as e_nb FROM
(SELECT TYP_NAME , EEL_YEAR, EEL_COU_CODE, EEL_LFS_CODE, EEL_HTY_CODE , sum(EEL_VALUE) as bcurrent, COUNT(*) as nb
FROM DATAWG.Bcurrent where EEL_EMU_NAMESHORT like '%_total%' and EEL_MISSVALUEQUAL is null
group by TYP_NAME, EEL_YEAR, EEL_COU_CODE, EEL_LFS_CODE, EEL_HTY_CODE  ) as total full outer JOIN
(SELECT TYP_NAME , EEL_YEAR, EEL_COU_CODE, EEL_LFS_CODE, EEL_HTY_CODE  , sum(EEL_VALUE) as bcurrent, COUNT(*) as nb
FROM DATAWG.Bcurrent where EEL_EMU_NAMESHORT not like '%_total%'  and EEL_MISSVALUEQUAL is null
...
where total.bcurrent is not null and emu.bcurrent is not null; -- landings by EMU and in total
